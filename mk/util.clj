...
(defn uv-venv [d]
  (sh! "env UV_VENV_IN_PROJECT=1 uv venv --clear" d true))

(defn uv-compile [d infile lockf torch-index]
  (if (= infile "requirements.gpu.in")
    (sh! (format "env UV_VENV_IN_PROJECT=1 uv pip compile --index %s %s -o %s --index-strategy unsafe-best-match"
                 torch-index infile lockf)
        d true)
    (sh! (format "env UV_VENV_IN_PROJECT=1 uv pip compile --emit-index-url %s -o %s"
                 infile lockf)
        d true)))

(defn uv-sync [d lockf]
  (if (str/includes? lockf "cpu")
    (sh! (format "env UV_VENV_IN_PROJECT=1 uv pip sync %s --index https://download.pytorch.org/whl/cpu --index-strategy unsafe-best-match" lockf) d true)
    (sh! (format "env UV_VENV_IN_PROJECT=1 uv pip sync %s" lockf) d true)))
...
(defn cuda-probe [svc-dir gpu?]
  (when gpu?
    (sh! (str
          "env UV_VENV_IN_PROJECT=1 uv run python - <<'PY'\n"
          "import ctypes, sys\n"
