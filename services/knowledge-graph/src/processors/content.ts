...
export class ContentProcessor {
  ...
  private async discoverFiles(repositoryPath: string): Promise<string[]> {
    ...
    try {
      ...
      this.logger.error(
        'ContentProcessor',
        'Failed to discover files in repository',
        {
          repositoryPath,
          error: error instanceof Error ? error.message : 'Unknown error',
        },
      ...
      throw new Error(
        `Failed to discover files in repository: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
