...
test('PUT /v1/files writes full file', async (t) => {
    await withServer(ROOT, async (req) => {
        const content = 'hello\nworld';
        const res = await req.put('/v1/files').send({ path: TEST_FILE, content }).expect(200);
        t.true(res.body.ok);
...
test('PUT /v1/files returns 400 for missing path', async (t) => {
    await withServer(ROOT, async (req) => {
        const res = await req.put('/v1/files').send({ content: 'nope' }).expect(400);
        t.false(res.body.ok);
    });
});
...
test('PUT /v1/files returns 400 for missing content/lines', async (t) => {
    await withServer(ROOT, async (req) => {
        const res = await req.put('/v1/files').send({ path: TEST_FILE }).expect(400);
        t.false(res.body.ok);
    });
});
