...
export class AgentSupervisor {
    ...
    stream(id, res) {
        ...
        if (replay) {
            res.write(`event: replay\ndata: ${JSON.stringify({ text: replay.slice(-8192) })}\n\n`);
        }
        res.on('close', () => {
    ...
    start({ prompt, args = [], cwd = ROOT_PATH, env = {}, auto = true, tty = true }) {
        const id = nanoid();
        ...
        if (wantTty) {
            ...
            const cmd = [shQuote(CODEX_BIN), 'exec', '--full-auto', '--cd', shQuote(root)]
        ...
        proc.on('exit', (code, signal) => {
            ...
            updateAgentMeta(id, { exited: true, code, signal, finishedAt: Date.now() }).catch(
                () => {},
            );
        });
...
export async function restoreAgentsFromStore() {
    ...
    for (const meta of metas) {
        const exists = (meta.mode === 'pty' ? ptySupervisor : supervisor).procs.has(meta.id);
        if (exists) continue;
