...
export function createAuth() {
    ...
    const mount = (app) => {
        app.get('/auth/me', async (req, res) => {
            if (!enabled) return res.json({ ok: true, auth: false, cookie: cookieName });
            ...
            try {
                if (mode === 'static') {
                    ...
                    if (!ok) return res.status(401).json({ ok: false, error: 'unauthorized' });
                    return res.json({ ok: true, auth: true, mode: 'static', cookie: cookieName });
                }
                if (mode === 'jwt') {
                    try {
                        await verifyJwtAny(t);
                        return res.json({ ok: true, auth: true, mode: 'jwt', cookie: cookieName });
                    } catch (err) {
                        const msg = String(err?.message || err);
                        if (/missing jwks/.test(msg) && jwtSecret) {
