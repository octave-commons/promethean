...
function resolveRef(spec, schema) {
    ...
    if (schema.$ref && typeof schema.$ref === 'string') {
        ...
        if (!m) return schema;
        const path = m[1].split('/').map((s) => s.replace(/~1/g, '/').replace(/~0/g, '~'));
        let cur = spec;
...
class ApiResponseViewer extends LitElement {
    ...
    `;
    render() {
        const content = this.data ? JSON.stringify(this.data, null, 2) : this.text || '';
        return html`
...
class ApiRequestForm extends LitElement {
    ...
    buildControls(schema, path = []) {
        ...
        if (type === 'object' && Object.keys(props).length) {
            ...
            controls.push(
                this.renderInput(path.join('.'), schema, required.includes(path[path.length - 1])),
            );
        }
        return controls;
    }
    ...
    renderInput(name, schema, required) {
        ...
        if (enums && Array.isArray(enums)) {
            control = html`<select id=${id} data-name=${name} .value=${schema.default ?? enums[0]}>
                ${enums.map((v) => html`<option value=${String(v)}>${String(v)}</option>`)}
            </select>`;
        } else if (t === 'boolean') {
            control = html`<input
                id=${id}
            ...
            control = html`<textarea id=${id} data-name=${name} placeholder="{ }"></textarea>`;
        } else {
            control = html`<input
                id=${id}
                data-name=${name}
        ...
        return html`
            <div class="field">
                <label for=${id}
                    >${title}${required ? html` <span title="required">*</span>` : ''}</label
                >
    ...
    render() {
        ...
        const paramControls = (this.parameters || []).map((p) => this.renderParamInput(p));
        return html`
...
class ApiEndpointCard extends LitElement {
    ...
    getRequestExamples() {
        return this.op?.requestBody?.content?.['application/json']?.examples || null;
    }
    getParameters() {
    ...
    async send(e) {
        ...
        let url = this.path;
        const pathParams = (this.op?.parameters || []).filter((p) => p.in === 'path');
        for (const p of pathParams) {
            const v = params?.[p.name] ?? '';
            url = url.replace(`{${p.name}}`, encodeURIComponent(v));
        }
        const queryParams = (this.op?.parameters || []).filter((p) => p.in === 'query');
    ...
    render() {
        ...
        const favs = new Set(JSON.parse(localStorage.getItem('apiFavorites') || '[]'));
                ...
                ${this.streaming
                    ...
                    : this.response !== null
                      ? html`<api-response-viewer
                            .data=${typeof this.response === 'string' ? null : this.response}
...
class ApiDocs extends LitElement {
    ...
    render() {
        ...
        for (const [path, ops] of Object.entries(this.spec.paths || {})) {
            for (const m of methods) if (ops[m]) entries.push({ method: m, path, op: ops[m] });
        }
        const tags = new Set();
        for (const e of entries) {
            if (Array.isArray(e.op.tags) && e.op.tags.length) e.tag = e.op.tags[0];
            else e.tag = (e.path.split('/')[1] || 'General').replace(/\{.*\}/, '') || 'General';
            tags.add(e.tag);
        }
        ...
        return html`
            <div class="row">
                ...
                >
                    ...
                    ${Array.from(tags)
                        .sort()
                        .map((t) => html`<option ?selected=${this.tag === t}>${t}</option>`)}
                </select>
            </div>
            ${order
