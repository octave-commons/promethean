...
test('enqueues processing tasks for board and file changes', async (t) => {
    ...
    for (const client of wss.clients) {
        client.send(JSON.stringify({ event: { type: 'file-watcher-board-change' } }));
        client.send(JSON.stringify({ event: { type: 'file-watcher-task-change' } }));
    }

    await wait(100);
    t.truthy(messages.find((m) => m.action === 'enqueue' && m.task?.kind === 'board'));
    t.truthy(messages.find((m) => m.action === 'enqueue' && m.task?.kind === 'tasks'));
