...
export async function handleSocialMessageCreated(evt: any) {
    ...
    console.log(`[discord-message-indexer] upsert into ${tenantCfg.storage.mongo_db}.messages`, {
        key: { provider: doc.provider, tenant: doc.tenant, foreign_id: doc.foreign_id },
    });
    return { db, doc };
}
