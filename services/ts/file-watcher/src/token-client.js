async function fetchToken(cfg) {
    try {
        ...
        if (!body?.access_token)
            return null;
        return { access_token: String(body.access_token), expires_in: body.expires_in };
    }
    catch {
        return null;
    }
...
export function createTokenProviderFromEnv() {
    ...
    return async () => {
        ...
        if (cached && now < cached.expires_at - 60)
            return cached.access_token; // refresh 60s early
        const tok = await fetchToken(cfg);
        if (!tok)
            return undefined;
        const ttl = typeof tok.expires_in === 'number' && tok.expires_in > 0 ? tok.expires_in : 3600;
