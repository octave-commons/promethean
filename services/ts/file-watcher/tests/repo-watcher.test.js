...
test('repo watcher posts index/remove for non-ignored files', async (t) => {
    ...
    const indexCalls = calls.filter((c) => c.url.endsWith('/indexer/index') && c.body.path === 'src/example.ts');
    t.true(indexCalls.length === 1);
    t.true(calls.some((c) => c.url.endsWith('/indexer/remove') && c.body.path === 'src/old.ts'));
