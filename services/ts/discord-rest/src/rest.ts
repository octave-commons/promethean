...
export class DiscordRestProxy {
    ...
    routeForPostMessage(spaceUrn: string) {
        ...
        return { method: 'POST', route: `/channels/${channelId}/messages` } as const;
    }

    routeForDeleteMessage(spaceUrn: string, messageId: string) {
        const parts = spaceUrn.split(':');
        const channelId = parts[parts.length - 1];
        return { method: 'DELETE', route: `/channels/${channelId}/messages/${messageId}` } as const;
    }
    ...
    async send(
        ...
        if (!this.limiter.canSend(bucket)) {
            return {
                ...
                status: 429,
                bucket,
                retry_after_ms: Math.max(0, (this as any).limiter.until.get(bucket) - Date.now()),
            };
        }
