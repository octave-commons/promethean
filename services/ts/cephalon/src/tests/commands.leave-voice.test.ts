...
test('leave-voice command defers and dispatches event', async (t) => {
    ...
    const ctx = { bot: {} as any, dispatch: async (e: any) => void dispatched.push(e) };
    ...
    t.deepEqual(dispatched[0], { type: 'VOICE/LEAVE_REQUESTED', guildId: 'g1', by: 'u1' });
    t.deepEqual(interaction.calls[interaction.calls.length - 1], ['editReply', 'Leaving voice…']);
});
