...
export async function renderWaveForm(channelData: Float32Array, { width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT }) {
	...
	return annotateImage(pngBuffer, getCurrentDateTime(), 'Wave form of Desktop Audio');
}
...
export async function captureAndRenderWaveform({ duration = 5, width = 1024, height = 1024 } = {}) {
	...
	const { waveBuffer, channelData }: { waveBuffer: Buffer; channelData: Float32Array } = await captureAudio({
		duration,
	});
