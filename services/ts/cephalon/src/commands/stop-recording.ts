...
import { buildRecordSpeakerScope, makeRecordActions } from '../actions/record-speaker.scope.js';
...
export default async function execute(interaction: ChatInputCommandInteraction, ctx: { bot: Bot }) {
    ...
    const scope = { ...(await buildRecordSpeakerScope()), ...makeRecordActions() };
    const { ok } = await scope.stop({ bot: ctx.bot, user });
    await interaction.editReply(ok ? "I'm not recording you anymore… I promise…" : 'No active voice session.');
}
