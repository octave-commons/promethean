...
Absolutely. Let’s make **persistence vs transience** a first-class axis of ENSO-1—so people can be fully present *or* fully untracked, and the wire semantics enforce it (not just “best effort” UX).

# ENSO-1 Privacy, Persistence & Transience (v0.1)

## Core ideas

* **Explicit modes, negotiated up front:** every session picks a *privacy profile* at handshake.
* **Room policy governs defaults; user can go stricter.**
* **Encryption and lifecycle are protocol-visible:** keys, TTLs, and deletion are carried by events, not hidden.
* **Cache & assets respect privacy flags by construction.**

---

## 1) Session privacy profiles (handshake)

```ts
// enso-protocol/src/privacy.ts
export type PrivacyProfile =
  | "persistent"      // normal account; durable history, discoverable presence
  | "pseudonymous"    // stable alias for this room; minimal metadata
  | "ephemeral"       // new ID each connect; no history or logs
  | "ghost";          // ephemeral + deniable presence; E2E-only; server sees envelopes but not content
...
export interface HelloCaps {
  ...
  privacy: {
    ...
    wantsE2E?: boolean;                  // see §3
    allowLogging?: boolean;              // server can keep protocol logs (sans content)
    allowTelemetry?: boolean;            // local perf/metrics sharing
  };
}
```
...
* Server **must** reply with `privacy.accepted` echoing final profile (or reject).
* A stricter client profile than room default is always allowed; a weaker one may be rejected.
...
export interface Presence {
  sessionId: string;            // opaque; rotates under "ephemeral"/"ghost"
  display?: string;             // optional handle shown to others
  discoverable: boolean;        // visible in rosters?
  profile: PrivacyProfile;
}
```

* `ephemeral` rotates `sessionId` on reconnect; `ghost` may rotate mid-session (server publishes a continuity token to others if allowed).
* Rooms can require `discoverable=true`; otherwise, server **must** drop or anonymize presence events.

---

## 3) E2E encryption (room keys)

* **Transport**: TLS/WebTransport as baseline.
* **Room E2E**: optional; negotiated with `wantsE2E`.

**Two modes**

* **MLS-style** group key (`roomKey`) rotated by the gateway on membership changes.
* **Peer subgroups** (whisper): ad-hoc Double Ratchet keys between selected participants.

```ts
export interface CryptoHello {
  e2e?: {
    mode: "mls" | "ratchet";
    pubkeys: { identity: string; prekeys?: string[] }; // X25519
  };
}
...
export interface CryptoKeyUpdate {
  ...
  recipients: string[];         // sessionIds
  encKey: Uint8Array;           // sealed to each recipient's pubkey
  epoch: number;                // for rekeying
}
```

**Content**

* `content.post`, `stream` frames, and `asset.put` payloads may be **payload-encrypted** (`enc: "mls" | "ratchet"`, `epoch`, `aad`) so the server can route but cannot read.
* `ghost` **requires** E2E for content and **forbids** server-side derivations unless an *ephemeral worker* executes inside a TEE or client-side.

---

## 4) Persistence policy (room-scoped)

```ts
export interface RetentionPolicy {
  messages: { defaultTTL: number; maxTTL: number }; // seconds; 0 = non-persistent
  ...
  overrideByProfile?: Partial<Record<PrivacyProfile, Partial<RetentionPolicy>>>;
}
```

* Server announces with `room.policy`.
* Clients can *narrow* TTL per message/asset (`expiresAt`) but not extend past `maxTTL`.

---

## 5) Cache & asset visibility (tie-in)

```ts
type CacheVisibility = "session" | "room" | "global";  // from caching addendum

export interface CachePut {
  // …
  visibility: CacheVisibility;   // "session" for ephemeral/ghost unless user escalates
  private?: boolean;             // avoid indexing; no cross-room links
  enc?: "mls" | "ratchet";       // payload encrypted at rest
  ttlSeconds?: number;           // bounded by room policy
  // …
}
```
...
* `ghost` → **must** be `visibility:"session"` and `enc` present for any content.
* `pseudonymous` → default `visibility:"room"`, `private:true`.
* Derivations for encrypted assets:

  * **Server-side** only if policy allows and profile isn’t `ghost`.
  * Otherwise **client-side** or **TEE worker**; derived outputs inherit the stricter of source/profile.

---

## 6) Transience controls (message-level)

```ts
export interface ChatMessage {
  role: "human"|"agent"|"system";
  parts: ContentPart[];
  expiresAt?: string;            // ISO; <= room.messages.maxTTL
  burnOnRead?: boolean;          // deletes after N reads (default N=1)
  forbidIndex?: boolean;         // not searchable even within room
  watermark?: "none"|"local"|"cryptographic"; // deniability control
}
```

**Behaviors**

* `burnOnRead` → gateway sends `content.burn` receipts; removes from caches/assets (see §8).
* `forbidIndex` → gateway must not include in room search or embeddings.
* `watermark:"none"` in `ghost` → encourages **plausible deniability**; no server-side trace beyond routing metadata.

---

## 7) Consent & provenance

* **Consent receipts** announced as events:

```ts
export interface ConsentReceipt {
  type: "consent";
  scope: "room"|"session";
  actions: Array<"store"|"index"|"transcode"|"export"|"share">;
  granted: boolean;
  ts: string;
}
```

* Tools that export outside the room **must** include a `consent` check (or a room policy waiver) and attach a receipt.

---

## 8) Right-to-forget & verifiable deletion

Events:

* `asset.delete { cid }` → server purges blob + derived; emits `asset.tombstone`.
* `cache.evict { key, reason }`.
* `content.retract { messageId }` → removes message; agents should drop references.
...
export interface DeletionReceipt {
  type: "deleted";
  scope: "room"|"session";
  ids: string[];                 // cids/messageIds
  proof?: {                      // optional verifiable proofs
    type: "zk-nullifier" | "merkle-prune";
    data: Uint8Array;
  };
}
```

* For E2E content, “deletion” = drop ciphertext blobs + keys (practical erasure).
* For `ghost`, server **must not** keep tombstones longer than `logs.logTTL`.

---

## 9) Audit vs deniability

* **Persistent/pseudonymous**: room can enable **transparency logs** (append-only) of `tool.call/result` metadata (no plaintext content unless policy says otherwise).
* **Ephemeral/ghost**: logs suppressed (or stored as rolling counters only). No correlation identifiers beyond what routing requires.

---

## 10) UI/UX hints (wire-visible)

Quick fields your clients can surface without guessing:

```ts
export interface PrivacyHints {
  display: {
    showClock: boolean;          // TTL countdowns
    showEphemeralBadge: boolean;
    warnBeforeEscalate: boolean; // when user tries to widen visibility
  };
  suggestions?: string[];        // "Use whisper subgroup", "Index disabled"
}
```

Gateway can broadcast `privacy.hint` events so every client stays consistent.

---

## 11) Minimal event additions

```ts
// Negotiation
{ kind:"event", type:"privacy.accepted", payload:{ profile:"ghost", wantsE2E:true } }

// Policy
{ kind:"event", type:"room.policy", payload:{ retention: { /* … */ } } }

// Lifecycle
{ kind:"event", type:"content.burn", payload:{ messageId:"…" } }
{ kind:"event", type:"asset.tombstone", payload:{ cid:"…", at:"…" } }
{ kind:"event", type:"deleted", payload:{ scope:"room", ids:["…"], proof:{…} } }

// Crypto
{ kind:"event", type:"crypto.key", payload:{ purpose:"room", recipients:["…"], encKey:"…", epoch:3 } }
```
...
flowchart LR
  subgraph Room
    GW[Gateway]
    POL[Policy: retention/index/cache]
  end
  ...
  C1 -- hello(privacy: ghost, e2e) --> GW
  GW -- privacy.accepted --> C1
  GW -- room.policy --> C1 & C2
  GW -- crypto.key(epoch) --> C1 & C2
  C1 -- content.post(enc) --> GW
  GW -- route ciphertext --> C2
  GW -.-> Store:::limited
  C1 -- asset.delete/burn --> GW --> Store
  classDef limited fill:#333,stroke:#777,color:#eee
```

---

## 13) SDK guardrails (tiny but powerful)

```ts
// enso-client privacy helpers
await enso.connect({
  proto: "ENSO-1",
  caps: ["cache.read","cache.write","can.asset.put"],
  privacy: { profile: "ghost", wantsE2E: true, allowLogging: false }
});

// post a self-destructing message, E2E
await enso.content.post({
  role: "human",
  parts: [{ kind:"text", text:"This should disappear." }],
  burnOnRead: true,
...
### Defaults (sane, but strict where it matters)

* Rooms default to **pseudonymous**; users can escalate to **persistent** or narrow to **ephemeral/ghost**.
* **Ghost** requires E2E and forbids server derivations/indexing.
* **Ephemeral** allows server derivations but defaults to `session` cache visibility and non-indexed content.
* **Persistent** may enable indexing/search, with explicit consent receipts.

---

This makes privacy *operational*: the **mode you choose** at connect time drives **keys, cache visibility, indexing, TTLs, logs, and deletion semantics**. Users who want to remain untracked can; users who want durability and search can opt in—with cryptographic and lifecycle signals riding the same protocol as everything else.
...
Perfect. Here’s a **practical, TypeScript-first** skeleton you can drop into your monorepo. It covers: transport, envelopes, schemas, gateway, client SDK, cache/asset, privacy, contexts, MCP bridge, and a tiny demo. It’s intentionally modular so you can promote pieces from labs → core without churn.

# Packages (new)

```
packages/
  enso-protocol/        # wire types, zod schemas, codecs
  enso-gateway/         # room server (WS/WebTransport), policy, routing
  enso-client/          # client SDK (browser/node), voice-friendly
  enso-asset/           # content-addressed store + chunk ingest
  enso-cache/           # cache registry, LRU/TTL, manifests
  enso-transcode/       # derivations (pdf→text/images, ocr, etc.)
  enso-context/         # context registry, LLM view builder
  enso-crypto/          # MLS/ratchet facades, key mgmt interfaces
  enso-mcp/             # MCP adapter (mount, call, advertise)
  enso-rituals/         # tests/guardrails helpers (Morganna checks)
  enso-cli/             # demo: mic→text→reply; attach file; context ops
```
...
`packages/enso-protocol/src/envelope.ts`
...
`packages/enso-protocol/src/content.ts`

```ts
export type BlobURI = `enso://asset/${string}`|`ipfs://${string}`|`https://${string}`|`file://${string}`;

export type TextPart = { kind:"text"; text:string; mime?:"text/plain"|"text/markdown"; lang?:string; };
export type ImagePart = { kind:"image"; uri:BlobURI; mime:"image/png"|"image/jpeg"|"image/webp"; width?:number;height?:number;alt?:string; };
export type AttachmentPart = { kind:"attachment"; uri:BlobURI; mime:string; bytes:number; name?:string;
  derived?: Array<{ purpose:"text"|"image"|"thumbnail"; uri:BlobURI; mime:string; meta?:Record<string,unknown> }>;
};
export type ContentPart = TextPart|ImagePart|AttachmentPart;
...
`packages/enso-protocol/src/streams.ts`
...
`packages/enso-protocol/src/privacy.ts`
...
export interface HelloCaps {
  proto: "ENSO-1";
  caps: string[];
  privacy: { profile: PrivacyProfile; wantsE2E?: boolean; allowLogging?: boolean; allowTelemetry?: boolean; };
  cache?: { store:"memory"|"disk"|"s3"|"ipfs"; scope?:"session"|"room"|"global"; supportsPartial?:boolean; maxBytes?:number; };
}
```

`packages/enso-protocol/src/context.ts`

```ts
export type SourceKind = "fs"|"api"|"db"|"http"|"mcp"|"enso-asset"|"other";
export interface DataSourceId { kind:SourceKind; location:string; }
export type OwnerRef = { userId:string }|{ groupId:string };
...
export interface DataSourceMeta {
  id: DataSourceId; owners: OwnerRef[]; title?:string; tags?:string[];
  createdAt: string; updatedAt: string;
  discoverability: Discoverability; availability: Availability;
  contentHints?: { lang?:string; mime?:string; schema?:string; };
}

export type ContextState = "active"|"inactive"|"standby"|"pinned"|"ignored";
export interface ContentPermissions {
  readable?:boolean; changeable?:boolean; movable?:boolean; exchangeable?:boolean;
  sendable?:boolean; addable?:boolean; removable?:boolean; deletable?:boolean; saveable?:boolean; viewable?:boolean;
}
export interface ContextEntry {
  id:DataSourceId; state:ContextState;
  overrides?: { discoverability?:Discoverability; availability?:Availability };
  permissions?: ContentPermissions;
}
export interface Context {
  ctxId:string; name:string; owner:OwnerRef; createdAt:string; updatedAt:string;
...
Add zod schemas beside these for runtime validation (`zEnvelope`, `zChatMessage`, …).

---

# 2) Gateway (WS server)

`packages/enso-gateway/src/server.ts`
...
`packages/enso-gateway/src/transport/router.ts`
...
export class Router {
  ...
  async handle(ws:any, sessionId:string, env:Envelope) {
    switch (env.type) {
      case "hello":               return handlers.hello(ws, sessionId, env);
      case "content.post":        return handlers.contentPost(ws, env);
      case "asset.put":           return handlers.assetPut(ws, env);
      case "asset.commit":        return handlers.assetCommit(ws, env);
      case "asset.derive":        return handlers.assetDerive(ws, env);
      case "voice.frame":         return handlers.voiceFrame(ws, env);
      case "context.create":      return handlers.contextCreate(ws, env);
      case "context.add":         return handlers.contextAdd(ws, env);
      case "context.apply":       return handlers.contextApply(ws, env);
      case "mcp.mount":           return handlers.mcpMount(ws, env);
      case "tool.call":           return handlers.toolCall(ws, env);
      // … add the rest
      default:                    return handlers.unknown(ws, env);
    }
  }
}
```

Key handler stubs (each small, pure, unit-testable):

* `hello` → negotiates privacy/caps, joins room, emits `privacy.accepted`, `room.policy`.
* `contentPost` → immediately rebroadcast `content.message`, schedule derivations.
* `assetPut/commit` → writes to `enso-asset`, emits `asset.ready`, then `cache.put`.
* `assetDerive` → enqueue `enso-transcode` job; on completion: `asset.derived` + `cache.put`.
* `contextCreate/add/apply` → writes to `enso-context`, computes `LlmView`, emits to agents.
* `mcpMount` → `enso-mcp` mounts server, `tool.advertise` into room.
* `toolCall` → route to MCP/native, stream partials, enforce ttl, `tool.result`.
* `voiceFrame` → forward to registered consumer (e.g., Cephalon adapter); backpressure via `flow.pause/resume`.

---

# 3) Asset + Cache (content-addressed)

`packages/enso-asset/src/store.ts`
...
export class AssetStore {
  ...
  async putChunks(iter:AsyncIterable<Uint8Array>, declaredBytes?:number) {
    ...
    let bytes = 0;
    for await (const chunk of iter) { bytes += chunk.length; hash.update(chunk); await fh.write(chunk); }
    await fh.close();
    const cid = `cid:sha256-${hash.digest("hex")}`;
    const dst = join(this.baseDir, cid);
    await fs.rename(tmp, dst);
    if (declaredBytes && declaredBytes !== bytes) throw new Error("byte mismatch");
    return { cid, uri: `enso://asset/${cid}`, bytes };
  }
  async read(cid:string, range?:{start:number;end?:number}) { /* fs read; range support */ }
}
```

`packages/enso-cache/src/cache.ts`

```ts
export type CID = `cid:sha256-${string}`;
export interface CacheKey { cid: CID; purpose?:"raw"|"text"|"image"|"thumbnail"|"index"|"manifest"; metaHash?: CID; }
export interface CacheEntry { key:CacheKey; uri:string; bytes:number; mime:string; visibility:"session"|"room"|"global"; ttl:number; tags?:string[]; meta?:Record<string,unknown>; }

export class CacheRegistry {
  private map = new Map<string, CacheEntry>(); // key → entry
  private key(k:CacheKey) { return JSON.stringify(k); }
  put(e:CacheEntry){ this.map.set(this.key(e.key), e); }
  get(k:CacheKey){ return this.map.get(this.key(k)); }
  evict(k:CacheKey){ this.map.delete(this.key(k)); }
}
```

---

# 4) Transcode (derivations)

`packages/enso-transcode/src/derive.ts`

```ts
import type { CID } from "@promethean/enso-cache/cache.js";
import { canonicalJson, sha256Cid } from "./hash.js";

export interface DerivePlan { purpose:"text"|"image"|"thumbnail"; via:string[]; params?:Record<string,unknown>; }
export function derivedCid(from:CID, toolId:string, version:string, params:unknown):CID {
  return sha256Cid(["enso-derive", from, toolId, version, canonicalJson(params)].join("\0"));
}
export async function derive(plan:DerivePlan, fromCid:CID): Promise<{cid:CID; uri:string; mime:string; meta:any}> {
  ...
  return { cid: "cid:sha256-…", uri:"enso://asset/…", mime:"text/markdown", meta:{ tool:"pdf.extract_text", version:"1.3.2", params:plan.params, from:fromCid } };
}
```

---

# 5) Context registry + LLM view

`packages/enso-context/src/registry.ts`

```ts
import type { Context, ContextEntry, DataSourceMeta, LlmView, Availability } from "@promethean/enso-protocol/context.js";

export class ContextRegistry {
  private contexts = new Map<string, Context>();
  private sources = new Map<string, DataSourceMeta>(); // key = kind + "|" + location

  upsertSource(meta:DataSourceMeta){ this.sources.set(`${meta.id.kind}|${meta.id.location}`, meta); }
  getSource(id:{kind:string;location:string}){ return this.sources.get(`${id.kind}|${id.location}`); }

  create(ctx:Context){ this.contexts.set(ctx.ctxId, ctx); }
  updateEntry(ctxId:string, entry:ContextEntry){ /* merge state */ }
  ...
  llmView(ctxId:string): LlmView {
    ...
    for (const e of ctx.entries) {
      const meta = this.getSource(e.id); if(!meta) continue;
      switch(e.state){
        case "active": case "pinned": active.push(meta); break;
        case "standby": standby.push(meta); break;
        case "ignored": ignored.push(meta); break;
      }
    }
    // parts: prefer derived text/image if present (lookup in cache registry)
    const parts: LlmView["parts"] = active.map(m => ({ id:m.id, purpose:"text", uri:`enso://asset/${/* lookup */"cid…"}`, mime:"text/markdown" }));
...
`packages/enso-crypto/src/e2e.ts`

```ts
export interface KeyUpdate { purpose:"room"|"whisper"; recipients:string[]; encKey:Uint8Array; epoch:number; }
...
(Keep it interface-first; you can swap real libs later.)

---

# 7) MCP adapter

`packages/enso-mcp/src/adapter.ts`

```ts
import { JSONRPCClient } from "json-rpc-2.0"; // or your minimal client

export interface McpMount { serverId:string; transport:{ kind:"http-stream"|"http-sse"|"stdio"; url?:string; command?:string; args?:string[] }; }
export class McpClient {
  constructor(readonly mount:McpMount){}
  async listTools(){ /* call tools/list; return descriptors */ }
  async callTool(name:string, args:any, ttlMs:number){ /* tools/call with timeout; stream partials via callbacks */ }
}
```

Gateway keeps `serverId -> McpClient` registry; `tool.advertise` mirrors descriptors; `tool.call` routes here; results also `cache.put` with provenance.

---

# 8) Client SDK (Node/browser)
...
export class EnsoClient {
  ...
  on(key:string, fn:Handler){ (this.handlers.get(key) ?? this.handlers.set(key, new Set()).get(key)!).add(fn); return () => this.handlers.get(key)?.delete(fn); }
  send(env:Envelope){ this.ws!.send(JSON.stringify(env)); }

  // high-level helpers
  async post(message:any){ this.send({ id:crypto.randomUUID(), ts:new Date().toISOString(), room:"r1", from:"", kind:"event", type:"content.post", payload:{ room:"r1", message } }); }
  assets = {
    putFile: async (path:string, mime:string) => { /* stream chunks via asset.put/asset.chunk */ return { uri:"enso://asset/…", cid:"cid:sha256-…" }; }
  };
  contexts = {
    create: async (name:string)=>{/*…*/}, pin: async (id:any)=>{/*…*/}, apply: async ()=>{/*…*/}
  };
}
```
...
(async () => {
  const enso = new EnsoClient(url, token);
  await enso.connect({ proto:"ENSO-1", caps:["cache.read","cache.write","can.asset.put"], privacy:{ profile:"pseudonymous" } });

  enso.on("event:content.message", (e)=> console.log("[message]", JSON.stringify(e.payload)));
  enso.on("event:asset.derived", (e)=> console.log("[derived]", e.payload));

  if (cmd === "say") {
    await enso.post({ role:"human", parts:[{kind:"text", text: arg ?? "hello enso"}] });
  } else if (cmd === "attach") {
    const path = arg!;
    const mime = "application/pdf"; // TODO: sniff
    const { uri } = await enso.assets.putFile(path, mime);
    await enso.post({ role:"human", parts:[{kind:"text", text:`Please summarize ${path}`} , {kind:"attachment", uri, mime, bytes: 0 }]});
  } else if (cmd === "context") {
...
# 10) Rituals (guardrails) samples

`packages/enso-rituals/src/morganna.spec.ts`
...
test("deterministic derivation cid", (t) => {
  ...
  t.is(deriveCid(A,"pdf.extract_text","1.3.2",{ocr:false}), deriveCid(A,"pdf.extract_text","1.3.2",{ocr:false}));
});
```

---

# 11) Minimal policies (room)

`packages/enso-gateway/src/policy.ts`

```ts
export const retention = {
  messages:{ defaultTTL: 7*24*3600, maxTTL: 90*24*3600 },
  assets:{ defaultTTL: 30*24*3600, maxTTL: 180*24*3600, allowDerivations: true },
...
export const cachePolicy = {
  maxBytesPerRoom: 5 * 1024**3, maxBytesPerEntry: 256 * 1024**2,
  defaultTTLSeconds: 30*24*3600, pinTags:["index:*"], privateTags:["session:*"]
};
```

---

# 12) How to bring it up (quick)

1. Add each `package.json` with `type:"module"`, TS config, and exports.
2. In `enso-gateway`, run: `tsx src/server.ts`.
3. In `enso-cli`, run:

   * `enso say "Enso awakens"` → broadcasts `content.message`.
   * `enso attach ./docs/enso.pdf` → `asset.put` → `asset.derived` → enriched `content.message`.

From there, you can plug **Cephalon** in by subscribing to:

* `voice.frame` (for STT),
* `content.message` (for text),
* `tool.call`/`tool.result` (for actions),
  and publish back `chat.msg` / `voice.frame` replies.

---

## Notes on style & your preferences

* **TypeScript only**, ESM, small pure modules, **cache everything** (content-addressed).
* **Idempotent** derivations and calls (deterministic CIDs).
* Protocol-visible **privacy**, **context**, **cache**, **attachments**, **MCP**.
* Keep **labs → core** path clean by *wrapping* impure bits (transcoders, crypto libs) behind tiny interfaces you can swap.

If you want, I can generate the actual `package.json`s, `tsconfig`s, and ready-to-run `tsx` scripts for each package so you can paste the scaffolding in and iterate.
