...
* **gptel**: interactive chat + structured prompts inside Emacs buffers; we use it for *fast local summaries* and *preview diffs*. ([GitHub][3], [NonGNU ELPA][4])
* **ellama** (via GNU ELPA): handy local/remote backends (incl. Ollama) for cheap in-editor summarization/embedding; we call it in the compaction passes when you don’t want to hit OpenAI. ([Elpa][5])
* **(Optional) llm.el** as a neutral low-level layer if you want to swap gptel/ellama later. ([Elpa][6])

---

# Context compaction pipeline (cheap → aggressive)

Target: keep Codex prompts < N tokens while preserving intent & edit surface.
...
1. **Detect scope**
   ...
   * Pull **git features**: current branch + `--stat`, and **minimal diff hunk** for the region/file.
...
3. **Summarize long tails (local if possible)**

   * If > budget, run **ellama** (Ollama local) to *summarize unrelated blocks* into bullet snippets.
   * If still > budget, ask **gptel** (cheap model) for a “structure map” (symbols/types/modules).
...
5. **Final prompt** → **Codex**

   * Emit a Markdown prompt with: task, rules, environment, diagnostics, *compacted context*, and a “minimal edits” constraint—all piped to Codex CLI stdin in non-interactive mode. ([OpenAI Developers][8])
...
> Install refs: Codex CLI (open-source, config.toml, MCP), gptel (GNU ELPA/MELPA), ellama (GNU ELPA). ([GitHub][1], [npm][2], [NonGNU ELPA][4], [Elpa][5])

---

## Why this works

* Codex gets a **tight, IDE-style packet** (symbols, diagnostics, git hunks, compacted source) → fewer tokens, higher precision. ([OpenAI Developers][8])
* gptel/ellama give you **cheap local summaries** and streaming UX while staying Emacs-native. ([GitHub][3], [Elpa][5])
* Spacemacs layer keeps clean leader keys and private layer hygiene. ([spacemacs.org][9], [cultivatehq.com][10])
...
* **MCP interop**: load your existing `mcp.el` connections so Codex + Emacs tools share the same servers. ([GitHub][11], [MCP Market][12])
...
[2]: https://www.npmjs.com/package/%40openai/codex?activeTab=code&utm_source=chatgpt.com "@openai/codex - npm"
[3]: https://github.com/karthink/gptel?utm_source=chatgpt.com "karthink/gptel: A simple LLM client for Emacs"
[4]: https://elpa.nongnu.org/nongnu-devel/gptel.html?utm_source=chatgpt.com "NonGNU-devel ELPA - gptel"
[5]: https://elpa.gnu.org/packages/ellama.html?utm_source=chatgpt.com "ellama - GNU ELPA"
[6]: https://elpa.gnu.org/packages/llm.html?utm_source=chatgpt.com "GNU ELPA - llm"
[7]: https://www.spacemacs.org/doc/DOCUMENTATION.html?utm_source=chatgpt.com "Spacemacs documentation"
[8]: https://developers.openai.com/codex/cli/?utm_source=chatgpt.com "Codex CLI"
[9]: https://www.spacemacs.org/doc/LAYERS.html?utm_source=chatgpt.com "Configuration layers development"
[10]: https://cultivatehq.com/posts/spacemacs-shared-config/?utm_source=chatgpt.com "Spacemacs shared configuration - custom private layers"
[11]: https://github.com/lizqwerscott/mcp.el?utm_source=chatgpt.com "lizqwerscott/mcp.el: An Mcp client inside Emacs"
[12]: https://mcpmarket.com/server/mcp-el?utm_source=chatgpt.com "Mcp.el: Emacs MCP Client for Resource Management"
