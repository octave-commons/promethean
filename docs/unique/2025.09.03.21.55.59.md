---
uuid: 014e93e7-68ff-4353-85f9-08ce58631c2f
created_at: 2025.09.03.21.55.59.md
...
async function batchEmbed(chunks) {
  const response = await fetch('http://ollama.com/embed/batch', {
    method: 'POST',
...
if (isMainThread) {
  ...
  parentPort.on('message', async (chunk) => {
    const response = await fetch('http://ollama.com/embed', {
      method: 'POST',
