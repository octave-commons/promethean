Implement a `kanban heal <reason> --config kanban.scar.json` command Utilizing vector embeddings for cosine similarity and nearest neighbor searches, along with a large language model (LLM) to identify repair tasks and log failures for future analysis.
