...
// Check each CLI session ID
  ...
  for (const cliId of cliSessionIds) {
    ...
    const partialMatch = sessionEntries.find(entry => entry.id && entry.id.includes(cliId));
    
    console.log(`\n📍 Session ID: ${cliId}`);
    console.log(`  Exact match (session:${cliId}): ${exactMatch ? '✅ Found' : '❌ Not found'}`);
    console.log(`  Partial match: ${partialMatch ? '✅ Found (' + partialMatch.id + ')' : '❌ Not found'}`);
    
    if (exactMatch) {
      try {
        const sessionData = JSON.parse(exactMatch.text);
        console.log(`  Title: ${sessionData.title || 'No title'}`);
        console.log(`  Created: ${sessionData.createdAt || sessionData.time?.created || 'No timestamp'}`);
