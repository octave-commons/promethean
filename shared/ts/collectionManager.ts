...
import { AGENT_NAME } from "../js/env.js";
...
> {
  ...
  >(name: string, textKey: TTextKey, timeStampKey: TTimeKey) {
    ...
    const mongoCollection =
      db.collection<CollectionEntry<TTextKey, TTimeKey>>(collectionName);
  ...
  async addEntry(entry: CollectionEntry<TextKey, TimeKey>) {
    ...
    if (!entry[this.timeStampKey]) {
      entry[this.timeStampKey] = Date.now() as CollectionEntry<
        TextKey,
        TimeKey
      >[TimeKey];
    }
  ...
  async getMostRecent(
    ...
    return (
      ...
      timestamp: new Date(
        (entry as Record<TimeKey, any>)[this.timeStampKey],
      ).getTime(),
      metadata: entry.metadata,
  ...
  async getMostRelevant(
    ...
    return queryResult.documents
      ...
      .map((doc, i) => ({
        ...
        metadata: meta[i],
        timestamp:
          meta[i]?.timeStamp || meta[i]?.[this.timeStampKey] || Date.now(),
      }))
      .filter((doc) => {
