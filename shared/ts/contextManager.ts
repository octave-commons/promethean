...
export const formatMessage = (
  m: CollectionEntry<"text", "timestamp">,
): string =>
...
export class ContextManager {
  ...
  async createCollection(
    ...
    const collectionManager = await CollectionManager.create<string, string>(
      name,
      textKey,
      timeStampKey,
    );
  ...
  async getAllRelatedDocuments(
    ...
    console.log(
      "Getting related documents for querys:",
      querys.length,
      "with limit:",
      limit,
    );
  ...
  async getLatestDocuments(
    ...
    console.log(
      "Getting latest documents from collections:",
  ...
  async compileContext(
    ...
    console.log(
      "Compiling context with texts:",
      texts.length,
      "and limit:",
      limit,
    );
    const latest = await this.getLatestDocuments(recentLimit);
    const query = [...texts, ...latest.map((doc) => doc.text)].slice(
      -queryLimit,
    );
    ...
    return Promise.all([related, latest]).then(([relatedDocs, latestDocs]) => {
      ...
      console.log(
        "You won't believe this but... the results are this long:",
        results.length,
      );
