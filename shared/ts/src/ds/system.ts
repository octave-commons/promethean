...
export function makeStrictSystem(w: World, spec: SystemSpec) {
  ...
  const ctx: SystemCtx = {
    ...
    iter: w.iter.bind(w),
    iterAll: ((...cs: any[]) =>
      iterAll(w, spec.query(w), cs as any)) as unknown as SystemCtx['iterAll'],
...
export function assertDisjointOwnership(
  systems: ReturnType<typeof makeStrictSystem>[],
  specs: SystemSpec[],
) {
  ...
  for (const s of specs)
    for (const c of s.owns ?? []) {
      ...
      if (seen)
        throw new Error(
          `Ownership conflict: '${seen}' and '${specs.find(
            (x) => x.owns?.some((o) => o.id === c.id),
