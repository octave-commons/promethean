...
var { cond, create, extend, mixin } = require('kit/js/util'),
    R = require('ramda'),
    Path = require('path'),
    assert = require('assert'),
    chokidar = require('chokidar'),
    fs = require('fs'),
    { TreeMap } = require('tree-kit');
...
var FileSystem = EventEmitter.define('FileSystem', {
    ...
    insert(rel = this.rel, type = File, root = this.root, sys = this) {
        return sys.find(rel).catch((e) => {
            var absPath = _findAbsolutePath(rel, root);
            return tokenize(rel)
                .reduce(fillSubDir, [Promise.resolve(), root])[0]
                .then((nil) => {
