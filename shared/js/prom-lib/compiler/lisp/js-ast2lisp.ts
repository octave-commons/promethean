...
export function estreeProgramToLisp(
  ...
  if (ast.body.length === 1 && ast.body[0].type === "ExpressionStatement") {
    ...
    if (
      ...
      const fn = ce.callee as
        | EST.FunctionExpression
        | EST.ArrowFunctionExpression;
...
function blockOrSingle(s: EST.Statement, opts: Js2LispOptions): S {
  return s.type === "BlockStatement"
    ? stmt(s, opts)!
    : stmt({ type: "BlockStatement", body: [s] } as any, opts)!;
}
  ...
  switch (n.type) {
    ...
    case "ConditionalExpression": {
      return list([
        sym("if"),
        expr(n.test, opts),
        expr(n.consequent, opts),
        expr(n.alternate, opts),
      ]);
    }
    ...
    case "TemplateLiteral":
      if (n.expressions.length === 0)
        return str(n.quasis[0]?.value.cooked ?? "");
      ...
      for (let i = 0; i < n.quasis.length; i++) {
        ...
        if (i < n.expressions.length)
          parts.push(expr(n.expressions[i] as EST.Expression, opts));
      }
...
function member(
  n: EST.MemberExpression,
  opts: Js2LispOptions,
): { obj: S; key: S; callStyle: "dot" | "bracket" } {
