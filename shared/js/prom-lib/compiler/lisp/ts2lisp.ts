...
async function transpileTS(tsSource: string, opts: TsToLispOptions) {
        ...
        notes.push(
          ...res.diagnostics.map(
            (d: any) =>
              ...
              (d.code ?? "") +
              ": " +
              (d.messageText?.toString?.() ?? String(d.messageText)),
          ),
        );
      }
      return {
  ...
  throw new Error(
    "No TS transpiler available. Install 'typescript' (or 'sucrase' in the browser).",
  );
}

async function dynamicImportTS() {
  try {
    return await import(/* @vite-ignore */ "typescript");
  } catch {
    return null;
  }
}
