...
export class Scheduler {
  ...
  compile(): SchedulePlan {
    ...
    const reads = (s: CompiledSystem) =>
      new Set([
        ...(s.reads ?? []),
        ...(s.readsComponents ?? []).map((c) => `cmp:${c.id}`),
      ]);
    const writes = (s: CompiledSystem) =>
      new Set([
        ...(s.writes ?? []),
        ...(s.writesComponents ?? []).map((c) => `cmp:${c.id}`),
      ]);
    ...
    for (let i = 0; i < compiled.length; i++) {
      for (let j = i + 1; j < compiled.length; j++) {
        ...
        const writeRead =
          intersects(wA, union(rB, wB)) || intersects(wB, union(rA, wA));
        if (writeRead) {
...
function conflictsInBatch(
  ...
  candidate: CompiledSystem,
): boolean {
  const rC = new Set([
    ...(candidate.reads ?? []),
    ...(candidate.readsComponents ?? []).map((c) => `cmp:${c.id}`),
  ]);
  const wC = new Set([
    ...(candidate.writes ?? []),
    ...(candidate.writesComponents ?? []).map((c) => `cmp:${c.id}`),
  ]);
  for (const e of existing) {
    const rE = new Set([
      ...(e.reads ?? []),
      ...(e.readsComponents ?? []).map((c) => `cmp:${c.id}`),
    ]);
    const wE = new Set([
      ...(e.writes ?? []),
      ...(e.writesComponents ?? []).map((c) => `cmp:${c.id}`),
    ]);
    const conflict =
      intersects(wC, union(rE, wE)) || intersects(wE, union(rC, wC));
