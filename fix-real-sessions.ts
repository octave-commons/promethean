...
/**
 * Fix Agent Task Detection Script for Real Sessions Only
 *
 * This script updates only actual session entries (not events or messages) in the dual store
 * to set the isAgentTask property based on session title patterns.
 */
...
async function main() {
  console.log('🔧 Starting Agent Task Detection Fix for Real Sessions Only...\n');
  ...
  try {
    ...
    console.log('🔍 Finding real sessions (not events or messages)...\n');

    // Get sessions from dual store
    const storedSessions = await sessionStore.getMostRecent(1000);

    // Filter only actual session entries (not events, messages, etc.)
    const realSessions = storedSessions.filter((session) => {
      const id = session.id || '';
      // Only include entries that:
      // 1. Start with "session:" but don't contain multiple colons (events have multiple colons)
      // 2. Don't end with ":messages"
      // 3. Don't contain "event:" in the ID
      return (
        (id.startsWith('session:') && !id.includes(':messages') && id.split(':').length <= 3) ||
        // Also include sessions without the session: prefix (legacy format)
        (!id.includes(':') && !id.includes('event') && !id.includes('messages'))
      );
    });

    console.log(
      `📊 Found ${realSessions.length} real sessions out of ${storedSessions.length} total entries\n`,
    );
    ...
    console.log(`\n🎉 Summary:`);
    console.log(`   Total entries checked: ${storedSessions.length}`);
