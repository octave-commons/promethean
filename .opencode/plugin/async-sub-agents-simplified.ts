...
// Tool schema helpers
const createTool = <TArgs extends Record<string, any>, TReturn>(
  description: string,
  args: TArgs,
...
export const MyPlugin: Plugin = async ({ client }) => {
  ...
  return {
    tool: {
      search_sessions: createTool(
        'Search past sessions by semantic embedding',
        {
      ...
      list_sessions: createTool(
        'List all active sessions with pagination and activity status',
        {
      ...
      index_sessions: createTool('Index all past sessions for semantic search', {}, async () => {
        const { data: sessionsList, error } = await client.session.list();
        if (error) return `Failed to fetch sessions: ${error}`;
      ...
      monitor_agents: createTool(
        'Monitor the status of all spawned sub-agent tasks',
        {},
        async () => {
          try {
      ...
      get_agent_status: createTool(
        'Get the status of a specific sub-agent by session ID',
        {
      ...
      cleanup_completed_agents: createTool(
        'Remove completed or failed agent tasks from monitoring',
        {
          olderThan: {
            type: 'number',
        ...
        async ({ olderThan = 60 }: { olderThan?: number }) => {
          const now = Date.now();
