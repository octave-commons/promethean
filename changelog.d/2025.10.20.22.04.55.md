...
- `@promethean/persistence` ContextStore → conversation compilation for LLMs
- `@promethean/agent-context` ContextManager → agent state via event sourcing
...
```ts
import { makeAgentStateManager } from '@promethean/agents/agent-context';
