...
- Renamed `packages/agents/pantheon` to `@promethean/pantheon-cljs`
...
- Updated export maps to only expose stable, intentional APIs:
  - `@promethean/pantheon-core`: Main entry point only
  - `@promethean/pantheon-persistence`: Main entry point only
  - `@promethean/pantheon-llm-*`: Main entry point only
  - `@promethean/pantheon`: Main + intentional sub-exports (adapters, cli, actors, utils)
  - `@promethean/pantheon-mcp`: Main entry point only (already correct)
...
- Corrected package names from `@promethean/pantheon-fp` to `@promethean/pantheon`
