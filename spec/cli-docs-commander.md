...
- Semantic search uses Elasticsearch: configurable URL/index/auth/fields/limit, returns table/JSON with titles/paths/highlights; fallback message when ES config missing. Optional Ollama local embeddings with `--ollama-url`/`DOCS_OLLAMA_URL` (no external deps) and Chroma placeholders. Handle errors cleanly.
