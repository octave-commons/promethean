...
export async function evaluate({
  ...
  const promptsData: unknown = JSON.parse(await fs.readFile(path.resolve(promptsPath), 'utf-8'));
  const prompts = promptsData as { prompts: PromptChunk[] };
  const contextsData: unknown = JSON.parse(await fs.readFile(path.resolve(contextPath), 'utf-8'));
  const enhancedContexts = contextsData as {
    contexts: TaskContext[];
    buildTestResults?: EnhancedTaskContext['buildTestResults'];
  };
  ...
  for (const ctx of enhancedContexts.contexts) {
      ...
      buildTestResult
        ? [
            ...
            buildTestResult.buildResult?.errors.length
              ? `Build Errors: ${buildTestResult.buildResult.errors.slice(0, 200).join('; ')}`
              : '',
            buildTestResult.testResult?.failedTests.length
              ? `Failed Tests: ${buildTestResult.testResult.failedTests.slice(0, 200).join('; ')}`
              : '',
            buildTestResult.lintResult?.errors.length
              ? `Lint Errors: ${buildTestResult.lintResult.errors.slice(0, 200).join('; ')}`
              : '',
            '',
          ]
            .filter(Boolean)
    ...
    const obj: unknown = await ollamaJSON(model, `SYSTEM:\n${sys}\n\nUSER:\n${user}`).catch(() => ({
