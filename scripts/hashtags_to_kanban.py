...
def main() -> None:
    ...
    parser.add_argument(
        "--wikilinks",
        action="store_true",
        help="Emit Obsidian wikilinks instead of markdown links",
    )
    parser.add_argument(
        "--encode-urls",
        action="store_true",
...
def verify_board(board_path: Path, *, require_existing_files: bool = True) -> list[str]:
    ...
    for i, raw in enumerate(text, start=1):
        ...
        # no raw http(s) links in cards
        if BAD_HTTP.search(line):
            errors.append(f"L{i}: External URL found in card: {line.strip()}")

        # enforce wikilinks for linked cards
        if MD_LINK.search(line):
            errors.append(
                f"L{i}: Markdown link used; wikilinks required: {line.strip()}"
            )
