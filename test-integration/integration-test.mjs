...
const mockBoard: Board = {
  columns: [
    { id: 'testing', title: 'Testing', tasks: [], limit: 5 },
    { id: 'review', title: 'Review', tasks: [], limit: 5 },
  ],
  wipLimits: { testing: 5, review: 5 },
};
 ...
 */
async function testHighCoverageTransition() {
  ...
  try {
    ...
    const result = await engine.validateTransition(task, 'review', mockBoard);
 ...
 */
async function testLowCoverageTransition() {
 ...
 */
async function testMissingCoverageReport() {
...
async function testPerformance() {
  ...
  try {
    ...
    console.log(`✅ Performance: ${withinLimit ? 'PASS' : 'FAIL'} (${duration}ms <= ${TEST_CONFIG.timeoutMs}ms)`);
