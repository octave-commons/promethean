...
- [agents](../agents/platforms/codex/agents.md) – an overview of the framework, how agents like Duck fit into it and the allowed operations for automated tools.
...
- [site README](../agents/readme.md) – instructions for building and serving the portfolio site.
...
- [Kanban board](../agents/instructions/kanban.md) – the Kanban board used to track work. When opened in Obsidian with the Kanban plugin, it renders as an interactive board. See [board_usage|board usage] for how to interact with it.
...
- [vault‑config README](../agents/readme.md) – guidance for using this repo as an Obsidian vault. It covers settings for GitHub‑compatible Markdown, recommended plugins and how to install the baseline configuration.
...
- [services](../agents/readme.md) – docs for individual subsystems under `services/`.
- [agents](../agents/readme.md) – behaviour notes and prompts for each agent.
- [shared](../agents/readme.md) – reusable libraries and clients shared across services.
...
- **Use relative Markdown links.** To ensure links work on GitHub, write them in standard Markdown with a relative path: `[kanban](kanban.md)`. Obsidian’s wikilink shorthand can be handy locally, but the repository’s recommended settings convert it to a proper link on save or via pre‑commit.
...
1. Read [agents](../agents/platforms/codex/agents.md) and [file-structure|file‑structure] to understand the architecture, repository layout and language choices.

2. Browse the [Kanban board](../agents/instructions/kanban.md) to see what work is planned or in progress.
