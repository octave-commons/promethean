...
* **If restricted to current Ready only:** `[Update Makefile for agent commands](Update%20makefile%20to%20have%20commands%20specific%20for%20agents.md)` (quick win that improves daily flow) **or** `[Mirror shared utils with language-specific doc folders.md]` (reduces duplication before deeper queue work).
