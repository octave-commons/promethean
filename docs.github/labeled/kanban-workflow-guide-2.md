...
## 2. Check the [kanban](../agents/instructions/kanban.md) Board
...
- If **still blocked**:
  - Identify if another task would unblock it.
    - If yes, link the tasks using `[wikilinks](wikilinks.md)` and add commentary to both.
...
- If multiple loosely related tasks exist:
  ...
  - Mention related ones with `[wikilinks](wikilinks.md)`.
