...
(setv GPU_SERVICES #{"stt" "tts"})   ; only these services actually need GPU wheels
...
(define-service-list SERVICES_PY "services/py" (and (not (in  "templates" path)) (not (in "shared" path)) (not (in "__pycache__" path))))
  ...
  (let [infile (reqs-file-for d)
       ...
       (if (= infile "requirements.gpu.in")
           (sh (.format
               ...
               :cwd d :shell True)
           (sh (.format
                "UV_VENV_IN_PROJECT=1 uv pip compile --index https://download.pytorch.org/whl/cpu {} -o {} --index-strategy unsafe-best-match"
                infile lockf)
               :cwd d :shell True))))

(defn uv-sync [d]
  (let [lockf (lockfile-for d)]
       (if (= lockf "requirements.gpu.lock")
           (sh (.format "UV_VENV_IN_PROJECT=1 uv pip sync {} --index {} --index-strategy unsafe-best-match" lockf TORCH_INDEX) :cwd d :shell True)
           (sh (.format "UV_VENV_IN_PROJECT=1 uv pip sync {} --index https://download.pytorch.org/whl/cpu --index-strategy unsafe-best-match" lockf) :cwd d :shell True))))
