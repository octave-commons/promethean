...
await root.open();
let qCount = 0, hitsTotal = 0, ge01 = 0, max = 0;
...
let chunkDocs = 0, chunkCount = 0;
for await (const [doc, arr] of chunks.iterator()) { chunkDocs++; chunkCount += (arr||[]).length; }
let docCount = 0;
for await (const _ of docs.iterator()) { docCount++; }
console.log(JSON.stringify({ qKeys: qCount, hitsTotal, ge01, max, chunkDocs, chunkCount, docCount }, null, 2));
await root.close();
