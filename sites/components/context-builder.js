  ...
  return {
    async view(path, opts = {}) {
      ...
      const res = await fetch(
        `${baseUrl.replace(/\/$/, "")}/view/${encodeURIComponent(
          path,
        )}?${q.toString()}`,
      );
...
export function chromaHttpProvider({ baseUrl, collection }) {
  if (!baseUrl || !collection)
    console.warn("chromaHttpProvider: missing baseUrl/collection");
  return {
    async query({ embedding, topK = 8, where = {} }) {
      ...
      const res = await fetch(
        `${baseUrl.replace(/\/$/, "")}/collections/${encodeURIComponent(
          collection,
        )}/query`,
        {
          method: "POST",
...
export function simpleSearchProvider({ url }) {
  return {
    async search(q, { topS = 3 } = {}) {
      const res = await fetch(
        `${url}?${new URLSearchParams({ q, k: String(topS) })}`,
      );
...
export const ContextBuilder = {
  ...
  async build(params) {
    ...
    const { blocks: packed, used } = packBlocks(
      blocks,
      ctxBudgetTokens,
      priority,
    );
    ...
    const selectedIds = new Set(
      packed.flatMap(([_, arr]) => arr.map((c) => c.id)),
    );
  ...
  assemblePrompt({ chips, systemPrompt = "", M = 6 }) {
    ...
    function lines(label) {
      const arr = chips.filter(
        (c) => c.selected && label === kindToLabel(c.kind),
      );
      if (!arr.length) return "";
      const body = arr
        .map((c) => `[#${c.id}] ${c.payload?.text || ""}`)
        .join("\n\n");
      return `${label}\n${body}`;
    }
