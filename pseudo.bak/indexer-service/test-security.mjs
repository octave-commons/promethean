...
// Import the functions directly from the TypeScript file
import fs from 'fs';
import path from 'path';

// Read and evaluate the security functions
const indexerCode = fs.readFileSync('./src/routes/indexer.ts', 'utf8');

// Extract the isSafeRelPath function using eval (for testing purposes only)
const securityFunctions = `
${indexerCode}

// Export for testing
globalThis.isSafeRelPath = isSafeRelPath;
globalThis.validatePathArray = validatePathArray;
`;

eval(securityFunctions);
...
maliciousPayloads.forEach((payload, index) => {
  try {
    const result = globalThis.isSafeRelPath(payload);
    if (!result) {
      console.log(
...
validPaths.forEach((path, index) => {
  try {
    const result = globalThis.isSafeRelPath(path);
    if (result) {
