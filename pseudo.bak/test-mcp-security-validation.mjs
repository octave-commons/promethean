...
// Mock the MCP security functions for testing
...
function isAllowedFileExtension(filePath) {
  ...
  const dangerousFiles = [
    'rootkit', 'backdoor', 'payload', 'exploit', 'malware',
...
describe('P0 MCP Security Validation', () => {
  ...
  describe('Path Traversal Protection', () => {
    test('should block basic path traversal attempts', () => {
      ...
      maliciousPaths.forEach(path => {
        ...
        assert.strictEqual(result, false, `Should block path traversal: ${path}`);
      });
    });
    ...
    test('should block Unicode homograph attacks', () => {
      ...
      unicodeAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block Unicode attack: ${path}`);
      });
    });
    ...
    test('should block encoded path traversal', () => {
      ...
      encodedAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block encoded attack: ${path}`);
      });
    });
    ...
    test('should block absolute path attempts', () => {
      ...
      absolutePaths.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block absolute path: ${path}`);
      });
    });
  });
  ...
  describe('Dangerous Character Filtering', () => {
    test('should block paths with dangerous characters', () => {
      ...
      dangerousPaths.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block dangerous chars: ${path}`);
      });
    });
  });
  ...
  describe('Windows Path Security', () => {
    test('should block Windows-specific attacks', () => {
      ...
      windowsAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block Windows attack: ${path}`);
      });
    });
  });
  ...
  describe('Unix Path Security', () => {
    test('should block Unix dangerous paths', () => {
      ...
      unixAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block Unix attack: ${path}`);
      });
    });
  });
  ...
  describe('Glob Pattern Attack Protection', () => {
    test('should block glob pattern attacks', () => {
      ...
      globAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block glob attack: ${path}`);
      });
    });
  });
  ...
  describe('Input Validation Bypass Protection', () => {
    test('should prevent array input bypasses', () => {
      ...
      arrayInputs.forEach(input => {
        ...
        assert.strictEqual(result.valid, false, `Should reject array input: ${JSON.stringify(input)}`);
      });
    });
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    test('should handle path normalization edge cases', () => {
      ...
      edgeCases.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block normalization bypass: ${path}`);
      });
    });
    ...
    test('should prevent null byte injection', () => {
      ...
      nullByteAttacks.forEach(path => {
        const result = isSafeRelPath(path);
        assert.strictEqual(result, false, `Should block null byte injection: ${path}`);
      });
    });
  });
});
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const testRunner = async () => {
    ...
    console.log('   node test-mcp-security-validation.mjs');
