...
test('deployment: docker build verification', async (t) => {
  ...
  try {
    const { stdout: buildOutput } = await execa(
      'docker',
      ['build', '-t', 'omni-service:test', '--target', 'production', './packages/omni-service'],
      {
...
test('deployment: docker compose validation', async (t) => {
  try {
    const { stdout: validationOutput } = await execa(
      'docker-compose',
      ['config', '--quiet', './packages/omni-service/docker-compose.yml'],
      {
        cwd: '/home/err/devel/promethean',
...
test('deployment: docker compose startup verification', async (t) => {
  ...
  try {
    ...
    // Start services
    const { stdout: startOutput } = await execa(
      'docker-compose',
      ['-p', 'omni-test', 'up', '-d', '--build', 'omni-service', 'redis'],
      {
        cwd: '/home/err/devel/promethean/packages/omni-service',
        timeout: 600000, // 10 minutes
      },
    ...
    const { stdout: psOutput } = await execa('docker-compose', ['-p', 'omni-test', 'ps'], {
      cwd: '/home/err/devel/promethean/packages/omni-service',
      timeout: 300000,
    });
    ...
    const { stdout: healthOutput } = await execa('curl', ['-f', 'http://localhost:3000/health'], {
      timeout: 30000,
    });
    ...
    const { stdout: adapterOutput } = await execa(
      'curl',
      ['-f', 'http://localhost:3000/adapters/status'],
      {
        timeout: 30000,
      },
    );
...
test('deployment: nginx configuration validation', async (t) => {
  try {
    ...
    const { stdout: nginxTestOutput } = await execa(
      'nginx',
      [
        '-t',
        '-c',
...
test('deployment: resource limits and optimization', async (t) => {
  ...
  try {
    // Get image size
    const { stdout: sizeOutput } = await execa(
      'docker',
      ['images', 'omni-service:latest', '--format', '{{.Size}}'],
      {
        timeout: 30000,
      },
    );
    ...
    t.true(
      imageSize < 200 * 1024 * 1024,
      `Image size should be less than 200MB, got ${imageSize} bytes`,
    );

    // Test image layers
    const { stdout: layersOutput } = await execa(
      'docker',
      ['history', 'omni-service:latest', '--format', '{{.ID}}'],
      {
        timeout: 30000,
      },
    );
    ...
    try {
      const { stdout: scanOutput } = await execa(
        'trivy',
        ['image', '--severity', 'HIGH,CRITICAL', '--exit-code', '1', 'omni-service:latest'],
        {
          timeout: 120000,
        },
      );
...
test('deployment: configuration generation verification', async (t) => {
  try {
    ...
    const { stdout: configOutput } = await execa(
      'node',
      [
        '--loader',
        'ts-node/esm',
    ...
    try {
      ...
      await execa(
        ...
        [
          ...
          containerName,
          '-p',
          '3001', // Use different port to avoid conflicts
          '-e',
          'JWT_SECRET=this-is-a-test-jwt-secret-that-is-at-least-32-characters-long',
          '-e',
          'NODE_ENV=test',
      ...
      // Test health endpoint
      const { stdout: healthOutput } = await execa('curl', ['-f', 'http://localhost:3001/health'], {
        timeout: 30000,
      });
      ...
      const { stdout: rootOutput } = await execa('curl', ['-f', 'http://localhost:3001/'], {
        timeout: 30000,
      });
      ...
      t.is(
        ...
        'Root endpoint should return correct service name',
      );

      // Test that we can make authenticated requests
      const { stdout: authOutput } = await execa(
        'curl',
        [
          '-f',
          '-X',
