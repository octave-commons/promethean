...
export class PermissionUtils {
  ...
  static matchesPermission(permission: string, resource: string, action: string): boolean {
    ...
    // Pattern matching
    if (permResource && permResource.includes('*')) {
      const resourcePattern = permResource.replace(/\*/g, '.*');
      const resourceRegex = new RegExp(`^${resourcePattern}$`);

      if (resourceRegex.test(resource) && permActions?.split(',').includes(action)) {
        return true;
      }
    }
    ...
    if (permResource?.endsWith('*') && resource.startsWith(permResource.slice(0, -1))) {
      return (
        permActions?.split(',').includes(action) || permActions?.split(',').includes('*') || false
      );
    }
