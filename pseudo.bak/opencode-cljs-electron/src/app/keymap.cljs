  ...
  {:leader {:bindings {"SPC" {:name "Leader"
                              :bindings {"f" {:name "File"
                                      ...
                                      "e" {:name "Editor"
                                       ...
                                       "o" {:name "Opencode"
                                              :bindings {"c" {:name "Connect to Opencode"
                                                             :handler #(app.opencode/connect-to-opencode "http://localhost:3000")}
                                                        "d" {:name "Disconnect from Opencode"
                                                             :handler #'app.opencode/disconnect-from-opencode}
                                                        "t" {:name "List Tools"
                                                             :handler #(-> (app.opencode/list-available-tools)
                                                                         (.then (fn [result]
                                                                                  (when-not (:error result)
                                                                                    (js/alert (str "Available tools:\n" 
                                                                                                  (clojure.string/join "\n" (map :name result))))))))}
                                                        "a" {:name "Spawn Agent"
                                                             :handler #(let [agent-type (js/prompt "Agent type:")
                                                                            prompt (js/prompt "Agent prompt:")]
                                                                        (when (and agent-type prompt)
                                                                          (-> (app.opencode/spawn-agent agent-type prompt)
                                                                              (.then (fn [result]
                                                                                       (when-not (:error result)
                                                                                         (app.opencode/create-agent-chat-buffer (:agent-id result) agent-type)))))))}
                                                        "e" {:name "Execute Tool"
                                                             :handler #(let [tool-name (js/prompt "Tool name:")
                                                                            parameters (js/prompt "Parameters (JSON):")]
                                                                        (when (and tool-name parameters)
                                                                          (try
                                                                            (let [parsed-params (js/JSON.parse parameters)]
                                                                              (-> (app.opencode/execute-tool tool-name (js->clj parsed-params :keywordize-keys true))
                                                                                  (.then (fn [result]
                                                                                           (app.opencode/create-tool-execution-buffer tool-name 
                                                                                                                                (js->clj parsed-params :keywordize-keys true)
                                                                                                                                (:execution-id result))))))
                                                                            (catch js/Error e
                                                                              (js/alert (str "Invalid JSON: " (.-message e)))))))}
                                                        "s" {:name "Show Opencode Status"
                                                             :handler #(let [state (app.opencode/get-opencode-state)]
                                                                         (js/alert (str "Opencode Status:\n"
                                                                                       "Connected: " (:connected? state) "\n"
                                                                                       "Session: " (:session-id state) "\n"
                                                                                       "Tools: " (count (:available-tools state)) "\n"
                                                                                       "Agents: " (count (:active-agents state)))))}}}
"h" {:name "Help"
                                              :bindings {"b" {:name "Describe bindings"
                                                             :handler #(show-which-key ["SPC" "h" "b"])}}}}}}}})

;; Evil mode keybindings
(def evil-keybindings
