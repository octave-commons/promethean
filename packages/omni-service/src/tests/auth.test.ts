...
test('rbac-manager: role and permission management', async (t) => {
  ...
  t.deepEqual(adminRole?.permissions, [
...
test('rbac-manager: permission utilities', async (t) => {
  ...
  const canReadOrWrite = rbac.hasAnyPermission(userId, adminRoles, 'test:resource', [
    'read',
    'write',
  ]);
  t.true(canReadOrWrite.granted, 'Admin should have at least one of read/write permissions');

  const canReadAndWrite = rbac.hasAllPermissions(userId, adminRoles, 'test:resource', [
    'read',
    'write',
  ]);
  t.true(canReadAndWrite.granted, 'Admin should have both read and write permissions');

  const canReadWriteAndDelete = rbac.hasAllPermissions(userId, ['user'], 'test:resource', [
    'read',
    'write',
    'delete',
  ]);
...
test('config: environment configuration loading', async (t) => {
  const { config, getAuthConfig, getAdapterConfig } = await import('../config.js');
