...
test("integration: GraphQL end-to-end workflow", async (t) => {
  ...
  t.is(createData.data.createUser.name, "GraphQL Test User", "Created user should have correct name");
  ...
  t.is(updateData.data.updateUser.id, createData.data.createUser.id, "Updated user ID should match");
...
test("integration: MCP end-to-end workflow", async (t) => {
  ...
  t.is(echoData.result.result, "Hello from MCP integration test!", "Echo tool should return correct text");
...
test("integration: concurrent request handling", async (t) => {
  ...
  mcpResults.forEach((result, index) => {
    ...
    const data = JSON.parse(result.payload);
    t.is(data.result.result, `Concurrent test ${index}`, `MCP Request ${index} should return correct text`);
  });
...
test("integration: error handling and resilience", async (t) => {
  ...
  const missingParamsResponse = await app.inject({
    ...
    url: '/api/v1/users',
    headers: {
      'Authorization': `Bearer ${createAuthManager().generateTokens({
        id: 'test-user',
        roles: ['user']
      }).accessToken}`,
      'Content-Type': 'application/json',
