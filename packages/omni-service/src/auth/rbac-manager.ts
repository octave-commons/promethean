import type { Role, Permission, PermissionCheck, UserContext } from "./types.js";
...
export class RBACManager {
  ...
  constructor(cacheTTL: number = 300) { // 5 minutes default
  ...
  getUserPermissions(userId: string, userRoles: string[]): Set<string> {
    ...
    const permissions = new Set(
      PermissionUtils.flattenPermissions(
    ...
    for (const role of this.roles.values()) {
      for (const permission of role.permissions) {
        if (
          ...
          permissions.push(PermissionUtils.formatPermission(permission.resource, permission.actions));
        }
      }
    }
   ...
   */
  hasRole(userId: string, userRoles: string[], roleName: string): boolean {
   ...
   */
  hasAnyRole(userId: string, userRoles: string[], roleNames: string[]): boolean {
    const hierarchy = this.getRoleHierarchy(userRoles);
