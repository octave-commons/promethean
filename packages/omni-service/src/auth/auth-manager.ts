...
export class AuthManager {
  ...
  private extractToken(request: FastifyRequest): string | null {
    ...
    if (this.config.apikey.enabled) {
      ...
      if (this.config.apikey.queryParam) {
        const queryApiKey = request.query[this.config.apikey.queryParam] as string;
    ...
    if (this.config.session.enabled && request.cookies) {
      const cookieToken = request.cookies[this.config.session.cookieName];
  ...
  createPermissionChecker(user: UserContext) {
    return {
      ...
      canRead: (resource: string) =>
        this.rbacManager.canRead(user.id, user.roles, resource),
      
      canWrite: (resource: string) =>
        this.rbacManager.canWrite(user.id, user.roles, resource),
      
      canDelete: (resource: string) =>
        this.rbacManager.canDelete(user.id, user.roles, resource),
      
      hasAdminAccess: (resource: string) =>
        this.rbacManager.hasAdminAccess(user.id, user.roles, resource),
      
      hasRole: (roleName: string) =>
        this.rbacManager.hasRole(user.id, user.roles, roleName),
      
      hasAnyRole: (roleNames: string[]) =>
        this.rbacManager.hasAnyRole(user.id, user.roles, roleNames),
      
      getRoleInfo: () =>
        this.rbacManager.getUserRoleInfo(user.id, user.roles),
    };
  }
  ...
  setAuthCookie(reply: FastifyReply, token: string): void {
    if (this.config.session.enabled) {
      reply.cookie(this.config.session.cookieName, token, {
  ...
  clearAuthCookie(reply: FastifyReply): void {
    if (this.config.session.enabled) {
      reply.clearCookie(this.config.session.cookieName, {
        path: "/",
      });
    }
  }
