...
class MockDataStore {
  ...
  constructor() {
    ...
    this.data.set("users", [
      { id: "1", name: "Alice", email: "alice@example.com", role: "user", createdAt: "2025-06-20T00:00:00Z" },
      { id: "2", name: "Bob", email: "bob@example.com", role: "admin", createdAt: "2025-06-20T01:00:00Z" },
      { id: "3", name: "Charlie", email: "charlie@example.com", role: "user", createdAt: "2025-06-20T02:00:00Z" },
    ]);
    
    this.data.set("posts", [
      { id: "1", title: "First Post", content: "This is the first post", authorId: "1", status: "published", createdAt: "2025-06-20T12:00:00Z" },
      { id: "2", title: "Second Post", content: "This is the second post", authorId: "2", status: "draft", createdAt: "2025-06-20T13:00:00Z" },
      { id: "3", title: "Third Post", content: "This is the third post", authorId: "1", status: "published", createdAt: "2025-06-20T14:00:00Z" },
    ]);
  }
...
export function mountRestAdapter(
      ...
      try {
        ...
        if (routeOptions.requireAuth !== false) {
          ...
          if (routeOptions.roles && routeOptions.roles.length > 0) {
            ...
            if (authManager) {
              const hasRequiredRole = authManager.getRBACManager().hasAnyRole(
                request.user.id,
                request.user.roles,
                routeOptions.roles
              );
          ...
          if (routeOptions.permissions && routeOptions.permissions.length > 0) {
            ...
            if (authManager) {
              for (const perm of routeOptions.permissions) {
                const hasPermission = authManager.getRBACManager().hasAnyPermission(
                  request.user.id,
                  request.user.roles,
                  perm.resource,
                  perm.actions
        ...
        app.log.error(`REST API Error (${request.id}):`, error);
  ...
  app.get(`${fullPrefix}/health`, createHandler(async (request, reply) => {
    const healthStatus = {
      status: "ok",
      adapter: "rest",
  ...
  app.get(`${fullPrefix}/:resource`, createHandler(async (request, reply) => {
  ...
  app.get(`${fullPrefix}/:resource/:id`, createHandler(async (request, reply) => {
  ...
  app.post(`${fullPrefix}/:resource`, createHandler(async (request, reply) => {
    const { resource } = request.params as { resource: string };
  ...
  app.put(`${fullPrefix}/:resource/:id`, createHandler(async (request, reply) => {
    const { resource, id } = request.params as { resource: string; id: string };
    const body = request.body as any;
  ...
  app.delete(`${fullPrefix}/:resource/:id`, createHandler(async (request, reply) => {
    const { resource, id } = request.params as { resource: string; id: string };
  ...
  app.get(`${fullPrefix}/users/me`, createHandler(async (request, reply) => {
  ...
  // Schema information
  app.get(`${fullPrefix}/schema`, createHandler(async (request, reply) => {
    const schemas = {
      users: {
        id: "string",
        name: "string",
  ...
  if (options.enableDocs) {
    app.get(`${fullPrefix}/docs`, createHandler(async (request, reply) => {
      const docs = {
