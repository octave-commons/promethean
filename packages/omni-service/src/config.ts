...
const defaultConfig: EnhancedOmniServiceConfig = {
  ...
  jwt: {
    secret: 'default-jwt-secret-change-in-production-32chars',
    algorithm: 'HS256',
    expiresIn: '1h',
    issuer: 'omni-service',
    audience: 'omni-service',
    refreshExpiresIn: '7d',
  },
  ...
  const envConfig: Partial<EnhancedOmniServiceConfig> = {
    ...
    jwt: process.env.JWT_SECRET
      ? {
          ...
          issuer: process.env.JWT_ISSUER || 'omni-service',
          audience: process.env.JWT_AUDIENCE || 'omni-service',
    ...
    // API key configuration
    apikey:
      process.env.APIKEY_ENABLED === 'false'
        ? {
            enabled: false,
    ...
    session:
      process.env.SESSION_ENABLED === 'false'
        ? {
            enabled: false,
            cookieName: process.env.SESSION_COOKIE_NAME || 'omni-token',
            cookieOptions: {
              httpOnly: process.env.SESSION_COOKIE_HTTPONLY !== 'false',
              secure: process.env.SESSION_COOKIE_SECURE !== 'false',
              sameSite:
                (process.env.SESSION_COOKIE_SAME_SITE as 'strict' | 'lax' | 'none') || 'strict',
  ...
  static validateSession(config: EnhancedOmniServiceConfig['session']): {
    ...
    if (config.enabled && config.cookieOptions) {
      ...
      if (!validSameSite.includes(sameSite)) {
        errors.push(
          `Invalid sameSite value: ${sameSite}. Must be one of: ${validSameSite.join(', ')}`,
        );
      }
