...
import type { Task } from '@promethean-os/kanban-sdk';
import type {
  TaskAnalysisRequest,
  TaskRewriteRequest,
...
import { runPantheonComputation } from '@promethean-os/kanban-transition-rules/pantheon/runtime';
...
export class TaskAIManager {
  ...
  private validateAnalysisResult(analysis: any): any {
    ...
    if (analysis.estimatedEffort && typeof analysis.estimatedEffort === 'object') {
      result.estimatedEffort = {
        hours:
          typeof analysis.estimatedEffort.hours === 'number' ? analysis.estimatedEffort.hours : 0,
        confidence:
          typeof analysis.estimatedEffort.confidence === 'number'
            ? Math.min(100, Math.max(0, analysis.estimatedEffort.confidence))
            : 50,
        breakdown: Array.isArray(analysis.estimatedEffort.breakdown)
          ? analysis.estimatedEffort.breakdown
          : [],
      };
    }
  ...
  private validateBreakdownResult(breakdown: any, includeEstimates: boolean): any[] {
    ...
    return breakdown.subtasks.map((subtask: any) => ({
      ...
      estimatedHours:
        includeEstimates && typeof subtask.estimatedHours === 'number'
  ...
  private analyzeChanges(
    ...
    originalLines.forEach((origLine) => {
      const rewrittenMatch = rewrittenLines.find(
        (rewLine) =>
    ...
    return {
      summary,
      highlights: [
        `Content length changed from ${original.length} to ${rewritten.length} characters`,
      ],
      additions,
      modifications,
      removals,
