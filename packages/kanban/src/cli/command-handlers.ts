...
const toPosixRelative = (baseDir: string, absolutePath: string): string => {
  const relative = path.relative(baseDir, absolutePath);
  const normalized = relative.length === 0 ? path.basename(absolutePath) : relative;
  return normalized.split(path.sep).join('/');
};

type KanbanInitTemplate = {
  readonly boardFile: string;
  readonly tasksDir: string;
...
const resolveWithinBase = (baseDir: string, candidate: string): string =>
  path.isAbsolute(candidate) ? candidate : path.resolve(baseDir, candidate);

const handleInit: CommandHandler = async (args, context) => {
  const baseDir = args[0] ? path.resolve(process.cwd(), args[0]!) : process.cwd();
  const templatePath = path.resolve(__dirname, '../../promethean.kanban.json');
  const templateRaw = await readFile(templatePath, 'utf8');
  const template = JSON.parse(templateRaw) as KanbanInitTemplate;

  const providedBoard = context.boardFile?.trim();
  const providedTasks = context.tasksDir?.trim();

  const boardAbsolute = resolveWithinBase(
    baseDir,
    providedBoard && providedBoard.length > 0 ? providedBoard : template.boardFile,
  );
  const tasksAbsolute = resolveWithinBase(
    baseDir,
    providedTasks && providedTasks.length > 0 ? providedTasks : template.tasksDir,
  );
  const cachePathAbsolute =
    typeof template.cachePath === 'string'
      ? resolveWithinBase(baseDir, template.cachePath)
      : undefined;

  const configPath = path.resolve(baseDir, 'promethean.kanban.json');

  const [configAlreadyExists, boardAlreadyExists, tasksDirExists] = await Promise.all([
    pathExists(configPath),
    pathExists(boardAbsolute),
    pathExists(tasksAbsolute),
  ]);

  await mkdir(path.dirname(boardAbsolute), { recursive: true });
  await mkdir(tasksAbsolute, { recursive: true });
  if (cachePathAbsolute) {
    await mkdir(cachePathAbsolute, { recursive: true });
  }

  const updatedConfig: KanbanInitTemplate = {
    ...template,
    boardFile: toPosixRelative(baseDir, boardAbsolute),
    tasksDir: toPosixRelative(baseDir, tasksAbsolute),
    cachePath:
      typeof template.cachePath === 'string' && cachePathAbsolute
        ? toPosixRelative(baseDir, cachePathAbsolute)
        : template.cachePath,
  };
  ...
  if (!boardAlreadyExists) {
    const placeholderBoard = [
      '# Kanban Board',
      '',
      '> Generated by `kanban init`',
      '',
      'Run `kanban regenerate` after adding tasks to populate this board automatically.',
      '',
    ].join('\n');
    await writeFile(boardAbsolute, placeholderBoard, 'utf8');
  }

  return {
    configPath,
    boardFile: boardAbsolute,
    tasksDir: tasksAbsolute,
    createdConfig: !configAlreadyExists,
    createdBoard: !boardAlreadyExists,
    createdTasksDir: !tasksDirExists,
  };
};
