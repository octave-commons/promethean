...
const defaultEnvironments = {
  development: {
    ...
    health: {
      ...
      alertChannels: [
        ...
        { type: 'file' as const, enabled: true, config: { path: 'logs/kanban-health-dev.log' } },
      ],
      rules: [],
      scheduling: {
        metricsCollection: '*/2 * * * *',
  ...
  production: {
    ...
    health: {
      ...
      alertChannels: [
        { type: 'console' as const, enabled: false },
        { type: 'file' as const, enabled: true, config: { path: 'logs/kanban-health.log' } },
        {
          type: 'webhook' as const,
          enabled: false,
          config: { url: process.env.HEALTH_WEBHOOK_URL },
        },
        { type: 'mcp' as const, enabled: true, config: { bridge: 'promethean-mcp' } },
      ],
      rules: [],
      scheduling: {
        metricsCollection: '*/5 * * * *',
