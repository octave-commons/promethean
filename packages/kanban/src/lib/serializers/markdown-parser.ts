...
const parseCardLine = (
  body: string,
  isDone: boolean,
): { readonly card: Card; readonly hasInlineId: boolean } => {
...
const appendColumn = (
  ...
  column: ColumnState,
): readonly ColumnState[] => [...columns, column];
...
const parseColumns = (markdown: string): readonly ColumnState[] => {
  ...
  const finalColumns = processed.current
    ? appendColumn(processed.columns, processed.current)
    : processed.columns;
