...
test('should create backup of file', async t => {
  ...
  t.truthy(backupPath);
  t.true(await fs.access(backupPath).then(() => true).catch(() => false));
  ...
  t.true(await fs.access(metadataPath).then(() => true).catch(() => false));
...
test('should list backups', async t => {
  ...
  const file1Metadata1 = JSON.parse(await fs.readFile(file1Backups[0].replace('.md', '.meta.json'), 'utf8'));
  const file1Metadata2 = JSON.parse(await fs.readFile(file1Backups[1].replace('.md', '.meta.json'), 'utf8'));
...
test('should get backup statistics', async t => {
  ...
  const file2 = await createTestFile(testDir, 'stats-test-2.md', 'Larger content with more text to increase file size for testing purposes.');
...
test('should verify backup integrity', async t => {
  ...
  await t.throwsAsync(
    () => backupManager.restoreBackup(backupPath, testFile),
    { message: /integrity/ }
  );
});
...
test('should handle missing files gracefully', async t => {
  ...
  await t.throwsAsync(
    () => backupManager.restoreBackup(nonExistentBackup, nonExistentFile),
