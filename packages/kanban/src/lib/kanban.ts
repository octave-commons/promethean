...
const parseLimit = (header: string): number | null => {
  ...
  const m = header.match(/\((?:wip|limit)\s*[:=]\s*(\d+)\)/i) ||
...
const parseColumnsFromMarkdown = (markdown: string): ColumnData[] => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (taskMatch) {
      ...
      const labels = Array.from(titlePart.matchAll(/#([\w-]+)/g)).map(m => m[1]);
...
const taskFromFM = (fm: FM, body: string): Task | null => {
  ...
  const t: Task = {
    ...
    labels: Array.isArray(fm.labels) ? fm.labels.map(String) :
      (typeof fm.labels === 'string' ? fm.labels.split(/[\s,]+/).filter(Boolean) : []),
...
export const loadBoard = async (boardPath: string, tasksDir: string): Promise<Board> => {
  ...
  const cols: ColumnData[] = Array.from(byStatus.entries()).map(([name, ts]) => (
    { name, count: ts.length, limit: null, tasks: ts }
  ));
  return { columns: cols };
};
...
export const countTasks = (board: Board, column?: string): number => {
  ...
  const c = board.columns.find(c => c.name.toLowerCase() === column.toLowerCase());
  return c ? c.count : 0;
};
...
const serializeBoard = (board: Board): string => {
  ...
  for (const col of board.columns) {
    ...
    for (const t of col.tasks) {
      ...
      lines.push(`- [${done}] ${t.title} ${labels}${prio} (uuid:${t.uuid})`.trim());
    }
    lines.push('');
  }
  return lines.join('\n');
...
export const updateStatus = async (board: Board, uuid: string, newStatus: string, boardPath: string): Promise<Task | undefined> => {
  ...
  let target = board.columns.find(c => c.name.toLowerCase() === newStatus.toLowerCase());
  if (!target) {
...
export const moveTask = async (board: Board, uuid: string, delta: number, boardPath: string): Promise<{ uuid: string; column: string; rank: number } | undefined> => {
  for (const col of board.columns) {
    const idx = col.tasks.findIndex(t => t.uuid === uuid);
    if (idx >= 0) {
...
export const pullFromTasks = async (board: Board, tasksDir: string, boardPath: string): Promise<{ added: number; moved: number }> => {
  const tasks = await readTasksFolder(tasksDir);
  let added = 0, moved = 0;
  ...
  for (const t of tasks) {
    ...
    if (!loc) {
      let col = board.columns.find(c => c.name.toLowerCase() === String(t.status).toLowerCase());
      if (!col) {
      ...
      if (loc.col.name.toLowerCase() !== String(t.status).toLowerCase()) {
        ...
        loc.col.count -= 1;
        // add to new
        let dest = board.columns.find(c => c.name.toLowerCase() === String(t.status).toLowerCase());
        if (!dest) {
...
export const pushToTasks = async (board: Board, tasksDir: string): Promise<{ added: number; moved: number }> => {
  let added = 0, moved = 0;
  ...
  for (const col of board.columns) {
    for (const t of col.tasks) {
      ...
      if (!exists) added++; else moved++;
    }
  }
  return { added, moved };
};

export const syncBoardAndTasks = async (board: Board, tasksDir: string, boardPath: string): Promise<{ board: { added: number; moved: number }, tasks: { added: number; moved: number }, conflicting: string[] }> => {
  const taskFiles = await readTasksFolder(tasksDir);
  const boardById = new Map<string, Task>();
  for (const col of board.columns) for (const t of col.tasks) boardById.set(t.uuid, { ...t, status: col.name });
  ...
  for (const [id, t] of tasksById) {
    ...
    if (!b) continue;
    if ((b.title ?? '') !== (t.title ?? '') || String(b.status) !== String(t.status)) conflicting.push(id);
  }
...
export const regenerateBoard = async (tasksDir: string, boardPath: string): Promise<{ totalTasks: number }> => {
  const tasks = await readTasksFolder(tasksDir);
  const byStatus = new Map<string, Task[]>();
  for (const t of tasks) {
    const k = String(t.status || 'Todo');
  ...
  const columns: ColumnData[] = Array.from(byStatus.entries()).map(([name, ts]) => (
    { name, count: ts.length, limit: null, tasks: ts }
  ));
...
export const indexForSearch = async (_tasksDir: string): Promise<{ started: boolean }> =>
  ({ started: true }); // noop stub – your indexer can hook here later

export const searchTasks = async (board: Board, term: string): Promise<{ exact: Task[]; similar: Task[] }> => {
