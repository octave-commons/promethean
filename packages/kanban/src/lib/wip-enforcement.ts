...
export class WIPLimitEnforcement {
  ...
  async generateCapacitySuggestions(
    ...
    const overCapacityCol = targetBoard.columns.find(
      (col) => columnKey(col.name) === columnKey(overCapacityColumn),
    );
  ...
  private getRecentViolations(columnName: string, hours: number): WIPViolation[] {
    ...
    return this.violationHistory.filter(
      (v) => columnKey(v.column) === columnKey(columnName) && new Date(v.timestamp) >= since,
    );
  }
