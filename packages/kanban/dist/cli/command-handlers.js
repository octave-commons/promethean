...
import { createTransitionRulesEngine } from '../lib/transition-rules.js';
...
const parseUiOptions = (tokens, acc = {}) => {
    if (tokens.length === 0) {
        return acc;
    }
...
const withBoard = async (context, effect) => {
    const board = await loadBoard(context.boardFile, context.tasksDir);
    return effect(board);
};
const handleCount = (args, context) => withBoard(context, (board) => {
...
const handleGetColumn = (args, context) => withBoard(context, (board) => {
    const mutableBoard = board;
    const column = getColumn(mutableBoard, requireArg(args[0], 'column name'));
    return column;
});
const handleGetByColumn = (args, context) => withBoard(context, (board) => {
    const mutableBoard = board;
    const tasks = getTasksByColumn(mutableBoard, requireArg(args[0], 'column name'));
    return tasks;
});
const handleFind = (args, context) => withBoard(context, (board) => {
    const mutableBoard = board;
...
const handleFindByTitle = (args, context) => withBoard(context, (board) => {
    const mutableBoard = board;
...
const handleUpdateStatus = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    ...
    try {
        ...
        console.warn('Warning: Transition rules engine not available:', error instanceof Error ? error.message : String(error));
    }
    ...
    try {
        ...
        console.warn('Warning: Event log manager not available:', error instanceof Error ? error.message : String(error));
    }
    const updated = await updateStatus(mutableBoard, id, status, context.boardFile, context.tasksDir, transitionRulesEngine, reason, eventLogManager, 'human');
    return updated;
});
const handleMove = (offset) => (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    const id = requireArg(args[0], 'task id');
    const result = await moveTask(mutableBoard, id, offset, context.boardFile);
    return result;
});
const handlePull = (_args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    const result = await pullFromTasks(mutableBoard, context.tasksDir, context.boardFile);
    // Enhanced logging for pull operation
    if (result.moved > 0) {
        console.log(`📝 Pull completed: ${result.added} added, ${result.moved} status changes from files`);
    }
    else {
...
const handlePush = (_args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    ...
    if (result.statusUpdated > 0) {
        console.log(`📝 Push completed: ${result.added} added, ${result.moved} moved, ${result.statusUpdated} manual edits preserved`);
    }
    else {
...
const handleSync = (_args, context) => withBoard(context, async (board) => {
    ...
    console.log(`📁 Files: ${result.tasks.added} added, ${result.tasks.moved} moved, ${result.tasks.statusUpdated} status updates`);
    console.log(`🔄 Total changes: ${totalChanges}`);
    return result;
});
const handleRegenerate = (_args, context) => {
    return regenerateBoard(context.tasksDir, context.boardFile);
};
const handleGenerateByTags = (args, context) => {
    if (args.length === 0) {
...
const handleIndexForSearch = (_args, context) => {
    return indexForSearch(context.tasksDir);
};
const handleSearch = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    const term = requireArg(args.join(' ').trim(), 'search term');
    const result = await searchTasks(mutableBoard, term);
    return result;
});
const handleUi = async (args, context) => {
...
const handleCompareTasks = async (args, context) => {
    if (args.length === 0) {
...
const handleBreakdownTask = async (args, context) => {
    if (args.length === 0) {
...
const handlePrioritizeTasks = async (_args, _context) => {
    if (_args.length === 0) {
...
const parseDevOptions = (tokens, acc = {}) => {
    if (tokens.length === 0) {
        return acc;
    }
    const [head, ...tail] = tokens;
...
const handleDev = async (args, context) => {
...
const handleShowTransitions = async (_args, context) => {
    try {
        // Try multiple reasonable config paths
        const possiblePaths = [
            // From current working directory (could be repo root)
...
const handleShowProcess = async (_args, _context) => {
...
const handleList = (args, context) => withBoard(context, async (board) => {
...
const handleAudit = (args, context) => withBoard(context, async (board) => {
    const configResult = await loadKanbanConfig({
        argv: process.argv,
        env: process.env,
    });
    ...
    const taskAnalyses = await Promise.all(allTasks.map(async ({ task, columnName }) => {
        ...
        const statusAnalysis = gitTracker.analyzeTaskStatus(task, taskFilePath || `${context.tasksDir}/${task.uuid}.md`);
        // Update progress
        processedTasks++;
        if (processedTasks % 50 === 0 || processedTasks === totalTasks) {
            process.stderr.write(`\r📊 Progress: ${processedTasks}/${totalTasks} (${Math.round((processedTasks / totalTasks) * 100)}%)`);
        }
        return {
            task,
            columnName,
    ...
    const illegalTransitionsFound = results.inconsistencies.filter((inc) => inc.invalidEvent).length;
    ...
    if (!verbose) {
        ...
        for (const inconsistency of results.inconsistencies) {
            ...
            if (inconsistency.invalidEvent) {
                console.log(`   🚨 ILLEGAL TRANSITION: ${inconsistency.invalidEvent.fromStatus} → ${inconsistency.invalidEvent.toStatus}`);
            ...
            if (!dryRun) {
                try {
                    // Fix the task status
                    await updateStatus(board, inconsistency.task.uuid, inconsistency.expected, context.boardFile, context.tasksDir, undefined, `Audit correction: Reset to last valid state from event log`, eventLogManager, 'system');
    ...
    if (orphanedEvents.length > 0) {
        if (verbose) {
            for (const { taskId, events } of orphanedEvents) {
                console.log(`⚠️  ORPHANED EVENTS: Task ${taskId} has ${events.length} events but not found in board`);
            ...
            console.log(`⚠️  Orphaned events: ${orphanedEvents.length} tasks have events but not in board`);
        }
    }
    ...
    const columnAnalysis = await analyzeColumnNormalization(board.columns.map((col) => col.name), canonicalStatuses);
    const actionableGroups = columnAnalysis.groups.filter((group) => group.members.some((member) => member.action !== 'keep'));
    if (actionableGroups.length > 0) {
        ...
        for (const group of actionableGroups) {
            ...
            for (const member of group.members) {
                if (member.action === 'keep')
                    continue;
                const verb = member.action === 'merge' ? 'merge into' : 'rename to';
                console.log(`     • ${member.originalName} → ${verb} ${group.canonicalName} (${member.reason})`);
            }
        }
        if (dryRun) {
            console.log('   💡 Run with --fix to apply these column normalization changes automatically');
        }
        else {
            const applied = applyColumnNormalization(board, columnAnalysis);
            if (applied > 0) {
                await writeBoard(context.boardFile, board);
                console.log(`   ✅ Applied ${applied} column normalization ${applied === 1 ? 'update' : 'updates'}`);
            }
            else {
    ...
    console.log(`   ✅ Healthy tasks: ${healthyTasksFound} (${((healthyTasksFound / totalTasksAnalyzed) * 100).toFixed(1)}%)`);
    console.log(`   ⚠️  Untracked tasks: ${untrackedTasksFound} (${((untrackedTasksFound / totalTasksAnalyzed) * 100).toFixed(1)}%)`);
    console.log(`   🚨 Truly orphaned tasks: ${orphanedTasksFound} (${((orphanedTasksFound / totalTasksAnalyzed) * 100).toFixed(1)}%)`);
    if (!dryRun) {
...
const handleCommitStats = (_args, context) => withBoard(context, async (board) => {
    const gitTracker = new TaskGitTracker({ repoRoot: process.cwd() });
...
const handleEnforceWipLimits = (args, context) => withBoard(context, async (board) => {
    const configResult = await loadKanbanConfig({
        argv: process.argv,
        env: process.env,
    });
    ...
    for (const column of board.columns) {
        ...
        if (column.limit && column.tasks.length > column.limit) {
            ...
            console.log(`   Current: ${column.tasks.length}/${column.limit} (${violationCount} over limit)`);
            ...
            for (const task of tasksToMove) {
                ...
                if (!dryRun) {
                    try {
                        ...
                        if (previousColumn) {
                            await updateStatus(board, task.uuid, previousColumn.name, context.boardFile, context.tasksDir, undefined, `WIP limit enforcement: moved from ${column.name} to ${previousColumn.name}`, eventLogManager, 'system');
                            totalCorrections++;
...
const findPreviousColumn = (currentColumnName, columns) => {
    const workflowOrder = [
        'icebox',
        'incoming',
        'accepted',
    ...
    const currentIndex = workflowOrder.findIndex((col) => columnKey(col) === columnKey(currentColumnName));
    if (currentIndex <= 0)
        return null;
...
const handleWipMonitor = (args, context) => withBoard(context, async (board) => {
    const configResult = await loadKanbanConfig();
    const wipEnforcement = await createWIPLimitEnforcement({
        config: configResult.config,
    });
    const monitor = await wipEnforcement.getCapacityMonitor(board);
    ...
    const displayMonitor = (data) => {
        ...
        for (const column of data.columns) {
            const icon = column.status === 'critical'
                ? '🚨'
            ...
            const utilizationBar = column.limit
                ...
                : 'N/A';
            console.log(`${icon} ${column.name.padEnd(15)} ${column.current.toString().padStart(3)}/${column.limit?.toString().padStart(3) || '∞'} ${utilizationBar} ${column.utilization.toFixed(1)}%`);
        }
        console.log('');
...
const handleWipCompliance = (args, context) => withBoard(context, async (_board) => {
    const configResult = await loadKanbanConfig();
    const wipEnforcement = await createWIPLimitEnforcement({
        config: configResult.config,
    });
...
const handleWipViolations = (args, context) => withBoard(context, async (_board) => {
    const configResult = await loadKanbanConfig();
    const wipEnforcement = await createWIPLimitEnforcement({
        config: configResult.config,
    });
    ...
    for (const violation of violations) {
        const icon = violation.severity === 'critical' ? '🚨' : violation.severity === 'error' ? '❌' : '⚠️';
...
const handleWipSuggestions = (args, context) => withBoard(context, async (board) => {
    const configResult = await loadKanbanConfig();
    const wipEnforcement = await createWIPLimitEnforcement({
        config: configResult.config,
    });
    ...
    console.log('');
    const suggestions = await wipEnforcement.generateCapacitySuggestions(column, board);
...
const parseCreateTaskArgs = (args) => {
    if (args.length === 0) {
...
const handleCreate = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
    const taskArgs = parseCreateTaskArgs(args);
    const newTask = await createTask(mutableBoard, taskArgs.status || 'incoming', {
...
const parseUpdateTaskArgs = (args) => {
    if (args.length === 0) {
...
const handleUpdate = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const parseDeleteTaskArgs = (args) => {
    if (args.length === 0) {
...
const handleDelete = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const parseCreateEpicArgs = (args) => {
    if (args.length === 0) {
...
const handleCreateEpic = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const handleAddTask = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const parseRemoveTaskArgs = (args) => {
    if (args.length < 2) {
...
const handleRemoveTask = (args, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const handleListEpics = (_, context) => withBoard(context, async (board) => {
    const mutableBoard = board;
...
const handleEpicStatus = (args, context) => withBoard(context, async (board) => {
    if (args.length === 0) {
 ...
 */
const handleHeal = (args, context) => withBoard(context, async () => {
    ...
    if (result.scar) {
        ...
        console.log(`   Range: ${result.scar.startSha.substring(0, 8)}..${result.scar.endSha.substring(0, 8)}`);
    }
    if (result.tagResult) {
        console.log('');
 ...
 */
function parseArgValue(argv, flag, defaultValue) {
 ...
 */
function parseArgValues(argv, flag) {
    const values = [];
...
const handleRebuildEventLog = createRebuildEventLogCommand('docs/agile/boards/generated.md', 'docs/agile/tasks').execute;
...
export const REMOTE_COMMANDS = Object.freeze(AVAILABLE_COMMANDS.filter((command) => command !== 'ui'));
