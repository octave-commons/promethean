...
const normalizeTask = (data, filePath, repoRoot, content) => {
    const rawId = data.id ?? data.uuid;
    ...
    const lastCommitSha = toOptionalString(data.lastCommitSha);
    const rawCommitHistory = data.commitHistory;
    const commitHistory = Array.isArray(rawCommitHistory)
        ? rawCommitHistory.filter((entry) => entry &&
            typeof entry === 'object' &&
...
const sortTasksById = (tasks) => Object.freeze([...tasks].sort((a, b) => a.id.localeCompare(b.id)));
export const indexTasks = async ({ tasksDir, exts, repoRoot, }) => {
    ...
    for (let i = 0; i < files.length; i += batchSize) {
        ...
        const validTasks = batchTasks.filter((task) => task !== null);
...
export const refreshTaskIndex = async (config) => {
 ...
 */
export const indexTasksToCache = async (options) => {
 ...
 */
export const migrateJsonlToCache = async (config, cache) => {
    const errors = [];
    let migrated = 0;
    try {
        ...
        for (const task of lines) {
            try {
                ...
                errors.push(`Failed to migrate task ${task.uuid}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
...
export const runIndexer = async (options) => {
