...
export class HealthConfigManager extends EventEmitter {
    ...
    async updateEnvironmentConfig(healthConfig) {
        const config = this.currentConfig || (await this.loadConfig());
    ...
    async rollbackToVersion(_version) {
...
export class ConfigValidator {
    static validateThresholds(thresholds) {
        const errors = [];
        const warnings = [];
        if (typeof thresholds.wipViolation !== 'number' ||
    ...
    static validateScheduling(scheduling) {
        const errors = [];
        const warnings = [];
        ...
        if (scheduling.reportGeneration) {
            ...
            if (scheduling.reportGeneration.weekly &&
...
//# sourceMappingURL=health-config-manager.js.map