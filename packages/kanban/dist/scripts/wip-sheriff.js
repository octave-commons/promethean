...
const BASIS = (argv.get("basis") ?? "points");
...
function splitKanbanSettings(md) {
...
async function parseBoard(md) {
    ...
    for (const line of lines) {
        ...
        if (current && itemMatch) {
            ...
            const filepath = path.join(TASKS_DIR, slug.endsWith(".md") ? slug : `${slug}.md`);
...
function laneUsage(l) {
    return BASIS === "points"
        ? l.cards.reduce((a, c) => a + (c.points || 1), 0)
        : l.cards.length;
}
function nearestSafeLeft(lanes, idx) {
    for (let i = idx - 1; i >= 0; i--) {
        const lane = lanes[i];
        if (!lane)
            continue;
        if (lane.capacity == null || lane.title.startsWith(SAFE_LEFT))
            return i;
    }
    if (lanes.length === 0) {
        return 0;
...
function renderBoard(lanes, preamble, footer) {
    const out = [];
    if (preamble)
...
(async function main() {
    ...
    const doingIdxs = DOING.map((name) => laneIndex.get(laneName(name))).filter((i) => i != null);
...
//# sourceMappingURL=wip-sheriff.js.map