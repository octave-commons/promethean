...
test('each available command has a registered handler', (t) => {
    for (const command of AVAILABLE_COMMANDS) {
        t.true(typeof COMMAND_HANDLERS[command] === 'function', `${command} should map to a command handler`);
    }
});
// CRUD Command Tests
...
test('create command creates a new task with basic title', async (t) => {
    ...
    const result = (await executeCommand('create', ['Test task'], context));
...
test('update command updates task title', async (t) => {
    const { context } = await setupTestEnvironment(t);
    // First create a task
    const created = (await executeCommand('create', ['Original title'], context));
    // Then update the title
    const result = (await executeCommand('update', [created.uuid, '--title=Updated title'], context));
...
test('update command updates task content', async (t) => {
    const { context } = await setupTestEnvironment(t);
    // First create a task
    const created = (await executeCommand('create', ['Test content'], context));
    // Then update the content
    const result = (await executeCommand('update', [created.uuid, '--content=Updated content goes here'], context));
...
test('update command updates both title and content', async (t) => {
    ...
    const result = (await executeCommand('update', [created.uuid, '--title=New title', '--content=New content'], context));
...
test('create command allows valid starting status icebox via CLI', async (t) => {
    const { context } = await setupTestEnvironment(t);
    const result = (await executeCommand('create', ['Test task in icebox', '--status=icebox'], context));
    t.is(result.title, 'Test task in icebox');
    t.is(result.status, 'icebox');
});
test('create command allows valid starting status incoming via CLI', async (t) => {
    const { context } = await setupTestEnvironment(t);
    const result = (await executeCommand('create', ['Test task in incoming', '--status=incoming'], context));
...
test('delete command requires confirmation', async (t) => {
    const { context } = await setupTestEnvironment(t);
    // First create a task
    const created = (await executeCommand('create', ['Task to delete'], context));
    // Try to delete without confirmation
    const result = (await executeCommand('delete', [created.uuid], context));
...
test('delete command with confirmation', async (t) => {
    const { context } = await setupTestEnvironment(t);
    // First create a task
    const created = (await executeCommand('create', ['Task to delete confirmed'], context));
    // Delete with confirmation
    const result = (await executeCommand('delete', [created.uuid, '--confirm'], context));
...
//# sourceMappingURL=command-handlers.test.js.map