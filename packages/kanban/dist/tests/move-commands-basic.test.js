...
test('move_up - basic functionality', async (t) => {
    ...
    const result = (await executeCommand('move_up', [task2.uuid], context));
    t.truthy(result);
...
test('move_down - basic functionality', async (t) => {
    ...
    const result = (await executeCommand('move_down', [task2.uuid], context));
    t.truthy(result);
    t.is(result.uuid, task2.uuid);
    t.is(result.column, 'Todo');
...
test('move_up - first task (no-op)', async (t) => {
    ...
    // Create tasks first using the create command
    const task1 = (await executeCommand('create', ['First task', '--status=Todo'], context));
    await executeCommand('create', ['Second task', '--status=Todo'], context);
    // Moving first task up should be no-op
    const result = (await executeCommand('move_up', [task1.uuid], context));
    t.truthy(result);
...
//# sourceMappingURL=move-commands-basic.test.js.map