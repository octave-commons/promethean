...
import { makeBoard, makeTask, snapshotTaskFiles, writeTaskFile, withTempDir, } from "../test-utils/helpers.js";
...
test("There are no broken links on the generated board file", async (t) => {
    ...
    for (const target of wikiLinkTargets) {
        t.true(existingFiles.has(`${target}.md`), `Expected board link [[${target}]] to have a matching task file`);
    }
});
test("There are no redundant board columns", async (t) => {
    ...
    for (const [, content] of snapshot.entries()) {
        t.true(content.includes('status: "Todo"'), "Expected task status to normalize to canonical column name");
    }
});
test("Task statuses are case insensitive, always mapping to a single column regardless of case or punctuation", async (t) => {
    ...
    await mkdir(tasksDir, { recursive: true });
    await writeFile(boardPath, "", "utf8");
    const statusVariants = ["In-Progress", "in-progress", "IN-PROGRESS"];
    await Promise.all(statusVariants.map(async (status, index) => {
        const task = makeTask({
    ...
    const snapshot = await snapshotTaskFiles(tasksDir);
    for (const [, content] of snapshot.entries()) {
        t.true(content.includes('status: "In-Progress"'), "Expected task status to normalize to the canonical column name");
    }
});
