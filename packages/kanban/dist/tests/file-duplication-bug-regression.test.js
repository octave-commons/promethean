...
test('FAILING REPRODUCTION: Descriptive filenames with dots get duplicated with " 2.md" suffix', async (t) => {
    ...
    const createdTask = await createTask(board, 'icebox', { title: fullTitle, content: 'First task content' }, tasksDir, boardPath);
...
test('FAILING REPRODUCTION: Pure numeric filenames do not get duplicated', async (t) => {
    ...
    await createTask(board, 'icebox', { title: numericTitle, content: 'Numeric task content' }, tasksDir, boardPath);
...
test('FAILING REPRODUCTION: Multiple duplication creates " 2 2.md" pattern', async (t) => {
    ...
    await createTask(board, 'icebox', { title: problematicTitle, content: 'First task content' }, tasksDir, boardPath);
...
test('FAILING REPRODUCTION: Complex descriptive names with special characters get duplicated', async (t) => {
    ...
    for (const title of complexTitles) {
        await createTask(board, 'icebox', { title, content: `Content for ${title}` }, tasksDir, boardPath);
    }
...
test('FAILING REPRODUCTION: Mixed filename types show inconsistent behavior', async (t) => {
    ...
    const numericTitle = '20251011235145';
    await createTask(board, 'icebox', { title: descriptiveTitle, content: 'Descriptive content' }, tasksDir, boardPath);
    await createTask(board, 'icebox', { title: numericTitle, content: 'Numeric content' }, tasksDir, boardPath);
    // Trigger the duplication
    await regenerateBoard(tasksDir, boardPath);
    const files = await readdir(tasksDir);
...
test('FAILING REPRODUCTION: Board load and save cycle creates duplicates', async (t) => {
    ...
    await createTask(board, 'icebox', { title: descriptiveTitle, content: 'Test content' }, tasksDir, boardPath);
...
//# sourceMappingURL=file-duplication-bug-regression.test.js.map