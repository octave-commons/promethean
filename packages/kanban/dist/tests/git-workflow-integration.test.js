...
test.beforeEach(async (t) => {
    ...
    await fs.writeFile(path.join(testDir, 'promethean.kanban.json'), JSON.stringify(kanbanContent, null, 2));
    await fs.writeFile(path.join(testDir, 'docs', 'agile', 'boards', 'generated.md'), '# Kanban Board\n\nNo tasks yet.');
...
test('postOperation creates commits and tags', async (t) => {
    ...
    // Create some changes
    await fs.writeFile(path.join(testDir, 'docs', 'agile', 'tasks', 'test-task.md'), '# Test Task\n\nTest content');
...
`;
        await fs.writeFile(path.join(testDir, 'docs', 'agile', 'tasks', `${task.uuid}.md`), taskContent);
    }
    // Update kanban board
...
//# sourceMappingURL=git-workflow-integration.test.js.map