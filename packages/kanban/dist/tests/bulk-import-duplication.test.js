...
test('bulk import operations do not create duplicates', async (t) => {
    ...
    t.true(task1?.content?.includes('Content 1') ?? false, 'First task should contain original content');
    t.true(task1?.content?.includes('## ⛓️ Blocked By') ?? false, 'First task should have Blocked By section');
    t.true(task2?.content?.includes('Content 2') ?? false, 'Second task should contain original content');
    t.true(task2?.content?.includes('## ⛓️ Blocked By') ?? false, 'Second task should have Blocked By section');
    t.true(task3?.content?.includes('Content 3') ?? false, 'Third task should contain original content');
    t.true(task3?.content?.includes('## ⛓️ Blocked By') ?? false, 'Third task should have Blocked By section');
});
test('bulk import with different columns allows same titles', async (t) => {
    ...
    for (const taskData of crossColumnTasks) {
        const task = await createTask(board, taskData.column, { title: taskData.title, content: taskData.content }, tasksDir, boardPath);
        createdTasks.push(task);
    }
    ...
    // All tasks should have formatted content with sections
    t.true(incomingTask?.content?.includes('Incoming content') ?? false, 'Incoming task should contain original content');
    t.true(incomingTask?.content?.includes('## ⛓️ Blocked By') ?? false, 'Incoming task should have Blocked By section');
    t.true(readyTask?.content?.includes('Ready content') ?? false, 'Ready task should contain original content');
    t.true(readyTask?.content?.includes('## ⛓️ Blocked By') ?? false, 'Ready task should have Blocked By section');
    t.true(inProgressTask?.content?.includes('In-progress content') ?? false, 'In-progress task should contain original content');
    t.true(inProgressTask?.content?.includes('## ⛓️ Blocked By') ?? false, 'In-progress task should have Blocked By section');
});
//# sourceMappingURL=bulk-import-duplication.test.js.map