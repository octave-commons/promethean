...
test('e2e - complete workflow with move operations', async (t) => {
    ...
    const task1 = (await executeCommand('create', ['Initial Task 1', '--status=Todo'], context));
    const task2 = (await executeCommand('create', ['Initial Task 2', '--status=Todo'], context));
    const task3 = (await executeCommand('create', ['Initial Task 3', '--status=Todo'], context));
    ...
    // Move task3 to top
    const move1Result = (await executeCommand('move_up', [task3.uuid], context));
    t.truthy(move1Result);
    t.is(move1Result.rank, 1);
    // Move task3 to top again
    const move2Result = (await executeCommand('move_up', [task3.uuid], context));
    t.truthy(move2Result);
    t.is(move2Result.rank, 0);
    // Move task1 to bottom
    const move3Result = (await executeCommand('move_down', [task1.uuid], context));
    t.truthy(move3Result);
    t.is(move3Result.rank, 2);
    const move4Result = (await executeCommand('move_down', [task1.uuid], context));
    t.truthy(move4Result);
    t.is(move4Result.rank, 2);
    // Phase 3: Add more tasks to different columns
    const task4 = (await executeCommand('create', ['Progress Task 1', '--status=In Progress'], context));
    const task5 = (await executeCommand('create', ['Done Task 1', '--status=Done'], context));
    // Phase 4: Move tasks in different columns
    const progressMove = (await executeCommand('move_up', [task4.uuid], context));
    t.truthy(progressMove);
    t.is(progressMove.column, 'In Progress');
    const doneMove = (await executeCommand('move_up', [task5.uuid], context));
    t.truthy(doneMove);
    ...
    const persistenceMove = (await executeCommand('move_down', [task2.uuid], newContext));
...
test('e2e - large scale move operations', async (t) => {
    ...
    for (let i = 0; i < 49; i++) {
        const result = (await executeCommand('move_down', [lastTask.uuid], context));
        t.truthy(result);
    }
...
test('e2e - move operations with task updates', async (t) => {
    ...
    const task2 = (await executeCommand('create', ['Original Task 2', '--status=Todo'], context));
    ...
    // Move task2 down
    const moveDownResult = (await executeCommand('move_down', [task2.uuid], context));
...
test('e2e - move operations across workflow transitions', async (t) => {
    ...
    const task = (await executeCommand('create', ['Workflow Task', '--status=Todo'], context));
    // Move within Todo (no-op since single task)
    const todoMove = (await executeCommand('move_up', [task.uuid], context));
    t.truthy(todoMove);
    ...
    const progressMove = (await executeCommand('move_up', [task.uuid], context));
    t.truthy(progressMove);
    t.is(progressMove.column, 'In Progress');
    // Add another task to In Progress
    const task2 = (await executeCommand('create', ['Another Task', '--status=In Progress'], context));
    // Now moves should work within In Progress
    const progressMove2 = (await executeCommand('move_up', [task2.uuid], context));
    ...
    const doneMove = (await executeCommand('move_up', [task.uuid], context));
    t.truthy(doneMove);
    t.is(doneMove.column, 'Done');
...
//# sourceMappingURL=move-commands-e2e-simple.test.js.map