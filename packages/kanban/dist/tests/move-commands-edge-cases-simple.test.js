...
test('move operations - single task in column', async (t) => {
    ...
    // Create single task
    const task1 = (await executeCommand('create', ['Single Task', '--status=Todo'], context));
    // Both moves should be no-ops
    const upResult = (await executeCommand('move_up', [task1.uuid], context));
    t.truthy(upResult);
    t.is(upResult.rank, 0);
    const downResult = (await executeCommand('move_down', [task1.uuid], context));
    t.truthy(downResult);
...
//# sourceMappingURL=move-commands-edge-cases-simple.test.js.map