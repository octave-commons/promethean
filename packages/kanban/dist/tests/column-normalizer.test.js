...
test('analyzeColumnNormalization detects review aliases', async (t) => {
    const analysis = await analyzeColumnNormalization(['Review', 'In_Review', 'QA Review'], ['todo', 'in_progress', 'review', 'done']);
    ...
    if (!reviewGroup)
        return;
    t.deepEqual(reviewGroup.members.map((member) => member.action).sort(), ['keep', 'merge', 'merge']);
});
test('applyColumnNormalization merges duplicate review columns', async (t) => {
    ...
    const analysis = await analyzeColumnNormalization(board.columns.map((col) => col.name), ['todo', 'in_progress', 'review', 'done']);
...
test('applyColumnNormalization renames lone alias when canonical missing', async (t) => {
    ...
    const analysis = await analyzeColumnNormalization(board.columns.map((col) => col.name), ['ready', 'review', 'done']);
    const changes = applyColumnNormalization(board, analysis);
    t.true(changes > 0);
    t.is(board.columns.length, 1);
...
//# sourceMappingURL=column-normalizer.test.js.map