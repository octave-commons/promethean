...
import { createScarContextBuilder, DEFAULT_SCAR_CONTEXT_OPTIONS, } from '../lib/heal/scar-context-builder.js';
...
const cleanupTestEnvironment = async (tempDir) => {
...
test('ScarContextBuilder - LLM operations tracking', async (t) => {
    ...
    try {
        ...
        // Add LLM operation
        await builder.addLLMOperation(context, 'analyze-task', { taskId: 'test-123', analysisType: 'quality' }, { score: 0.8, issues: ['missing-labels'] }, 150);
...
test('ScarContextBuilder - Progress updates', async (t) => {
    ...
    try {
        ...
        // Update progress
        await builder.updateProgress(context, 'custom-operation', { step: 1, result: 'success' }, 'info');
        await builder.updateProgress(context, 'warning-operation', { issue: 'something went wrong' }, 'warning');
...
//# sourceMappingURL=scar-context-builder.test.js.map