...
test('createTask is idempotent - same title returns existing task', async (t) => {
    ...
    // Create first task
    const firstTask = await createTask(board, 'incoming', { title: taskTitle, content: taskContent }, tasksDir, boardPath);
    // Create second task with same title
    const secondTask = await createTask(board, 'incoming', { title: taskTitle, content: taskContent }, tasksDir, boardPath);
...
test('createTask prevents duplicate titles with different content', async (t) => {
    ...
    // Create first task
    const firstTask = await createTask(board, 'incoming', { title: taskTitle, content: 'First content' }, tasksDir, boardPath);
    // Attempt to create second task with same title but different content
    const secondTask = await createTask(board, 'incoming', { title: taskTitle, content: 'Different content' }, tasksDir, boardPath);
    // Should return the first task, not create a duplicate
    t.is(firstTask.uuid, secondTask.uuid, 'Should return existing task UUID');
    t.true(firstTask.content?.includes('First content') && secondTask.content?.includes('First content'), 'Both tasks should contain original content');
    // Both should have the formatted content with sections
    t.true(firstTask.content?.includes('## ⛓️ Blocked By') ?? false, 'First task should have Blocked By section');
    t.true(firstTask.content?.includes('## ⛓️ Blocks') ?? false, 'First task should have Blocks section');
});
test('createTask allows same title in different columns', async (t) => {
    ...
    const incomingTask = await createTask(board, 'incoming', { title: taskTitle, content: 'Todo content' }, tasksDir, boardPath);
...
test('createTask handles case-insensitive title matching', async (t) => {
    ...
    const firstTask = await createTask(board, 'incoming', { title: 'case sensitive task', content: 'First' }, tasksDir, boardPath);
    // Create task with uppercase title
    const secondTask = await createTask(board, 'incoming', { title: 'CASE SENSITIVE TASK', content: 'Second' }, tasksDir, boardPath);
...
test('createTask trims whitespace for title matching', async (t) => {
    ...
    const firstTask = await createTask(board, 'incoming', { title: 'Whitespace Task', content: 'First' }, tasksDir, boardPath);
    // Create task with extra whitespace
    const secondTask = await createTask(board, 'incoming', { title: '  Whitespace Task  ', content: 'Second' }, tasksDir, boardPath);
...
test('board regeneration does not create duplicate tasks', async (t) => {
    ...
    // Create initial task
    const originalTask = await createTask(board, 'incoming', { title: taskTitle, content: 'Original content' }, tasksDir, boardPath);
    ...
    const incomingColumn = regeneratedBoard.columns.find((col) => col.name === 'incoming');
    const regeneratedTask = incomingColumn?.tasks.find((task) => task.title === taskTitle);
...
test('createTask with UUID uses existing task if title matches', async (t) => {
    ...
    const firstTask = await createTask(board, 'incoming', { title: taskTitle, content: 'First', uuid: specificUuid }, tasksDir, boardPath);
    // Create task with same title but different UUID
    const secondTask = await createTask(board, 'incoming', { title: taskTitle, content: 'Second', uuid: 'different-uuid-67890' }, tasksDir, boardPath);
...
//# sourceMappingURL=task-duplication-regression.test.js.map