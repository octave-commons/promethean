...
import { pullFromTasks, pushToTasks, regenerateBoard, syncBoardAndTasks, } from "../lib/kanban.js";
import { makeBoard, makeTask, withTempDir, writeTaskFile, } from "../test-utils/helpers.js";
...
test("regenerateBoard overwrites duplicate board entries", async (t) => {
    ...
    const boardContent = await readFile(boardPath, "utf8");
    const uuidMatches = Array.from(boardContent.matchAll(/\(uuid:([^)]+)\)/g)).map((match) => match[1]);
