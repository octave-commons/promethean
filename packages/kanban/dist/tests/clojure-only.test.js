...
test('TransitionRulesEngine requires Clojure DSL - throws error when DSL path missing', async (t) => {
    ...
    await t.throwsAsync(async () => engine.initialize(), { message: 'Clojure DSL path is required. TypeScript transition rules are no longer supported.' });
});
test('TransitionRulesEngine requires Clojure DSL - throws error when DSL file not found', async (t) => {
    const configWithMissingDSL = {
        enabled: true,
        enforcement: 'strict',
    ...
    await t.throwsAsync(async () => engine.initialize(), { message: /Clojure DSL not found at: \/nonexistent\/path\/rules\.clj/ });
});
...
//# sourceMappingURL=clojure-only.test.js.map