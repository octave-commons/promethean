...
import { archiveTask, countTasks, createTask, deleteTask, findTaskById, findTaskByTitle, getColumn, getTasksByColumn, moveTask, pullFromTasks, pushToTasks, regenerateBoard, renameTask, searchTasks, syncBoardAndTasks, updateStatus, updateTaskDescription, } from '../lib/kanban.js';
import { getTaskFileByUuid, makeBoard, makeTask, withTempDir, writeTaskFile, } from '../test-utils/helpers.js';
...
test('move_up', async (t) => {
    ...
    t.truthy(result);
    t.deepEqual(getColumn(board, 'Todo').tasks.map((task) => task.uuid), ['todo-2', 'todo-1']);
});
...
test('createTask', async (t) => {
    ...
    await mkdir(tasksDir, { recursive: true });
    await writeFile(boardPath, '', 'utf8');
    const created = await createTask(board, 'Todo', {
...
test('updateTaskDescription', async (t) => {
    ...
    await writeFile(boardPath, '', 'utf8');
    await writeTaskFile(tasksDir, board.columns[0].tasks[0]);
    const updated = await updateTaskDescription(board, 'todo-1', 'Updated task body', tasksDir, boardPath);
    t.truthy(updated);
    t.is(getColumn(board, 'Todo').tasks.find((task) => task.uuid === 'todo-1')?.content, 'Updated task body');
    const persisted = await getTaskFileByUuid(tasksDir, 'todo-1');
    t.truthy(persisted);
