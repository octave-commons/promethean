...
const createLifecycleTestContext = async () => {
    ...
    const store = new Map();
    const createTask = async (uuid, title, status, body) => {
...
`;
        ...
        const task = { uuid, slug, title, status, content, sourcePath: filePath };
        store.set(uuid, task);
        return task;
    };
    const cache = {
        ...
        backupTask: async (uuid) => {
            const task = store.get(uuid);
            if (!task)
                throw new Error('Task not found for backup');
            const backupPath = path.join(root, `${uuid}.backup.md`);
            await writeFile(backupPath, task.content, 'utf8');
            return backupPath;
        },
    };
    const manager = new TaskLifecycleManager({ cache }, tasksDir, archiveDir);
    return {
        root,
        tasksDir,
        archiveDir,
...
//# sourceMappingURL=task-content-lifecycle.test.js.map