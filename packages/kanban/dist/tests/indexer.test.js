...
const writeTaskMarkdown = async (dir, fileName, task) => {
    const frontmatter = [
        '---',
        `id: ${task.id}`,
        `uuid: ${task.uuid}`,
...
const createSearchIndexFixture = async (repoDir) => {
    ...
    return { tasksDir, indexFile, configPath };
};
const seedSearchTasks = async (tasksDir) => {
...
const readIndexEntries = async (indexFile) => {
