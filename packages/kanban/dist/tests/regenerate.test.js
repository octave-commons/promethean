...
import { makeTask, snapshotTaskFiles, withTempDir, writeTaskFile, } from "../test-utils/helpers.js";
...
test("regenerateBoard retains tasks from both canonical and legacy schemas", async (t) => {
    ...
    await writeFile(path.join(tasksDir, "legacy-task.md"), legacyFrontmatter, "utf8");
...
test("regenerateBoard links to the hyphenated basename when titles have spaces", async (t) => {
    ...
    await regenerateBoard(tasksDir, boardPath);
    const boardContent = await readFile(boardPath, "utf8");
    t.true(boardContent.includes("[[spaced-out-task|Spaced Out Task]]"), "Board should link to the hyphenated basename");
    t.false(boardContent.includes("[[Spaced Out Task]]"), "Board should not link directly to the spaced title");
});
test("regenerateBoard drops stale fallback entries from existing board", async (t) => {
    ...
    const uuids = Array.from(regeneratedBoard.matchAll(/\(uuid:([^)]+)\)/g)).map((match) => match[1]);
