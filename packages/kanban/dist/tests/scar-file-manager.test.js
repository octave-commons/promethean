...
test('ScarFileManager constructor with custom config', (t) => {
    const testFilePath = t.context.testFilePath;
    const manager = new ScarFileManager({
...
test('loadScars loads valid scar records', async (t) => {
    ...
    const lines = mockScarRecords.map(scar => JSON.stringify({ ...scar, timestamp: scar.timestamp.toISOString() }));
...
//# sourceMappingURL=scar-file-manager.test.js.map