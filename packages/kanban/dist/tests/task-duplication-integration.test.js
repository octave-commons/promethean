...
test('integration: board operations do not create duplicate files', async (t) => {
    ...
    const task1 = await createTask(board, 'incoming', { title: 'Task 1', content: 'Content 1' }, tasksDir, boardPath);
    const task2 = await createTask(board, 'ready', { title: 'Task 2', content: 'Content 2' }, tasksDir, boardPath);
    const task3 = await createTask(board, 'in-progress', { title: 'Task 3', content: 'Content 3' }, tasksDir, boardPath);
    ...
    t.is(fileCountAfterCreation, fileCountAfterRegeneration, 'Board regeneration should not create new files');
    // Verify original tasks still exist
    const regeneratedBoard = await loadBoard(boardPath, tasksDir);
    const incomingTasks = regeneratedBoard.columns.find((col) => col.name === 'incoming')?.tasks || [];
    const readyTasks = regeneratedBoard.columns.find((col) => col.name === 'ready')?.tasks || [];
    const inProgressTasks = regeneratedBoard.columns.find((col) => col.name === 'in-progress')?.tasks || [];
...
test('integration: concurrent task creation does not create duplicates', async (t) => {
    ...
    const concurrentTasks = await Promise.all([
        createTask(board, 'incoming', { title: 'Concurrent Task', content: 'Version 1' }, tasksDir, boardPath),
        createTask(board, 'incoming', { title: 'Concurrent Task', content: 'Version 2' }, tasksDir, boardPath),
        createTask(board, 'incoming', { title: 'Concurrent Task', content: 'Version 3' }, tasksDir, boardPath),
    ]);
    ...
    t.true(concurrentTasks[0].content?.includes('Version 1') ?? false, 'Should preserve first task content');
    t.true(concurrentTasks[0].content?.includes('## ⛓️ Blocked By') ?? false, 'Should have Blocked By section');
});
test('integration: mixed operations maintain data integrity', async (t) => {
    ...
    const board = makeBoard([]);
    // Create tasks
    const taskA = await createTask(board, 'incoming', { title: 'Task A', content: 'Content A' }, tasksDir, boardPath);
    const taskB = await createTask(board, 'ready', { title: 'Task B', content: 'Content B' }, tasksDir, boardPath);
    // Regenerate board
    await regenerateBoard(tasksDir, boardPath);
    // Try to create duplicate
    const duplicateTaskA = await createTask(board, 'incoming', { title: 'Task A', content: 'New Content' }, tasksDir, boardPath);
    // Create new task
    const taskC = await createTask(board, 'incoming', { title: 'Task C', content: 'Content C' }, tasksDir, boardPath);
    // Regenerate again
    const finalBoard = await loadBoard(boardPath, tasksDir);
    // Verify data integrity
    const incomingTasks = finalBoard.columns.find((col) => col.name === 'incoming')?.tasks || [];
    const readyTasks = finalBoard.columns.find((col) => col.name === 'ready')?.tasks || [];
    t.is(incomingTasks.length, 2, 'Should have 2 incoming tasks');
    t.is(readyTasks.length, 1, 'Should have 1 ready task');
    const finalTaskA = incomingTasks.find((task) => task.title === 'Task A');
    const finalTaskC = incomingTasks.find((task) => task.title === 'Task C');
...
test('integration: file system reflects board state accurately', async (t) => {
    ...
    await createTask(board, 'incoming', { title: 'Todo Task 1', content: 'Todo 1' }, tasksDir, boardPath);
    await createTask(board, 'incoming', { title: 'Todo Task 2', content: 'Todo 2' }, tasksDir, boardPath);
    await createTask(board, 'incoming', { title: 'Ready Task', content: 'Ready' }, tasksDir, boardPath);
    await createTask(board, 'incoming', { title: 'In Progress Task', content: 'In Progress' }, tasksDir, boardPath);
    // Get file count
    const files = await readdir(tasksDir);
    const taskFiles = files.filter((file) => file.endsWith('.md'));
    // Load board and count tasks
    const loadedBoard = await loadBoard(boardPath, tasksDir);
    const totalTasks = loadedBoard.columns.reduce((sum, col) => sum + col.tasks.length, 0);
    ...
    await createTask(board, 'incoming', { title: 'Todo Task 1', content: 'Duplicate' }, tasksDir, boardPath);
    await createTask(board, 'ready', { title: 'Ready Task', content: 'Duplicate' }, tasksDir, boardPath);
    ...
    t.is(taskFilesAfter.length, taskFiles.length, 'File count should not change after duplicate creation');
});
test('integration: special characters in titles do not cause duplicates', async (t) => {
    ...
    for (const title of specialTitles) {
        const task = await createTask(board, 'incoming', { title, content: `Content for ${title}` }, tasksDir, boardPath);
        createdTasks.push(task);
    }
    // Try to create duplicates
    const duplicateTasks = [];
    for (const title of specialTitles) {
        const task = await createTask(board, 'incoming', { title, content: `Duplicate content for ${title}` }, tasksDir, boardPath);
    ...
    for (let i = 0; i < createdTasks.length; i++) {
        ...
        t.true(createdTasks[i]?.content?.includes(`Content for ${specialTitles[i]}`) ?? false, `Content ${i} should be preserved`);
    }
...
//# sourceMappingURL=task-duplication-integration.test.js.map