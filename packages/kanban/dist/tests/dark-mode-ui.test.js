...
test.describe('Dark Mode UI', () => {
    ...
    test.beforeAll(async () => {
        const dir = await withTempDir({ teardown: () => { } });
    ...
    test('dark mode toggle button has proper hover and active states', async ({ page }) => {
        ...
        const hoverTransform = await toggleButton.evaluate((el) => {
            return getComputedStyle(el).transform;
        });
        expect(hoverTransform).not.toBe('none');
        // Check active state
        await toggleButton.click();
        const activeTransform = await toggleButton.evaluate((el) => {
            return getComputedStyle(el).transform;
        });
...
//# sourceMappingURL=dark-mode-ui.test.js.map