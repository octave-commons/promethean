...
export const createTransitionRulesEngineState = (config) => {
...
export const initializeTransitionRulesEngine = async (state) => {
    if (!state.config.dslPath) {
        throw new Error('Clojure DSL path is required. TypeScript transition rules are no longer supported.');
    }
    try {
        ...
        throw new Error(`Clojure DSL not found at: ${state.config.dslPath}. Transition rules cannot function without Clojure DSL.`);
    }
};
// Check if a transition from one status to another is allowed
export const validateTransition = async (state, from, to, task, board) => {
    ...
    for (const globalRule of state.config.globalRules) {
        if (!globalRule.enabled)
            continue;
        try {
            const passes = await evaluateGlobalRule(state, globalRule, fromNormalized, toNormalized, task, board);
            if (!passes) {
    ...
    if (fromNormalized === 'testing' && toNormalized === 'review') {
        try {
            ...
            violations.push(`Testing transition validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    if (state.dslAvailable) {
        try {
            const dslResult = await evaluateCustomRule(state, `(evaluate-transition "${fromNormalized}" "${toNormalized}" task board)`, [], task, board);
            if (!dslResult) {
                violations.push(`Clojure DSL evaluation: transition ${fromNormalized} → ${toNormalized} is not allowed`);
            }
        }
        catch (error) {
            console.warn(`Failed to evaluate Clojure DSL transition:`, error);
            violations.push(`Clojure DSL evaluation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
...
export const getValidTransitions = (config, from) => {
    const fromNormalized = normalizeColumnName(from);
...
export const debugTransition = async (state, from, to, task, board) => {
    const fromNormalized = normalizeColumnName(from);
    const toNormalized = normalizeColumnName(to);
    // Evaluate WIP check
...
export const showProcessFlow = (config) => {
...
export const validateTestingToReviewTransition = async (state, task, _board) => {
    const violations = [];
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Testing transition validation timeout')), state.testingConfig.timeouts.totalAnalysis);
        });
...
export const extractTestingInfo = (task) => {
...
export const getTransitionsOverview = (state) => {
    const validTransitions = state.config.rules.flatMap((rule) => rule.from.flatMap((from) => rule.to.map((to) => ({
        from,
        to,
...
// Helper functions
export const normalizeColumnName = (column) => {
...
export const findTransitionRule = (config, from, to) => {
    return config.rules.find((rule) => rule.from.includes(from) && rule.to.includes(to));
};
export const isBackwardTransition = (from, to) => {
    const workflowOrder = [
        'icebox',
        'incoming',
        'accepted',
...
export const evaluateGlobalRule = async (state, rule, from, to, task, board) => {
...
export const evaluateWipLimit = async (targetColumn, board) => {
...
export const evaluateTaskExistence = (sourceColumn, task, board) => {
    const column = board.columns.find((col) => normalizeColumnName(col.name) === sourceColumn);
    if (!column)
        return false;
    return column.tasks.some((t) => t.uuid === task.uuid);
};
export const evaluateCustomCheck = async (state, checkName, task, board) => {
...
export const evaluateCustomRule = async (state, ruleImpl, _args, task, board) => {
    if (!state.dslAvailable) {
        throw new Error('Clojure DSL is required but not available. TypeScript transition rules are no longer supported.');
    }
    try {
...
const runTestingValidation = async (state, task, testingInfo) => {
    const request = {
        task: {
            uuid: task.uuid,
            title: task.title,
        ...
        }, // Type assertion to handle interface differences
        changedFiles: [],
        affectedPackages: [],
        reportPath: testingInfo.coverageReportPath,
        format: 'json',
    };
    await runTestingTransition(request, testingInfo.executedTests || [], testingInfo.requirementMappings || [], state.testingConfig, [], // testFiles - empty for now
    '/tmp');
};
//# sourceMappingURL=transition-rules-functional.js.map