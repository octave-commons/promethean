...
import { ollamaJSON, createLogger } from '@promethean-os/utils';
import { z } from 'zod';
...
const slugFromFileName = (fileName) => fileName.replace(/\.(md|json)$/i, '');
const extractFallbackTaskInfo = (filePath, raw) => {
...
const readTasksFromDirectory = async (dir) => {
    let entries = [];
    try {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            const estimatesObj = frontmatter.estimates;
            const estimates = estimatesObj && typeof estimatesObj === 'object'
                ? {
                    complexity: typeof estimatesObj.complexity === 'number'
                        ? estimatesObj.complexity
                        : typeof estimatesObj.complexity === 'string'
                            ? parseInt(estimatesObj.complexity, 10)
                            : undefined,
                    scale: typeof estimatesObj.scale === 'number'
                        ? estimatesObj.scale
 ...
 */
function analyzeTaskContent(task) {
    ...
    const complexityScore = complexityIndicators.filter((indicator) => fullText.includes(indicator)).length;
    const highComplexityScore = highComplexityIndicators.filter((indicator) => fullText.includes(indicator)).length;
    const lowComplexityScore = lowComplexityIndicators.filter((indicator) => fullText.includes(indicator)).length;
    const technicalComplexity = Math.max(1, Math.min(5, 2 + complexityScore * 0.5 + highComplexityScore * 1.5 - lowComplexityScore * 0.5));
    const researchComplexity = fullText.includes('research') ||
    ...
    const integrationComplexity = fullText.includes('integrate') ||
    ...
    const estimatedHours = Math.max(0.25, locImpact / 50 + fileCount * 0.5 + technicalComplexity * 0.5 + integrationComplexity * 0.5);
    return {
        locImpact,
        fileCount,
        technicalComplexity,
        ...
        hasExternalDependencies: fullText.includes('api') ||
            fullText.includes('service') ||
 ...
 */
function generateComplexityEstimationPrompt(task) {
 ...
 */
async function estimateTaskComplexity(task, model = 'qwen2.5:3b') {
    ...
    try {
        ...
        const parsed = ComplexitySchema.parse(raw);
        ...
        logger.warn(`LLM complexity estimation failed for ${task.title}, using fallback analysis:`, toErrorFields(error));
        // Fallback to rule-based estimation
        const overallScore = Math.min(10, Math.max(1, (baseAnalysis.technicalComplexity +
            baseAnalysis.integrationComplexity +
            baseAnalysis.researchComplexity) *
            1.5));
        const complexityLevel = overallScore <= 3
            ? 'simple'
            : overallScore <= 6
                ? 'moderate'
                : overallScore <= 8
 ...
 */
export async function estimateBatchComplexity(tasksDir, options = {}) {
    ...
    const filteredTasks = tasks
        ...
        if (taskStatus !== normalizedStatusFilter)
            return false;
        if (priorityFilter &&
 ...
 */
export function getTasksForLocalModel(estimates, maxComplexity = 6) {
    return estimates.filter((estimate) => estimate.suitableForLocalModel && estimate.overallScore <= maxComplexity);
}
/**
 * Save complexity estimates to file
 */
export async function saveComplexityEstimates(estimates, outputPath) {
    const report = {
