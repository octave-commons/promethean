...
export class WIPLimitEnforcement {
     ...
     */
    async validateWIPLimits(columnName, proposedChange = 0, board) {
     ...
     */
    async interceptStatusTransition(taskId, fromStatus, toStatus, board, options) {
     ...
     */
    async generateCapacitySuggestions(overCapacityColumn, board) {
        const suggestions = [];
        ...
        const underutilizedColumns = targetBoard.columns.filter((col) => {
            if (columnKey(col.name) === columnKey(overCapacityColumn))
                return false;
            if (!col.limit)
                return false; // Skip columns without limits
        ...
        const overCapacityCol = targetBoard.columns.find((col) => columnKey(col.name) === columnKey(overCapacityColumn));
     ...
     */
    async getCapacityMonitor(board) {
        const targetBoard = board || (await this.loadBoard());
        const columns = [];
    ...
    async getColumnTaskCount(columnName, board) {
        if (board) {
        ...
        this.capacityCache.set(cacheKey, {
            ...
            utilization: this.getColumnLimit(columnName)
                ? (count / this.getColumnLimit(columnName)) * 100
                : 0,
            status: 'healthy',
            tasks: column?.tasks || [],
        });
    ...
    getColumnLimit(columnName) {
        if (!this.config)
            return null;
        return this.config.wipLimits[columnName] || null;
    }
    calculateViolationSeverity(projected, limit) {
    ...
    async getTaskTitle(taskId, board) {
        if (board) {
    ...
    calculateSuggestionPriority(_fromColumn, _toColumn, taskCount) {
    ...
    recordViolation(violation) {
        ...
        console.log(`${icon} WIP Limit Violation: ${violation.taskTitle} in ${violation.column}`);
        if (violation.overrideReason) {
            console.log(`   🔓 Override: ${violation.overrideReason} by ${violation.overrideBy}`);
        }
    }
    getRecentViolations(columnName, hours) {
        const since = new Date();
        since.setHours(since.getHours() - hours);
        return this.violationHistory.filter((v) => columnKey(v.column) === columnKey(columnName) && new Date(v.timestamp) >= since);
    }
    generateComplianceRecommendations(violations, violationsByColumn) {
        ...
        if (chronicViolators.length > 0) {
            recommendations.push(`🔍 Consider reviewing WIP limits for columns with chronic violations: ${chronicViolators.map((c) => c.column).join(', ')}`);
        }
        // Check override patterns
        const overrides = violations.filter((v) => v.overrideReason);
        const overrideRate = violations.length > 0 ? (overrides.length / violations.length) * 100 : 0;
        if (overrideRate > 20) {
            recommendations.push('⚠️ High override rate detected. Consider adjusting WIP limits or reviewing workflow processes');
        }
        // Capacity balancing suggestions
        if (violations.some((v) => v.severity === 'critical')) {
            recommendations.push('🚨 Critical violations detected. Immediate capacity balancing required');
        }
        return recommendations;
    }
}
...
//# sourceMappingURL=wip-enforcement.js.map