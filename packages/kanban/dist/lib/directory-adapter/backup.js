 ...
 */
export class TaskBackupManager {
    backupDir;
    retentionDays;
    compressionEnabled;
    hashVerification;
     ...
     */
    async createBackup(filePath, reason, context) {
        try {
    ...
    async listBackups(filePath) {
        try {
            ...
            if (filePath) {
                ...
                const backupsWithMeta = await Promise.all(filteredBackups.map(async (backupPath) => ({
                    path: backupPath,
                    metadata: await this.loadBackupMetadata(backupPath),
                })));
                return backupsWithMeta
                    .sort((a, b) => (b.metadata?.timestamp.getTime() || 0) - (a.metadata?.timestamp.getTime() || 0))
                    .map((item) => item.path);
            }
     ...
     * Verify backup integrity
     */
    async verifyBackupIntegrity(backupPath, expectedHash) {
 ...
 */
export const createBackupManager = (config) => {
