...
const createTestConfig = (baseDir) => ({
    ...TEST_CONFIG,
...
const createTestTask = (uuid, title, overrides = {}) => ({
    id: uuid,
    uuid,
    title,
    status: 'open',
...
test('$0', async (t) => {
    ...
    await adapter.createTaskFile(task);
    // Update the task
    const updates = { title: 'Updated Title', status: 'doing' };
...
test('$0', async (t) => {
    ...
    const tasks = Array.from({ length: 10 }, (_, i) => createTestTask(`concurrent-${i}`, `Concurrent Task ${i}`));
