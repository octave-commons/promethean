...
export class CodeReviewRulesEngine {
    ...
    async performCodeReview(request) {
        ...
        if (this.hasRulesOfType(applicableRules, 'eslint')) {
            analysisPromises.push(this.runESLintAnalysis(changedFiles, applicableRules, violations, suggestions, actionItems));
        }
        if (this.hasRulesOfType(applicableRules, 'typescript')) {
            analysisPromises.push(this.runTypeScriptAnalysis(changedFiles, applicableRules, violations, suggestions, actionItems));
        }
        if (this.hasRulesOfType(applicableRules, 'security')) {
            analysisPromises.push(this.runSecurityAnalysis(changedFiles, applicableRules, violations, suggestions, actionItems));
        }
        if (this.hasRulesOfType(applicableRules, 'ai')) {
            analysisPromises.push(this.runAIAnalysis(task, changedFiles, applicableRules, violations, suggestions, actionItems));
        }
    ...
    async getAffectedPackages(task) {
        const content = task.content || '';
    ...
    async isCacheValid(entry, request) {
    ...
    getApplicableRules(transitionConfig) {
        if (!transitionConfig)
            return [];
        return this.config.rules.filter((rule) => transitionConfig.rules.includes(rule.id) && rule.enabled);
    }
    hasRulesOfType(rules, tool) {
        return rules.some((rule) => rule.tool === tool);
    }
    async runESLintAnalysis(files, _rules, violations, _suggestions, actionItems) {
        try {
    ...
    async runTypeScriptAnalysis(files, _rules, violations, _suggestions, _actionItems) {
        try {
    ...
    async runSecurityAnalysis(files, _rules, violations, _suggestions, actionItems) {
        try {
    ...
    async runAIAnalysis(task, files, _rules, _violations, suggestions, _actionItems) {
        try {
    ...
    calculateScore(metrics, _totalViolations) {
    ...
    generateSummary(metrics, score, context) {
    ...
    isBlocked(score, violations, context) {
        const transitionKey = `${context.fromStatus}->${context.toStatus}`;
        const transitionConfig = this.config.transitions[transitionKey];
        if (!transitionConfig)
            return false;
        ...
        if (transitionConfig.thresholds.maxErrors &&
            violations.filter((v) => v.severity === 'error').length >
                transitionConfig.thresholds.maxErrors) {
            return true;
        }
        if (transitionConfig.thresholds.maxViolations &&
    ...
    meetsTransitionThresholds(result, transitionConfig) {
        if (transitionConfig.thresholds.minScore &&
 ...
 */
export async function createCodeReviewRulesEngine(configPath) {
    let config;
    if (configPath) {
        try {
