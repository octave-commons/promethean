...
export class KanbanCodeReviewIntegration {
    codeReviewEngine;
    constructor(codeReviewEngine) {
        this.codeReviewEngine = codeReviewEngine;
    }
    /**
     * Validate kanban transition with code review
     */
    async validateTransition(from, to, task, board, actor = 'agent') {
        try {
            const request = {
                task,
                fromStatus: from,
     ...
     */
    async getTaskReviewSummary(task) {
     ...
     */
    async updateTaskWithReviewResults(task, reviewResult, transition) {
        ...
        const reviewEntry = {
            ...
            recommendations: reviewResult.suggestions?.map((s) => s.message) || [],
     ...
     */
    requiresCodeReview(from, to, task) {
     ...
     */
    getTransitionReviewConfig(from, to) {
        const transitionKey = `${from}->${to}`;
     ...
     */
    async generateReviewReport(task, reviewResult) {
        const report = [
            `# Code Review Report`,
            ``,
            `**Task:** ${task.title}`,
        ...
        if (reviewResult.actionItems && reviewResult.actionItems.length > 0) {
            ...
            for (const actionItem of reviewResult.actionItems) {
                const priority = actionItem.priority === 'high' ? '🔴' : actionItem.priority === 'medium' ? '🟡' : '🟢';
...
//# sourceMappingURL=kanban-integration.js.map