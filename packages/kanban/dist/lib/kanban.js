...
export const columnKey = (name) => normalizeColumnDisplayName(name)
    .normalize('NFKD')
    .toLowerCase()
    .replace(/[\s-]+/g, '_') // Convert spaces and hyphens to underscores
    .replace(/[^a-z0-9_]+/g, ''); // Remove other special chars
const tokenizeForLabels = (text) => text
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
    .split(/\s+/)
    .filter((token) => token.length > 2 && !STOPWORDS.has(token));
const generateAutoLabels = (title, body, limit = 4) => {
    const tokens = new Map();
    const addTokens = (source, weight) => {
...
const ensureLabelsPresent = (task, body) => {
...
const isFallbackSlug = (slug, uuid) => {
...
const fallbackFileBase = (uuid) => `Task ${uuid.slice(0, 8)}`;
const resolveTaskSlug = (task, baseName) => {
    const sanitizedBase = sanitizeFileNameBase(baseName);
    const explicitSlug = typeof task.slug === 'string' && task.slug.trim().length > 0 ? task.slug.trim() : undefined;
...
const deriveFileBaseFromTask = (task) => {
...
const slugMatchesSourcePath = (task) => {
    if (!task.sourcePath) {
        return false;
    }
    const normalizedSlug = typeof task.slug === 'string' && task.slug.length > 0 ? sanitizeFileNameBase(task.slug) : '';
    if (normalizedSlug.length === 0) {
        return false;
    }
    const normalizedSource = sanitizeFileNameBase(path.basename(task.sourcePath, path.extname(task.sourcePath)));
    return normalizedSource.length > 0 && normalizedSlug === normalizedSource;
};
const ensureUniqueFileBase = (base, used, uuid) => {
    ...
    const truncateForAttempt = (baseStr, attemptNum) => {
...
const mergeColumnsCaseInsensitive = (columns) => {
...
const parseLimit = (header) => {
    // look for "(limit 3)" or "[wip:3]" or "# wip=3"
    const match = header.match(/\((?:wip|limit)\s*[:=]\s*(\d+)\)/i) ??
...
const parseColumnsFromMarkdown = (markdown) => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (typeof doneFlag === 'string' && typeof rawTitle === 'string') {
            ...
            const titleClean = titlePart
                ...
                .trim();
            const title = titleClean.length > 0
                ? titleClean
...
const parseFrontmatter = (text) => {
    const res = parseMarkdownFrontmatter(text);
    return { fm: (res.data ?? {}), body: res.content || '' };
};
...
export const readTasksFolder = async (dir) => {
    ...
    for (const file of files) {
        ...
        if (file.endsWith('.json')) {
            ...
            try {
                ...
                console.error(`Failed to parse frontmatter for ${file}: ${error instanceof Error ? error.message : String(error)}`);
...
export const loadBoard = async (boardPath, tasksDir) => {
    ...
    const cols = Array.from(statusGroups.values()).map(({ name, tasks: ts }) => ({
        name,
        count: ts.length,
...
export const countTasks = (board, column) => {
    if (!column)
        return board.columns.reduce((acc, c) => acc + c.count, 0);
    const targetKey = columnKey(column);
    const c = board.columns.find((col) => columnKey(col.name) === targetKey);
    return c ? c.count : 0;
};
export const getColumn = (board, column) => {
    const targetKey = columnKey(column);
    const c = board.columns.find((col) => columnKey(col.name) === targetKey);
    if (c) {
...
export const getTasksByColumn = (board, column) => getColumn(board, column).tasks;
export const findTaskById = (board, uuid) => {
...
export const findTaskByTitle = (board, title) => {
 ...
 */
export const validateStartingStatus = (column) => {
    ...
    if (!validStartingStatuses.includes(normalizedColumn)) {
        throw new Error(`Invalid starting status: "${column}". Tasks can only be created with starting statuses: ${validStartingStatuses.join(', ')}. ` +
            `Use --status flag to specify a valid starting status when creating tasks.`);
    }
};
const ensureColumn = (board, column) => {
...
const resolveTaskFilePath = async (task, tasksDir) => {
    if (!task)
        return undefined;
    if (!tasksDir)
        return undefined;
    ...
    const tasks = await readTasksFolder(tasksDir).catch(() => []);
    const match = tasks.find((entry) => entry.uuid === task.uuid);
    return match?.sourcePath;
};
const assignStableSlugs = (columns) => {
    const ordered = [];
    let order = 0;
    for (const column of columns) {
...
const serializeBoard = (board) => {
    ...
    for (const col of columns) {
        ...
        for (const task of col.tasks) {
            ...
            const displayTitle = task.title && task.title.trim().length > 0 ? task.title.trim() : linkTarget;
            const wikiLink = displayTitle === linkTarget ? `[[${linkTarget}]]` : `[[${linkTarget}|${displayTitle}]]`;
            const labelSegment = (task.labels ?? []).length > 0
                ? (task.labels ?? []).map((label) => `#${label}`).join(' ')
                : '';
            const priorityValue = typeof task.priority === 'number' || typeof task.priority === 'string'
            ...
            let epicSegment = '';
            if (task.type === 'epic') {
                const subtasks = getEpicSubtasks(board, task);
...
const resolveKanbanFooter = async (boardPath) => {
    try {
...
const maybeRefreshIndex = async (tasksDir) => {
    try {
        const { config } = await loadKanbanConfig();
...
export const writeBoard = async (boardPath, board) => {
...
export const updateStatus = async (board, uuid, newStatus, boardPath, tasksDir, transitionRulesEngine, correctionReason, eventLogManager, actor = 'system') => {
    let found;
    for (const col of board.columns) {
    ...
    if (transitionRulesEngine) {
        try {
            ...
            const transitionResult = await transitionRulesEngine.validateTransition(currentStatus, normalizedStatus, enrichedTask, board);
    ...
    try {
        ...
        const wipValidation = await wipEnforcement.interceptStatusTransition(uuid, currentStatus, normalizedStatus, board, {
        ...
        if (wipValidation.blocked) {
            ...
            const suggestionMessage = wipValidation.suggestions && wipValidation.suggestions.length > 0
        ...
        if (wipValidation.violation && wipValidation.violation.severity === 'warning') {
            console.warn(`⚠️  WIP Limit Warning: ${wipValidation.violation.severity} violation in ${newStatus}`);
        ...
        if (target.limit && target.count >= target.limit) {
            throw new Error(`WIP limit violation: Cannot move task to '${target.name}' - column has ${target.count} tasks (limit: ${target.limit})`);
        }
    }
            ...
            if (taskFilePath) {
                ...
                try {
                    ...
                    const updatedFrontmatter = gitTracker.updateTaskCommitTracking({
                        ...existingFrontmatter,
                        ...finalTask,
                    }, taskFilePath, uuid, 'status_change');
...
export const moveTask = async (board, uuid, delta, boardPath) => {
    for (const col of board.columns) {
        const idx = col.tasks.findIndex((t) => t.uuid === uuid);
        if (idx >= 0) {
...
const formatLabels = (labels) => labels && labels.length > 0
    ...
    : '[]';
const fallbackTaskFromRaw = (filePath, raw) => {
    if (!raw.startsWith('---')) {
        return null;
    }
    let cursor = 3;
...
const normalizeFrontmatterForParsing = (raw) => raw;
const toFrontmatter = (t) => {
    ...
    lines.push(`status: ${quoteYamlString(t.status)}`, `priority: ${quoteYamlString(typeof t.priority === 'number' ? String(t.priority) : t.priority)}`, `labels: ${formatLabels(t.labels)}`, `created_at: ${quoteYamlString(t.created_at)}`, 'estimates:', `  complexity: ${formatScalar(est.complexity)}`, `  scale: ${formatScalar(est.scale)}`, `  time_to_completion: ${formatScalar(est.time_to_completion)}`);
...
export const pullFromTasks = async (board, tasksDir, boardPath) => {
    ...
    for (const t of tasks) {
        ...
        if (!loc) {
            ...
            if (currentKey !== statusKey) {
                ...
                moved++;
                console.log(`📝 Pulled status change for task "${normalizedTask.title}": ${loc.col.name} → ${normalizedStatus}`);
            }
        }
    }
    for (const column of board.columns) {
        ...
        for (const task of column.tasks) {
            ...
            const slugCandidate = typeof task.slug === 'string' && task.slug.length > 0
...
export const pushToTasks = async (board, tasksDir) => {
    let added = 0, moved = 0, statusUpdated = 0;
    ...
    for (const col of board.columns) {
        for (const task of col.tasks) {
            ...
            let finalTask = { ...task };
            const existingTaskForSlug = Array.from(existingByUuid.values()).find((t) => t.slug === baseName || ensureTaskFileBase(t) === baseName);
            if (existingTaskForSlug && existingTaskForSlug.uuid !== task.uuid) {
                ...
                console.log(`🔧 UUID mismatch detected: board has ${task.uuid} but file has ${existingTaskForSlug.uuid}. Using file UUID.`);
            ...
            if (existingTask && normalizedFileStatus && normalizedBoardStatus !== normalizedFileStatus) {
                statusUpdated++;
                console.log(`📝 Detected manual status change for task "${finalTask.title}": ${normalizedFileStatus} → ${normalizedBoardStatus}`);
            }
            ...
            if (existingFiles.has(conflictingFileName)) {
                ...
                if (conflictingUuid && conflictingUuid !== finalTask.uuid) {
                    ...
                    console.log(`🔄 Duplicate task detected: UUID ${finalTask.uuid} conflicts with existing UUID ${conflictingUuid}. Using existing file "${conflictingFileName}"`);
                    ...
                    console.log(`👻 Ghost file detected: "${conflictingFileName}" has no frontmatter. Using "${targetBase}.md"`);
                }
            ...
            try {
                ...
                const taskWithCommitTracking = gitTracker.updateTaskCommitTracking({
                    ...
                    content: finalContent,
                    created_at: preservedCreatedAt,
                }, finalTargetPath, finalTask.uuid, operation);
...
const persistBoardAndTasks = async (board, boardPath, tasksDir) => {
    if (boardPath) {
        await writeBoard(boardPath, board);
    }
...
const formatSectionBlock = (heading, items) => {
    const lines = items.length > 0 ? items.map((item) => `- ${item}`) : ['Nothing'];
    return `${heading}\n\n${lines.join('\n')}\n\n`;
};
const setSectionItems = (content, heading, items) => {
...
const ensureSectionExists = (content, heading) => {
...
const parseSectionItems = (content, heading) => {
    const pattern = new RegExp(`^${escapeRegExp(heading)}\\s*\\n([\\s\\S]*?)(?=^##\\s+|$)`, 'm');
...
const mergeSectionItems = (content, heading, additions) => {
...
const wikiLinkForTask = (task) => {
    const base = ensureTaskFileBase(task);
    const display = task.title && task.title.trim().length > 0 ? task.title.trim() : base;
    return `[[${base}|${display}]]`;
};
const ensureTaskContent = (task, fallback) => {
    const baseContent = task.content && task.content.length > 0 ? task.content : (fallback?.content ?? '');
...
export const createTask = async (board, column, input, tasksDir, boardPath) => {
    ...
    const existingTaskInColumn = existingTasks.find((task) => task.title.trim().toLowerCase() === normalizedTitle &&
    ...
    const boardTaskInColumn = targetColumn.tasks.find((task) => task.title.trim().toLowerCase() === normalizedTitle);
    ...
    board.columns.forEach((col) => col.tasks.forEach((task, index) => boardIndex.set(task.uuid, { column: col, index, task })));
    ...
    let contentFromTemplate = typeof templateContent === 'string'
    ...
    const updateLinkedTask = async (id, heading) => {
        const entry = boardIndex.get(id);
        if (entry) {
            const fallback = existingById.get(id);
        ...
        await fs.writeFile(existing.sourcePath, toFrontmatter({ ...nextTask, status: nextTask.status ?? 'Todo' }), 'utf8');
...
export const archiveTask = async (board, uuid, tasksDir, boardPath, options) => {
    ...
    const moved = { ...removed, status: target.name };
    target.tasks = [...target.tasks, moved];
    target.count = target.tasks.length;
    await persistBoardAndTasks(board, boardPath, tasksDir);
    return moved;
};
export const deleteTask = async (board, uuid, tasksDir, boardPath) => {
    const located = locateTask(board, uuid);
    if (!located)
        return false;
...
export const updateTaskDescription = async (board, uuid, content, tasksDir, boardPath) => {
    const located = locateTask(board, uuid);
    if (!located)
        return undefined;
...
export const renameTask = async (board, uuid, newTitle, tasksDir, boardPath) => {
    const located = locateTask(board, uuid);
    if (!located)
        return undefined;
...
export const syncBoardAndTasks = async (board, tasksDir, boardPath) => {
    ...
    for (const [id, fileTask] of tasksById) {
        ...
        if (fileTitle !== boardTitle) {
            conflicting.push(id);
            console.log(`⚠️  Title conflict for task ${id}: board="${boardTitle}" vs file="${fileTitle}"`);
        }
        // Check for status conflicts
        if (fileStatus !== boardStatus) {
            conflicting.push(id);
            console.log(`⚠️  Status conflict for task ${id}: board="${boardStatus}" vs file="${fileStatus}"`);
        }
    }
...
export const regenerateBoard = async (tasksDir, boardPath) => {
    ...
    const configuredColumns = Array.from(config.statusValues).map((statusValue) => {
    ...
    const additionalColumns = Array.from(statusGroups.entries())
...
export const mergeTasks = async (board, sourceUuids, targetUuid, tasksDir, boardPath, options) => {
    ...
    targetColumn.tasks = targetColumn.tasks.map((task) => task.uuid === targetUuid ? updatedTask : task);
...
const tokenize = (s) => (s || '')
    ...
    .split(/\s+/)
    .filter(Boolean);
export const indexForSearch = async (_tasksDir, options) => {
...
export const searchTasks = async (board, term) => {
    const needle = term.trim().toLowerCase();
    const all = board.columns.flatMap((c) => c.tasks.map((t) => ({ ...t, status: c.name })));
    const exact = all.filter((t) => t.title.toLowerCase().includes(needle) || (t.content ?? '').toLowerCase().includes(needle));
    const needToks = new Set(tokenize(term));
    const score = (t) => {
        const bag = new Set([...tokenize(t.title), ...tokenize(t.content ?? ''), ...(t.labels ?? [])]);
        let s = 0;
        for (const tok of needToks)
            if (bag.has(tok))
                s++;
        return s;
    };
 ...
 */
export const generateBoardByTags = async (tasksDir, boardPath, tags) => {
    // Load configuration to get all valid statuses
    const { config } = await loadKanbanConfig();
    ...
    const columns = Array.from(config.statusValues).map((statusValue) => {
        const displayName = normalizeColumnDisplayName(statusValue);
        const key = columnKey(statusValue);
        const existingGroup = statusGroups.get(key);
        return {
...
export const analyzeTask = async (board, uuid, analysisType, _tasksDir, _boardPath, context, options) => {
    try {
        const located = locateTask(board, uuid);
        if (!located)
            return undefined;
...
export const rewriteTask = async (board, uuid, rewriteType, _tasksDir, _boardPath, options) => {
    try {
        const located = locateTask(board, uuid);
        if (!located)
            return undefined;
...
export const breakdownTask = async (board, uuid, breakdownType, _tasksDir, _boardPath, options) => {
    try {
        const located = locateTask(board, uuid);
        if (!located)
            return undefined;
