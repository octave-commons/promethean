...
export class TaskGitTracker {
     ...
     */
    createTaskCommitMessage(taskUuid, operation, details) {
        const operationMap = {
            create: 'Create task',
            update: 'Update task',
     ...
     */
    updateTaskCommitTracking(frontmatter, taskFilePath, taskUuid, operation, details) {
     ...
     */
    validateTaskCommitTracking(frontmatter) {
        const issues = [];
     ...
     */
    analyzeTaskStatus(frontmatter, taskFilePath) {
        const issues = [];
        ...
        if (taskFilePath) {
            try {
                ...
                if (frontmatter.uuid) {
                    try {
                        const commitLog = execSync(`git log --oneline --grep="${frontmatter.uuid}" -n 5 --since="3 months ago"`, {
                            cwd: this.repoRoot,
                            encoding: 'utf8',
                        }).trim();
 ...
 */
export function updateTaskCommitTracking(frontmatter, taskFilePath, taskUuid, operation, details) {
    return defaultTaskGitTracker.updateTaskCommitTracking(frontmatter, taskFilePath, taskUuid, operation, details);
}
/**
 * Convenience function to validate task commit tracking
 */
export function validateTaskCommitTracking(frontmatter) {
    return defaultTaskGitTracker.validateTaskCommitTracking(frontmatter);
}
//# sourceMappingURL=task-git-tracker.js.map