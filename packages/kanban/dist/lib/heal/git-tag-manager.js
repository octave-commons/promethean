...
export class GitTagManager {
     ...
     */
    async createHealTag(reason, startSha, metadata) {
    ...
    async loadScarHistory() {
        try {
            ...
            return scars
                .filter((scar) => scar && typeof scar === 'object')
                .map((scar) => ({
                ...scar,
                timestamp: new Date(scar.timestamp),
            }))
                .filter((scar) => !isNaN(scar.timestamp.getTime()));
        }
        catch (error) {
     ...
     */
    async getCommitsBetweenTags(startTag, endTag) {
        try {
            await this.validateGitRepository();
            const { execSync } = await import('node:child_process');
     ...
     */
    async createAnnotatedTag(tag, commitSha, message) {
        const { execSync } = await import('node:child_process');
     ...
     */
    async createLightweightTag(tag, commitSha) {
        const { execSync } = await import('node:child_process');
     ...
     */
    generateTagMessage(reason, metadata) {
 ...
 */
export function createGitTagManager(repoRoot, options) {
    return new GitTagManager(repoRoot, options);
}
/**
 * Default options for git tag management
 */
export const DEFAULT_GIT_TAG_MANAGER_OPTIONS = {
    tagPrefix: 'heal',
...
//# sourceMappingURL=git-tag-manager.js.map