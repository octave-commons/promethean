...
export class ScarContextBuilder {
    boardPath;
    tasksDir;
    constructor(boardPath, tasksDir, _eventLogManager) {
     ...
     */
    async buildContext(reason, options = {}) {
        ...
        try {
            ...
            context.eventLog.push(createEventLogEntry('start-context-building', {
                reason,
            ...
            if (options.includePerformanceMetrics !== false) {
                ...
                context.eventLog.push(createEventLogEntry('system-metrics-analyzed', {
            ...
            if (options.includeTaskAnalysis) {
                ...
                context.eventLog.push(createEventLogEntry('task-analysis-completed', {
            ...
            context.eventLog.push(createEventLogEntry('git-analysis-completed', {
            ...
            if (options.searchTerms && options.searchTerms.length > 0) {
                ...
                context.eventLog.push(createEventLogEntry('task-search-completed', {
            ...
            context.eventLog.push(createEventLogEntry('previous-scars-loaded', {
                scarCount: context.previousScars.length,
            }, 'info'));
            // Validate context integrity
            const validation = validateScarContextIntegrity(context);
            if (!validation.isValid) {
                context.eventLog.push(createEventLogEntry('context-validation-failed', {
            ...
            context.eventLog.push(createEventLogEntry('context-building-completed', {
            ...
            context.eventLog.push(createEventLogEntry('context-building-failed', {
                error: errorMessage,
     ...
     */
    async analyzeSystemMetrics(board, tasks) {
     ...
     */
    async analyzeTasks(board, tasks, options) {
        const criticalTasks = [];
        const stuckTasks = [];
        const qualityIssues = [];
     ...
     */
    async analyzeGitHistory(maxDepth = 50) {
        const analysis = {
            headCommit: null,
            recentCommits: [],
            repoStatus: {
     ...
     * Search for relevant tasks based on search terms
     */
    async searchRelevantTasks(board, searchTerms, maxResults = 20) {
        const results = [];
     ...
     */
    createSearchSnippet(task, searchTerms) {
        const snippetParts = [];
        // Add title if it matches
        const matchingTerms = searchTerms.filter((term) => task.title.toLowerCase().includes(term.toLowerCase()));
        if (matchingTerms.length > 0) {
            snippetParts.push(`Title: ${task.title}`);
        }
        // Add matching labels
        const matchingLabels = task.labels?.filter((label) => searchTerms.some((term) => label.toLowerCase().includes(term.toLowerCase())));
        ...
        if (task.content) {
            const matchingContent = searchTerms.filter((term) => task.content.toLowerCase().includes(term.toLowerCase()));
     ...
     */
    async loadPreviousScars(_maxScars = 10) {
     ...
     */
    async addLLMOperation(context, operation, input, output, tokensUsed) {
        const llmOp = {
            id: randomUUID(),
            operation,
            input,
        ...
        context.eventLog.push(createEventLogEntry('llm-operation-completed', {
     ...
     */
    async updateProgress(context, operation, details, severity = 'info') {
    ...
    async finalizeContext(context) {
        context.eventLog.push(createEventLogEntry('context-finalized', {
            totalEvents: context.eventLog.length,
 ...
 */
export function createScarContextBuilder(boardPath, tasksDir, eventLogManager) {
    return new ScarContextBuilder(boardPath, tasksDir, eventLogManager);
}
/**
 * Default options for scar context building
 */
export const DEFAULT_SCAR_CONTEXT_OPTIONS = {
    maxPreviousScars: 10,
    maxSearchResults: 20,
    maxGitHistory: 50,
...
//# sourceMappingURL=scar-context-builder.js.map