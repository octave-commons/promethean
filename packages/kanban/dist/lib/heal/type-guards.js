 ...
 */
export function isEventLogEntry(value) {
    return (value &&
        typeof value === 'object' &&
 ...
 */
export function isScarRecord(value) {
    return (value &&
        typeof value === 'object' &&
 ...
 */
export function isLLMOperation(value) {
    return (value &&
        typeof value === 'object' &&
 ...
 */
export function isHealingStatus(value) {
 ...
 */
export function isHealingResult(value) {
    return (value &&
        typeof value === 'object' &&
 ...
 */
export function isScarContext(value) {
    return (value &&
        typeof value === 'object' &&
 ...
 */
export function validateScarContextIntegrity(context) {
    const errors = [];
    const warnings = [];
    ...
    const invalidShas = context.previousScars.filter((scar) => !/^[a-f0-9]{40}$/i.test(scar.start) || !/^[a-f0-9]{40}$/i.test(scar.end)).length;
    if (invalidShas > 0) {
        errors.push(`${invalidShas} scar records have invalid git SHAs`);
    }
    // Check for valid task IDs in search results
    const invalidTaskIds = context.searchResults.filter((result) => !/^[a-f0-9-]{36}$/.test(result.taskId)).length;
    if (invalidTaskIds > 0) {
 ...
 */
export function createEventLogEntry(operation, details, severity = 'info') {
    return {
        timestamp: new Date(),
 ...
 */
export function createScarRecord(start, end, tag, story) {
    return {
        start: String(start),
        end: String(end),
        tag: String(tag),
