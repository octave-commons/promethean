...
export function calculateQualityScore(details) {
    ...
    const score = passScore * weights.passRate +
 ...
 */
export function analyzeTestQuality(testContent, testResults) {
 ...
 */
function analyzeAssertionQuality(code) {
 ...
 */
function analyzeEdgeCaseCoverage(code) {
 ...
 */
function analyzeMockQuality(code) {
    ...
    goodMockPatterns.forEach((pattern) => {
        const matches = code.match(pattern);
        if (matches)
            goodMockCount += matches.length;
    });
    badMockPatterns.forEach((pattern) => {
        const matches = code.match(pattern);
        if (matches)
            badMockCount += matches.length;
    });
    const totalMocks = goodMockCount + badMockCount;
    if (totalMocks === 0)
        return 80; // Good score if no mocks needed
 ...
 */
function detectTestDuplication(code) {
    ...
    if (totalLines === 0)
        return 0;
    const duplicationPercentage = (duplicatedLines.reduce((sum, count) => sum + (count - 1), 0) / totalLines) * 100;
