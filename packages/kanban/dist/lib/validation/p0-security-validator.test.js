...
test.beforeEach(async (t) => {
    ...
    t.context.tasksDir = tasksDir;
    t.context.validator = createP0SecurityValidator({
        repoRoot: tempDir,
        tasksDir,
...
const createMockP0Task = (overrides = {}) => ({
...
const createMockNonP0Task = (overrides = {}) => ({
...
test('should require implementation plan for todo to in_progress transition', async (t) => {
    ...
    t.true(result.errors.includes('P0 security tasks require an implementation plan before starting work'));
    t.false(result.requirements.hasImplementationPlan);
});
test('should pass validation with implementation plan', async (t) => {
    const { validator, tasksDir } = t.context;
    const p0Task = createMockP0Task();
...
test('should require code changes for todo to in_progress transition', async (t) => {
    const { tasksDir, tempDir } = t.context;
    const p0Task = createMockP0Task();
    const taskFilePath = join(tasksDir, `${p0Task.uuid}.md`);
    await writeFile(taskFilePath, `
---
...
Detailed implementation approach here.
    ...
    const result = await validator.validateStatusTransition(p0Task, 'todo', 'in_progress');
    t.false(result.valid);
    t.true(result.errors.includes('P0 security tasks require committed code changes to move to in-progress'));
...
Implementation completed with proper security measures.
    ...
    t.false(result.valid);
    t.true(result.errors.includes('P0 security tasks require completed security review before testing'));
...
Security review completed and approved.
    ...
    const result = await validator.validateStatusTransition(p0Task, 'in_progress', 'testing');
    t.false(result.valid);
    t.true(result.errors.includes('P0 security tasks require defined test coverage plan before testing'));
...
Comprehensive test coverage defined and implemented.
    ...
    t.false(result.valid);
    t.true(result.errors.includes('P0 security tasks require updated documentation before completion'));
...
Basic task description without detailed plans.
    ...
    t.true(result.warnings.some((w) => w.includes('implementation plan')));
});
...
Detailed implementation approach here.
    ...
    for (const testCase of testCases) {
        ...
        t.true(result.errors.some((e) => e.includes('code changes')));
    }
});
...
//# sourceMappingURL=p0-security-validator.test.js.map