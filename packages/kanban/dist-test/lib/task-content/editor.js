...
import { parseTaskContent, findSection, validateTaskContent, createBackup, updateTimestamp } from './parser.js';
/**
 * Update entire task body content
 */
export async function updateTaskBody(cache, request) {
 ...
 */
export async function updateTaskSection(cache, request) {
 ...
 */
export async function getTaskSections(cache, taskUuid) {
    const indexedTask = await cache.readTask(taskUuid);
    if (!indexedTask) {
        throw new Error(`Task not found: ${taskUuid}`);
    }
 ...
 */
export async function getTaskSection(cache, taskUuid, sectionHeader) {
