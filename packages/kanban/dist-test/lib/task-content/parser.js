 ...
 */
export function findSection(sections, header) {
...
export function validateTaskContent(content) {
    ...
    try {
        ...
        const hasDescription = sections.some(s => s.header.toLowerCase().includes('description') ||
            s.header.toLowerCase().includes('overview'));
        if (!hasDescription) {
            suggestions.push('Add a description section for better task clarity');
        }
        // Check content quality
        const totalWords = sections.reduce((sum, section) => sum + section.content.split(/\s+/).length, 0);
        if (totalWords < 20) {
 ...
 */
export function analyzeTaskContent(content) {
    const { frontmatter, sections } = parseTaskContent(content);
    // Basic metrics
    ...
    const hasDescription = sections.some(s => s.header.toLowerCase().includes('description') ||
        s.header.toLowerCase().includes('overview'));
    const hasGoals = sections.some(s => s.header.toLowerCase().includes('goal') ||
        s.header.toLowerCase().includes('objective'));
    const hasAcceptanceCriteria = sections.some(s => s.header.toLowerCase().includes('acceptance') ||
        s.header.toLowerCase().includes('criteria'));
    const qualityScore = ((hasDescription ? 0.3 : 0) +
        (hasGoals ? 0.3 : 0) +
 ...
 */
export function updateTimestamp(frontmatter) {
    return {
        ...frontmatter,
