...
const columnKey = (name) => normalizeColumnDisplayName(name)
    .normalize('NFKD')
    .toLowerCase()
    .replace(/[^a-z0-9_]+/g, '');
...
const writeBoard = async (boardPath, board) => {
...
export const updateStatus = async (board, uuid, newStatus, boardPath, tasksDir, transitionRulesEngine, correctionReason, eventLogManager, actor = 'system') => {
    ...
    if (transitionRulesEngine) {
        try {
            const transitionResult = await transitionRulesEngine.validateTransition(currentStatus, normalizedStatus, found, board);
    ...
    if (eventLogManager) {
        try {
            await eventLogManager.logTransition(uuid, currentStatus, normalizedStatus, actor, correctionReason || `Status updated from ${currentStatus} to ${normalizedStatus}`, {
                boardPath,
...
const toFrontmatter = (t) => {
    ...
    lines.push(`status: ${quoteYamlString(t.status)}`, `priority: ${quoteYamlString(typeof t.priority === 'number' ? String(t.priority) : t.priority)}`, `labels: ${formatLabels(t.labels)}`, `created_at: ${quoteYamlString(t.created_at ?? NOW_ISO())}`, 'estimates:', `  complexity: ${formatScalar(est.complexity)}`, `  scale: ${formatScalar(est.scale)}`, `  time_to_completion: ${formatScalar(est.time_to_completion)}`);
