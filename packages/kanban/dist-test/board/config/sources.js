...
import { ARG_KEYS, CONFIG_SEARCH_PATHS, DEFAULT_CONFIG_BASENAME, ENV_KEYS, MARKERS, arrayHasKey, parseList, resolveWithBase, } from './shared.js';
const pathExists = (target) => access(target).then(() => true, () => false);
const findMarkers = async (dir) => {
...
const selectPreferred = (current, found, previous) => {
...
const selectFallback = (current, found, fallback) => typeof fallback === 'undefined' && found.includes('package.json') ? current : fallback;
const detectRepoRoot = (start, preferred, fallback) => {
...
const envKeys = Object.keys(ENV_KEYS);
...
const freezeStrings = (values) => {
    ...
    const copy = Object.freeze([...values]);
    return copy;
};
const mergeOverride = (current, key, value) => {
    const next = Object.freeze({ ...current, [key]: value });
    return next;
};
const appendRest = (token, parsed) => ({
    values: parsed.values,
    rest: freezeStrings([token, ...parsed.rest]),
});
const mergeArgValue = (key, value, parsed) => ({
    values: mergeOverride(parsed.values, key, value),
    rest: parsed.rest,
});
const parseInlineValue = (key, inline, tail) => {
...
const parseValueFromTail = (key, tail) => {
...
export const parseEnvConfig = (env) => envKeys.reduce((acc, key) => {
...
export function parseArgv(argv) {
    if (argv.length === 0) {
        return { values: Object.freeze({}), rest: EMPTY_STRINGS };
    }
...
const searchConfigUp = (current, stop) => {
...
const searchConfigPaths = async (repoRoot) => {
...
export const findConfigPath = (repo, explicitPath, cwd) => {
...
export const resolveRepo = (argValues, envValues, cwd) => {
    const argRepo = argValues.repo;
    if (typeof argRepo === 'string') {
        return Promise.resolve(resolveWithBase(cwd, argRepo));
    }
    const envRepo = envValues.repo;
