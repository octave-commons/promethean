...
test('FAILING REPRODUCTION: Descriptive filenames with dots get duplicated with " 2.md" suffix', async (t) => {
    ...
    // Create first task
    await createTask(board, 'todo', { title: fullTitle, content: 'First task content' }, tasksDir, boardPath);
    // Simulate the operation that triggers duplication (e.g., board regeneration)
    await regenerateBoard(tasksDir, boardPath);
    // Check files - this should FAIL because the bug creates duplicates
    const files = await readdir(tasksDir);
    const descriptiveFiles = files.filter((file) => file.includes(descriptiveTitle) && file.endsWith('.md'));
    // This assertion will FAIL due to the bug - it expects 1 file but finds 2
    t.is(descriptiveFiles.length, 1, `BUG REPRODUCED: Expected 1 file for "${descriptiveTitle}" but found ${descriptiveFiles.length}: ${descriptiveFiles.join(', ')}`);
    // Check for the specific " 2.md" duplication pattern
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `BUG REPRODUCED: Found files with " 2.md" suffix: ${duplicateFiles.join(', ')}`);
});
test('FAILING REPRODUCTION: Pure numeric filenames do not get duplicated', async (t) => {
    ...
    await createTask(board, 'todo', { title: numericTitle, content: 'Numeric task content' }, tasksDir, boardPath);
    ...
    t.is(numericFiles.length, 1, `Numeric filenames should work: Expected 1 file for "${numericTitle}" but found ${numericFiles.length}: ${numericFiles.join(', ')}`);
    // Should not have any duplicates
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `Numeric filenames should not create duplicates: ${duplicateFiles.join(', ')}`);
});
test('FAILING REPRODUCTION: Multiple duplication creates " 2 2.md" pattern', async (t) => {
    ...
    await createTask(board, 'todo', { title: problematicTitle, content: 'First task content' }, tasksDir, boardPath);
    ...
    const files = await readdir(tasksDir);
    const problematicFiles = files.filter((file) => file.includes(problematicTitle) && file.endsWith('.md'));
    // This assertion will FAIL - it expects 1 but finds multiple including " 2 2.md"
    t.is(problematicFiles.length, 1, `BUG REPRODUCED: Expected 1 file for "${problematicTitle}" but found ${problematicFiles.length}: ${problematicFiles.join(', ')}`);
    // Check for the specific " 2 2.md" pattern
    const doubleDuplicateFiles = files.filter((file) => file.includes(' 2 2.md'));
    t.true(doubleDuplicateFiles.length === 0, `BUG REPRODUCED: Found files with " 2 2.md" pattern: ${doubleDuplicateFiles.join(', ')}`);
});
test('FAILING REPRODUCTION: pushToTasks operation triggers descriptive filename duplication', async (t) => {
    ...
    const files = await readdir(tasksDir);
    const descriptiveFiles = files.filter((file) => file.includes(descriptiveTitle) && file.endsWith('.md'));
    t.is(descriptiveFiles.length, 1, `BUG REPRODUCED in pushToTasks: Expected 1 file for "${descriptiveTitle}" but found ${descriptiveFiles.length}: ${descriptiveFiles.join(', ')}`);
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `BUG REPRODUCED in pushToTasks: Found files with " 2.md" suffix: ${duplicateFiles.join(', ')}`);
});
test('FAILING REPRODUCTION: Complex descriptive names with special characters get duplicated', async (t) => {
    ...
    for (const title of complexTitles) {
        await createTask(board, 'todo', { title, content: `Content for ${title}` }, tasksDir, boardPath);
    }
    ...
    const files = await readdir(tasksDir);
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `BUG REPRODUCED: Complex descriptive names created duplicates: ${duplicateFiles.join(', ')}`);
    // Each title should have exactly one file
    for (const title of complexTitles) {
        const titleFiles = files.filter((file) => file.includes(title.replace(/^[\d.]+\./, '')) && file.endsWith('.md'));
        t.is(titleFiles.length, 1, `BUG REPRODUCED: Title "${title}" has ${titleFiles.length} files instead of 1: ${titleFiles.join(', ')}`);
    }
});
test('FAILING REPRODUCTION: Mixed filename types show inconsistent behavior', async (t) => {
    ...
    const numericTitle = '20251011235145';
    await createTask(board, 'todo', { title: descriptiveTitle, content: 'Descriptive content' }, tasksDir, boardPath);
    await createTask(board, 'todo', { title: numericTitle, content: 'Numeric content' }, tasksDir, boardPath);
    // Trigger the duplication
    await regenerateBoard(tasksDir, boardPath);
    const files = await readdir(tasksDir);
    // Numeric should work fine (1 file)
    const numericFiles = files.filter((file) => file.includes(numericTitle));
    t.is(numericFiles.length, 1, `Numeric files should work: found ${numericFiles.length} for ${numericTitle}`);
    ...
    t.is(descriptiveFiles.length, 1, `BUG REPRODUCED: Descriptive files duplicated: found ${descriptiveFiles.length} for implement-lmdb-cache-package: ${descriptiveFiles.join(', ')}`);
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `BUG REPRODUCED: Found ${duplicateFiles.length} files with " 2.md" suffix: ${duplicateFiles.join(', ')}`);
});
test('FAILING REPRODUCTION: Board load and save cycle creates duplicates', async (t) => {
    ...
    await createTask(board, 'todo', { title: descriptiveTitle, content: 'Test content' }, tasksDir, boardPath);
    ...
    t.is(taskFiles.length, 1, `BUG REPRODUCED: Load/save cycle created ${taskFiles.length} files for "${descriptiveTitle}": ${taskFiles.join(', ')}`);
    const duplicateFiles = files.filter((file) => file.includes(' 2.md'));
    t.true(duplicateFiles.length === 0, `BUG REPRODUCED: Load/save cycle created duplicates: ${duplicateFiles.join(', ')}`);
});
