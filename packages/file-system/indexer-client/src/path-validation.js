 ...
 */
export function validateAndNormalizePath(path, allowedBasePaths = []) {
  if (!path || typeof path !== 'string') {
    throw new Error('Path must be a non-empty string');
  }

  // Remove any null bytes and check if they existed
  if (path.includes('\0')) {
  ...
  const suspiciousPatterns = [
    ...
    /(^|\/)windows\//i, // Windows system directories (at start or after slash)
    /(^|\/)program files\//i, // Windows program files (at start or after slash)
    /(^|\/)users\//i, // Windows users directory (at start or after slash)
  ];
 ...
 */
export function validateFilePatterns(patterns, allowedBasePaths = []) {
 ...
 */
export function createPathValidator(allowedBasePaths) {
  return {
    validatePath: (path) => validateAndNormalizePath(path, allowedBasePaths),
    validatePatterns: (patterns) => validateFilePatterns(patterns, allowedBasePaths),
  };
}
