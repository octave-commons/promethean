...
export async function showStatus(configPath: string, pipelineName: string) {
  ...
  for (const s of pipeline.steps) {
    ...
    const curHash = await fingerprintFromGlobs(s.inputs ?? [], cwd, mode as any);
    ...
    const note = cached ? (haveOutputs ? "cache clean" : "fingerprint matches but outputs missing") : "needs run";
    console.log(`| ${s.id} | ${cached ? 'yes' : 'no'} | ${haveOutputs ? 'yes' : 'no'} | ${note} |`);
  }
}
