    ...
    await withPage.exec(t as any, { baseUrl: () => baseUrl! }, async (_t: any, { pageGoto, page }: any) => {
      ...
      const clickDir = async (name: string) => {
        await page.evaluate(async (label: string) => {
          ...
          const target = dirs.find((d) => (d.querySelector(".dir-name") as HTMLElement)?.textContent === label) as HTMLElement;
      ...
      const leafVisible = await page.evaluate(() => {
        const el = document.querySelector("file-tree");
        const root = el.shadowRoot as ShadowRoot;
        const file = Array.from(root.querySelectorAll(".file"))
          .find((n) => (n as HTMLElement).textContent?.trim() === "deep.md");
        return !!file;
      });
