class DocOpsStep extends HTMLElement {
  connectedCallback() {
    ...
    const fieldsAttr = (this.getAttribute('fields') || '').split(',').map(s=>s.trim());
    ...
    if (fieldsAttr.includes('collection')) addInput('collection', 'Collection', 'text', topColl());
    if (fieldsAttr.includes('embedModel')) addInput('embedModel', 'Embed Model', 'text', 'nomic-embed-text:latest');
    if (fieldsAttr.includes('genModel')) addInput('genModel', 'Gen Model', 'text', 'qwen3:4b');
    if (fieldsAttr.includes('k')) addInput('k', 'K', 'number', '16');
    if (fieldsAttr.includes('docT')) addInput('docT', 'Doc Threshold', 'number', '0.78');
    if (fieldsAttr.includes('refT')) addInput('refT', 'Ref Threshold', 'number', '0.85');
    if (fieldsAttr.includes('anchorStyle')) addInput('anchorStyle', 'Anchor Style', 'text', 'block');
  ...
  run() {
    ...
    const topDir = (document.getElementById('dir') || { value: '' }).value || '';
