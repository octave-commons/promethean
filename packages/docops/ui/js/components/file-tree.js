...
class FileTree extends HTMLElement {
  ...
  async refresh() {
      ...
      for (const n of nodes) {
        ...
        if (n.type === "dir") {
          ...
          span.addEventListener("click", async () => {
            const cbs = this.shadowRoot.querySelectorAll(
              "input[type=checkbox]",
            );
            cbs.forEach((el) => {
    ...
    this.updateSelection();
    // toolbar actions
    this.shadowRoot.getElementById("selAll").onclick = () => {
      this.shadowRoot
        .querySelectorAll("input[type=checkbox]")
        .forEach((cb) => (cb.checked = true));
      this.updateSelection();
    };
    this.shadowRoot.getElementById("selNone").onclick = () => {
      this.shadowRoot
        .querySelectorAll("input[type=checkbox]")
        .forEach((cb) => (cb.checked = false));
      this.updateSelection();
    };
    this.shadowRoot.getElementById("selBySize").onclick = () => {
      const minKB =
        Number(this.shadowRoot.getElementById("minKB").value || "0") || 0;
