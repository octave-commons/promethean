...
let DEBUG = false;
const dbg = (...xs: any[]) => { if (DEBUG) console.log('[04-relations]', ...xs); };
...
export async function runRelations(
  ...
  db: DBs,
  onProgress?: (p: { step: 'relations'; done: number; total: number; message?: string }) => void
) {
...
const refsForDoc = (
  ...
  Promise.all(
    ...
    .then((arrs) => {
      ...
      if (DEBUG) dbg('refsForDoc', uuid, 'chunks', chunks.length, 'refs', out.length);
      return out;
    });
...
const processDoc = (
  ...
  return fs
    ...
    .then((raw) => {
      ...
      return refsForDoc(uuid, chunks, REF_THRESHOLD, allowed).then((newRefs) => {
        // Rebuild sections every run: no merge with previous frontmatter
        const refs = Array.from(newRefs);
        if (DEBUG && refs.length === 0) dbg('no-refs', { uuid, path: fpath, chunks: chunks.length });
        const next: Front = {
          ...fm,
...
await collectEntries(docsKV.iterator())
  .then((allDocs) => {
    ...
    let scopedDocs = allDocs.filter(([, info]) => isDocFile((info as DocInfo).path));
    if (opts.files && opts.files.length) {
      const wanted = new Set(opts.files.map(p => path.resolve(p)));
      scopedDocs = scopedDocs.filter(([, info]) => wanted.has(path.resolve((info as DocInfo).path)));
    }
    ...
    return collectEntries(chunksKV.iterator()).then((allChunks) => {
      ...
      const scopedChunks = allChunks.filter(([uuid]) => allowedUuids.has(uuid));
      const totalChunks = scopedChunks.reduce((n, [, cs]) => n + ((cs as readonly Chunk[])?.length ?? 0), 0);
      ...
      return buildDocPairs(scopedChunks, allowedUuids).then((docPairs) => {
        const docsByUuid = new Map<string, DocInfo>(scopedDocs as [string, DocInfo][]);
        const chunksMap = new Map<string, readonly Chunk[]>(scopedChunks as [string, readonly Chunk[]][]);
        let done = 0;
        const total = scopedDocs.length;
        return Promise.all(
          scopedDocs.map((e) =>
            processDoc(e as [string, DocInfo], chunksMap, docsByUuid, docPairs, allowedUuids)
              .then(() => { done++; onProgress?.({ step: 'relations', done, total }); })
          )
        );
      });
    });
...
if (isDirect) {
  ...
  console.log(`04-relations: ROOT=${path.resolve(args["--docs-dir"])}, DOC_THRESHOLD=${args["--doc-threshold"]}, REF_THRESHOLD=${args["--ref-threshold"]}`);
