...
let __DBG = false;
const dbg = (...xs: any[]) => { if (__DBG) console.log('[03-query]', ...xs); };
...
const toScore = (distance: number) => {
  ...
  if (distance >= 0 && distance <= 1) return Math.max(0, Math.min(1, 1 - distance));
  return 1 / (1 + distance);
};
...
export async function runQuery(
  ...
  onProgress?: (p: { step: 'query'; done: number; total: number; message?: string }) => void
) {
  ...
  for await (const [, value] of db.chunks.iterator()) {
    ...
    const filtered = selectedUuids ? cs.filter(c => selectedUuids!.has(c.docUuid)) : cs;
    total += filtered.length;
    for (const c of filtered) byId.set(c.id, c);
  }

  let computed = 0, skipped = 0, totalHits = 0;
  const hist = new Array(11).fill(0);

  let processed = 0;
  for await (const [, value] of db.chunks.iterator()) {
    const csAll = (value as readonly Chunk[]) ?? Object.freeze<Chunk[]>([]);
    const cs = selectedUuids ? csAll.filter(c => selectedUuids!.has(c.docUuid)) : csAll;
    for (const q of cs) {
      if (!opts.force) {
        try {
      ...
      const hits = Object.freeze(mapHits(ids, dists, byId, q.docUuid, (opts.k ?? 16)));
