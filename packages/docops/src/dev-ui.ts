...
import { runFrontmatter, runEmbed, runQuery, runRelations, runFooters, runRename } from './index';
...
// Static UI serving
const UI_ROOT = path.resolve(path.dirname(url.fileURLToPath(import.meta.url)), '../ui');
...
app.get('/api/files', async (req, reply) => {
  ...
  async function tree(p: string): Promise<any> {
    ...
    for (const e of ents) {
      ...
      if (e.isDirectory()) out.push({ name: e.name, type: 'dir', children: await tree(fp) });
...
app.get('/api/preview', async (req, reply) => {
  ...
  try {
    const db = await dbPromise;
    const out = await computePreview({ uuid, file }, { dir, docThreshold: docT, refThreshold: refT }, db);
    reply.header('content-type', 'application/json');
...
app.get('/api/read', async (req, reply) => {
  try {
    ...
    if (!/\.(md|mdx|markdown|txt)$/i.test(abs)) throw new Error('Only markdown or text files allowed');
...
function log(line: (s: string) => void, s: string) { line(s); }
...
app.get('/api/run', async (req, reply) => {
  ...
  try {
    ...
    const embedder = new OllamaEmbeddingFunction({ model: embedModel, url: OLLAMA_URL });
    ...
    line('PROGRESS ' + JSON.stringify({ step: 'frontmatter', index: 1, of: 6 }));
    const sel = q.files as string | undefined;
    const files = sel ? JSON.parse(sel) : undefined;
    await runFrontmatter({ dir, genModel: 'qwen3:4b', files }, db, (p)=> line('PROGRESS ' + JSON.stringify({ step: p.step, percent: Math.round((p.done/p.total)*100), message: `${p.done}/${p.total}` })));
    log(line, '01-frontmatter: done.');
    line('PROGRESS ' + JSON.stringify({ step: 'embed', index: 2, of: 6 }));
    await runEmbed({ dir, embedModel: embedModel, collection, files }, db, coll, (p)=> line('PROGRESS ' + JSON.stringify({ step: p.step, percent: Math.round((p.done/p.total)*100), message: `${p.done}/${p.total}` })));
    log(line, '02-embed: done.');
    line('PROGRESS ' + JSON.stringify({ step: 'query', index: 3, of: 6 }));
    await runQuery({ embedModel: embedModel, collection, k: 16, force: true, files }, db, coll, (p)=> line('PROGRESS ' + JSON.stringify({ step: p.step, percent: Math.round((p.done/p.total)*100), message: `${p.done}/${p.total}` })));
    log(line, '03-query: done.');
    line('PROGRESS ' + JSON.stringify({ step: 'relations', index: 4, of: 6 }));
    await runRelations({ docsDir: dir, docThreshold: Number(docT), refThreshold: Number(refT), files }, db, (p)=> line('PROGRESS ' + JSON.stringify({ step: p.step, percent: Math.round((p.done/p.total)*100), message: `${p.done}/${p.total}` })));
    log(line, '04-relations: done.');
    line('PROGRESS ' + JSON.stringify({ step: 'footers', index: 5, of: 6 }));
    await runFooters({ dir, anchorStyle: 'block', files }, db, (p)=> line('PROGRESS ' + JSON.stringify({ step: p.step, percent: Math.round((p.done/p.total)*100), message: `${p.done}/${p.total}` })));
...
app.get('/api/run-step', async (req, reply) => {
  ...
  const docT = Number(q.docT ?? '0.78');
  const refT = Number(q.refT ?? '0.85');
  const anchorStyle = (q.anchorStyle || 'block') as 'block'|'heading'|'none';
  const sel = q.files as string | undefined;
  const files = sel ? JSON.parse(sel) : undefined;
  ...
  try {
    ...
    switch (step) {
      ...
      case 'query':
        ...
        break;
      case 'relations':
        await runRelations({ docsDir: dir, docThreshold: docT, refThreshold: refT, files }, db);
        break;
      case 'footers':
