...
test('computeFenceMap and injectAnchors avoid code fences', t => {
  ...
  const anchors = [ { line: 4, id: 'ref-uuid-4-1' }, { line: 6, id: 'ref-uuid-6-1' } ];
...
test.serial('writeJSONArrayStream and writeNDJSON produce valid files', async t => {
  await withTmp(async dir => {
    ...
    t.deepEqual(lines.map(l => JSON.parse(l)), [{a:1},{b:2}]);
  });
});
