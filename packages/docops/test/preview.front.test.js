...
import { computePreview } from '../dist/preview-front.js';
...
test.serial('computePreview tolerates empty qhits and chunks', async t => {
  await withTmp(async tmp => {
    ...
    await db.chunks.put('u-1', Object.freeze(parseMarkdownChunks(body).map((c, i)=>({ ...c, id:`u-1:${i}`, docUuid:'u-1', docPath:file }))));

    const res = await computePreview({ file }, { dir: tmp, docThreshold: 0.8, refThreshold: 0.9 }, db);
    t.is(res.uuid, 'u-1');
    t.is(res.title, 'Doc');
    t.truthy(res.front);
