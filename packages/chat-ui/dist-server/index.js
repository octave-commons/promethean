...
const ensureStores = async () => {
    if (!storesInitialized) {
        try {
            ...
            console.log('✅ Initialized ContextStore collections: sessionStore, eventStore, messageStore');
        }
        catch (error) {
...
function sessionsToChatUIFormat(sessions) {
...
function messagesToChatUIFormat(messages) {
...
app.get('/api/sessions', async (req, res) => {
    try {
        ...
        const sessionsWithCounts = await Promise.all(sortedSessions.map(async (session) => {
            try {
...
app.get('/api/sessions/:sessionId/messages', async (req, res) => {
    try {
        ...
        try {
            ...
            const sessionMessages = messageStoreEntries.filter((entry) => entry.metadata?.sessionID === sessionId);
