...
class AutoProjectKanbanSync {
  ...
  async findProject(projectName) {
    ...
    try {
      ...
      console.log(`⚠️  Could not search for projects (may lack permissions): ${error.message}`);
      console.log(`   Will attempt to create a new project anyway...`);
      return null;
    }
  }
  ...
  async createProject(projectName, repositoryId) {
      ...
      mutation {
        createProjectV2(
          input: {
            ...
            title: "${projectName}"
            description: "Auto-generated kanban board from Promethean development system - synced from internal kanban tasks"
          }
        ) {
          projectV2 {
            id
    ...
    try {
      ...
      console.log(`⚠️  Could not create project (may lack project permissions): ${error.message}`);
      console.log(`   Will create GitHub Issues instead...`);
      return null;
    }
  }
...
> **To see the full kanban board**, check the internal Promethean development system or contact the maintainers.`;
  ...
  async addIssueToProject(issue, task, project) {
    ...
    console.log(`   📋 Adding issue to project column: ${this.mapStatusToColumn(task.status)}`);
  ...
  async syncKanbanTasksToProject() {
    ...
    if (!project) {
      try {
        ...
        console.log(`ℹ️  Could not create project board, will create issues only`);
      }
    }
    ...
    const maxTasks = 15; // Sync more tasks for the full board
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const sync = new AutoProjectKanbanSync();
...
export default AutoProjectKanbanSync;