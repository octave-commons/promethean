...
import type { Task, Board, ColumnData } from '@promethean-os/kanban-sdk';
...
export const extractTestingInfo = (
  ...
  const executedTests = testsMatch ? testsMatch[1]?.split(',').map((t: string) => t.trim()) : [];
...
export const normalizeColumnName = (column: string): string => {
  // Mirror columnKey normalization to keep transition rules in sync with board logic.
  const stripTrailingCount = (value: string): string =>
    value.replace(/\s*\(\s*\d+\s*\)\s*$/g, '').trim();
  const display = stripTrailingCount(column.trim());
  const normalizedDisplay = display.length > 0 ? display : 'Todo';
...
export const evaluateWipLimit = async (targetColumn: string, board: Board): Promise<boolean> => {
  const column = board.columns.find(
    (col: ColumnData) => normalizeColumnName(col.name) === targetColumn,
  );
...
export const evaluateTaskExistence = (sourceColumn: string, task: Task, board: Board): boolean => {
  const column = board.columns.find(
    (col: ColumnData) => normalizeColumnName(col.name) === sourceColumn,
  );

  if (!column) return false;

  return column.tasks.some((t: Task) => t.uuid === task.uuid);
};
