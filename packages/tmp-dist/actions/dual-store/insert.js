...
export const insert = async (inputs, dependencies) => {
    const { entry } = inputs;
    const { state, chroma, mongo, env, time, uuid, logger } = dependencies;
    const entryId = entry.id ?? uuid();
    const textValue = entry[state.textKey];
    const primaryTimestamp = entry[state.timeStampKey];
    ...
    const metadataWithTimestamp = withTimestampMetadata(entry.metadata, state.timeStampKey, epochTimestamp);
    const enhancedEntry = {
        ...entry,
        id: entryId,
