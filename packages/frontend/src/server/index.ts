...
async function startServer() {
        ...
        <html>
          <head>
            ...
            <style>
              ...
              .app-card { border: 1px solid #ddd; border-radius: 8px; padding: 20px; text-decoration: none; color: inherit; }
    ...
    app.get(`/${app}`, (req, res) => {
      if (isDev) {
        return res.send(`
          ...
          <html>
            <head>
              <title>${app.replace('-', ' ').toUpperCase()} - Promethean</title>
              <meta charset="utf-8">
            ...
            <body>
              <div class="container">
                <div class="header">
                  ...
                  <h1>${app.replace('-', ' ').toUpperCase()}</h1>
                </div>
                <div class="app-info">
                  <strong>Application:</strong> ${app}<br>
                  <strong>Status:</strong> Loading...<br>
                  <strong>Development Mode:</strong> ${isDev ? 'Enabled' : 'Disabled'}
                </div>
                <div class="iframe-container">
                  <iframe src="/app/${app}" id="${app}-frame"></iframe>
                </div>
              </div>
              <script>
        ...
        const appPath = join(__dirname, `../dist/${app}/index.html`);
  ...
  if (isDev) {
    ...
    app.post('/api/build/:app', async (req, res) => {
      ...
      try {
        // Here you would trigger the build process for the specific app
        res.json({
          message: `Build started for ${app}`,
          status: 'building',
        });
      } catch (error) {
        res.status(500).json({
          error: `Build failed for ${app}: ${error.message}`,
        });
      }
    });
  }
