...
const usingDevUi = async (fn: (devUi: ReadonlyDeep<DevUiProc>) => unknown) => {
  ...
  const result = fn(devUi as ReadonlyDeep<DevUiProc>) as PromiseLike<void> | void;
...
const initCallTracking = async (page: ReadonlyDeep<Page>) => {
  ...
  await page.route('**/api/files**', (route: ReadonlyDeep<Route>) => {
    ...
    void page.evaluate(
      ({ capturedUrl, storageKey }: ReadonlyDeep<{ capturedUrl: string; storageKey: string }>) => {
...
const waitForFilesResponse = (page: ReadonlyDeep<Page>) =>
  page.waitForResponse((r: ReadonlyDeep<Response>) => r.url().includes('/api/files') && r.ok());
...
const clickDirByLabel = (page: ReadonlyDeep<Page>, label: string) =>
  page
    .waitForFunction((text: string) => {
      ...
      return dirs.some((d) => (d.querySelector('.dir-name') as HTMLElement)?.textContent === text);
    }, label)
    .then(() =>
      page.evaluate((text: string) => {
        ...
        const target = dirs.find(
          (d) => (d.querySelector('.dir-name') as HTMLElement)?.textContent === text,
...
const runScenario = async (assertions: Assertions, deps: ReadonlyDeep<Deps>) => {
