(ns omni-tools.config.core
  "Unified AI configuration management for the Promethean ecosystem.
   
   This namespace provides the core functionality for managing MCP server configurations
   across multiple IDEs and formats, with support for both STDIO and streamable HTTP
   transports."
...
(defn write-atomic!
  ...
  (let [p   (fs/path path)
        ...
        tmp (fs/create-temp-file {:dir (str dir) :prefix ".tmp-omni-" :suffix ".edn"})]
    (spit (str tmp) s)
...
(defn expand-home-str
  "Expand `$HOME` and leading `~` in a string `s` using current HOME.
   Does not make path absolute relative to any base; purely textual."
  [s]
  (let [home (or (System/getenv "HOME")
                 (System/getProperty "user.home"))
        s    (str s)
