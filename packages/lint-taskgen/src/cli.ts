...
import { writeFile, readFile, mkdir, access, appendFile } from 'node:fs/promises';
...
function buildBuckets(results: Res[]) {
  ...
  let buckets = Array.from(map.values()).map(b => ({ ruleId: b.ruleId, scope: b.scope, count: b.count, files: Array.from(b.files).length, examples: b.examples }))
    .sort((a,b) => b.count - a.count);
  if (MIN_HITS > 0) buckets = buckets.filter(b => b.count >= MIN_HITS);
  if (TOP_N > 0) buckets = buckets.slice(0, TOP_N);
...
').filter(Boolean));
  for (const ex of incoming) { if (seen.size >= 20) break; if (!seen.has(ex)) seen.add(ex); }
...
async function updateTaskFile(file: string, count: number, examples: string[]) {
  ...
  const marker = '
**Latest snapshot**';
  const snapshot = [`${marker} (${NOW_ISO()})`, '', `Hits: **${count}**`, '', '**Examples (merged)**', '```', mergeExamples('', examples), '```', ''].join('
');
...
async function emitKanbanTasks(buckets: Array<{ ruleId:string; scope:string; count:number; examples:string[] }>) {
  ...
  for (const b of buckets) {
    ...
    if (exists && UPDATE) { await updateTaskFile(file, b.count, b.examples); updated++; continue; }
    if (exists) continue;
...
', 'utf8');
  ...
  console.log(`kanban: created ${created} tasks, updated ${updated}, dir ${TASKS_DIR}`);
}
