...
test('recursive autocommit integration test with multiple repos and subrepos', async (t) => {
  ...
  try {
    ...
    const subrepo1 = await createSubrepo(mainRepo, 'subrepo1', 'https://github.com/example/subrepo1.git');
    const subrepo2 = await createSubrepo(mainRepo, 'subrepo2', 'https://github.com/example/subrepo2.git');
    ...
    const nestedSubrepo = await createSubrepo(nestedRepo, 'nested-subrepo', 'https://github.com/example/nested-subrepo.git');
    ...
    await writeFile(join(nestedSubrepo, 'new-nested-subrepo-file.txt'), 'New nested subrepo content');
...
test('recursive autocommit finds correct number of repositories', async (t) => {
  ...
  try {
    ...
    await createSubrepo(nestedDir, 'nested-subrepo', 'https://github.com/example/nested-subrepo.git');
