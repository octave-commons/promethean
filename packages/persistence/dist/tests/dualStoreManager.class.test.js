...
const createMongoHarness = () => {
    ...
    const client = {
        db: (name) => ({
            ...
            collection: (collectionName) => getCollection(`${name}.${collectionName}`),
        }),
        async close() {
            collections.clear();
        },
...
const createQueueStub = () => {
    ...
    let shutdowns = 0;
    return {
        add: async (id, document, metadata) => {
...
const setupManager = async () => {
    ...
    __setMongoClientForTests(mongoHarness.client);
    const manager = new DualStoreManager('test-collection', chromaHarness.collection, mongoCollection, 'text', 'timestamp');
    await shutdownAllQueues();
    manager.chromaWriteQueue = queueStub;
...
test('retryVectorWrite records failure after retries', async (t) => {
    ...
    const originalAdd = manager.chromaCollection.add;
    manager.chromaCollection.add = async () => {
    ...
    manager.chromaCollection.add = originalAdd;
    await manager.cleanup();
});
...
//# sourceMappingURL=dualStoreManager.class.test.js.map