...
export class CrossDomainSearchEngine {
    ...
    async search(query: CrossDomainSearchOptions): Promise<CrossDomainSearchResponse> {
        ...
        try {
            ...
            console.log(`[cross-domain-search] Found ${finalResults.length} enhanced results in ${response.took}ms`);
            return response;
        } catch (error) {
            throw new Error(`Cross-domain search failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
     ...
     */
    private processResults(results: EnhancedSearchResult[], options: CrossDomainSearchOptions): EnhancedSearchResult[] {
    ...
    private calculateScoreBreakdown(
        ...
        return {
            ...
            source: sourceWeight,
            type: typeWeight,
            final: baseScore * sourceWeight * typeWeight * (options.timeBoost ? 1 + recencyScore * 0.2 : 1.0),
        };
    }
     ...
     */
    private applyWeights(results: EnhancedSearchResult[], options: CrossDomainSearchOptions): EnhancedSearchResult[] {
        return results.map((result) => {
     ...
     */
    private groupResultsBySource(results: EnhancedSearchResult[], maxPerSource?: number): EnhancedSearchResult[] {
