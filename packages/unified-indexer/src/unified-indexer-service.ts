...
import type { UnifiedFileIndexer, FileIndexingOptions, FileIndexingStats } from '@promethean-os/persistence';
...
import { createContextStore, compileContext, getOrCreateCollection } from '@promethean-os/persistence';
...
export class UnifiedIndexerService {
    ...
    async performFullSync(): Promise<UnifiedIndexerStats> {
        ...
        try {
            ...
            stats.errors.push(`Full sync failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            stats.syncDuration = Date.now() - startTime;
            throw error;
        }
    }
    ...
    private async updateUnifiedCollection(): Promise<void> {
        const unifiedCollectionName = this.config.contextStore.collections.unified;
        const [newState, unifiedCollection] = await getOrCreateCollection(this.contextStore, unifiedCollectionName);
    ...
    private async performPeriodicSync(): Promise<void> {
        try {
            ...
            this.errors.push(`Periodic sync failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
}
 ...
 */
export async function createUnifiedIndexerService(config: UnifiedIndexerServiceConfig): Promise<UnifiedIndexerService> {
