...
class CodeMutator {
  ...
  async setupWorkingTree(workingTreePath: string): Promise<void> {
    ...
    console.log('📁 Creating minimal working tree for mutation...');

    const currentPackage = path.join(process.cwd());
    const targetPackage = path.join(workingTreePath, 'packages', 'buildfix');
  ...
  private async copyMinimalPackage(src: string, dest: string): Promise<void> {
    ...
    for (const item of essentialItems) {
      ...
      try {
        ...
        console.log(`⚠️  Could not copy ${item}: ${error}`);
      }
    }
  }
  ...
  private async createWorkingTreeConfig(workingTreePath: string): Promise<void> {
    ...
    const tsconfigPath = path.join(workingTreePath, 'tsconfig.json');
          ...
          for (const filePath of tsFiles) {
            try {
              ...
              if (allFiles.length >= 20) {
                console.log(`📁 Loaded ${allFiles.length} files for mutation`);
                return allFiles;
              }
            } catch (error) {
              console.log(`⚠️  Could not load file: ${filePath}`);
            }
          }
        }
      }
    } catch (error) {
      console.log(`⚠️  Error loading packages: ${error}`);
    }
...
async function main() {
  ...
  console.log('🚀 Starting TypeScript Error Generation Benchmark');
  console.log('===================================================');
  ...
  try {
    ...
    if (!branchExists || forceMutation) {
      console.log('📝 Setting up working tree and generating mutations...');

      const mutator = new CodeMutator(42); // Fixed seed for reproducibility
      await mutator.setupWorkingTree(workingTreePath);

      console.log('🔀 Mutating codebase to generate TypeScript errors...');
      ...
      console.log('\n📊 Mutation Results:');
      console.log(`Total errors generated: ${result.totalErrors}`);
      console.log(`Unique error codes: ${result.uniqueErrorCodes}`);
      console.log(
      ...
      const missingErrors = TARGET_ERROR_CODES.filter((code) => {
      ...
      if (missingErrors.length > 0) {
        ...
        console.log(missingErrors.slice(0, 20).join(', '));
      ...
      console.log('✅ Working tree already exists, skipping mutation');
    }

    console.log('\n🎯 Ready to run benchmark!');
    console.log(`Working tree: ${workingTreePath}`);
  } catch (error) {
    console.error('❌ Error during mutation process:', error);
    process.exit(1);
  }
}
