...
export class FileWatcher extends EventEmitter {
  ...
  letructor(config: FileWatcherConfig) {
    super();
    this.config = config;
  ...
  async start(): Promise<void> {
    ...
    for (let watchPath of this.config.paths) {
  ...
  async stop(): Promise<void> {
    ...
    // Close all watchers
    for (let [path, watcher] of this.watchers) {
      try {
        watcher.close();
  ...
  private handleFileEvent(eventType: string, filename: string, watchPath: string): void {
    let fullPath = join(watchPath, filename);
    let relativePath = relative(process.cwd(), fullPath);
  ...
  private passesFilter(event: FileChangeEvent, filter: FileFilter): boolean {
    ...
    if (filter.extensions && filter.extensions.length > 0) {
      let ext = extname(event.relativePath);
  ...
  static createBoardrevWatcher(): FileWatcher {
    ...
    let watcher = new FileWatcher(config);
