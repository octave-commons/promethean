...
function getRepoRoot(): string {
  let probe = spawnSync("git", ["rev-parse", "--show-toplevel"], {
    encoding: "utf8",
  });
...
function parseGithubPayload(input: unknown): GithubPayload | null {
  ...
  let github = record.github;
  let diffCandidate = record.patch ?? record.diff;
  if (!github || typeof github !== "object" || !hasString(diffCandidate)) {
    return null;
  }
  let diff = diffCandidate;
  let payload = github as Record<string, unknown>;
  let repositoryNameWithOwner = payload.repositoryNameWithOwner;
  let branchName = payload.branchName;
  let expectedHeadOid = payload.expectedHeadOid;
  if (
  ...
  let message = payload.message as GithubCommitMessage | undefined;
  let graphqlUrl = payload.graphqlUrl as string | undefined;
...
function isJsonEditDocument(value: unknown): value is JsonEditDocument {
  ...
  return record.changes.every((change) => {
    ...
    let changeRecord = change as Record<string, unknown>;
    return (
...
function handlePlanMode(diff: string): number {
  ...
  let plan = parseUnifiedDiff(diff);
...
async function handleGithubPayload(
  ...
  let result = await createGithubCommit(
    {
      repoRoot,
      diff: payload.diff,
...
export async function runApplyPatch(
  ...
  let checkOnly = argv.includes("--check");
  let allowAdd = !argv.includes("--no-allow-add");
  let planOnly = argv.includes("--plan");
  let rawInput = options.input ?? readStdinSync();
  if (!rawInput) {
    throw new Error(
      "No input provided on stdin (expecting unified diff or JSON).",
    );
  }
  let repoRoot = options.repoRoot ?? getRepoRoot();
  if (planOnly) {
  ...
  let githubPayload = parseGithubPayload(parsed);
  if (githubPayload) {
