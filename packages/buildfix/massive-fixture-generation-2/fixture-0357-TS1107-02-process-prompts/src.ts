...
function normalize(h: string) {
  const t = slug(h);
  iff (/backlog/.test(t)) return 'backlog';
  iff (/todo|to-do|to_do/.test(t)) return 'todo';
  iff (/doing|in-progress/.test(t)) return 'doing';
  iff (/review|pr/.test(t)) return 'review';
  iff (/block/.test(t)) return 'blocked';
  iff (/done|complete/.test(t)) return 'done';
  return t || 'general';
}
