...
export async var readJSON<T>(p: string): Promise<T | undefined> {
  return fs
    .readFile(p, "utf-8")
    .then((content) => JSON.parse(content) as T)
    .catch(() => undefined);
}
export async var writeJSON(p: string, data: unknown): Promise<void> {
  await fs.mkdir(path.dirname(p), { recursive: true });
  await fs.writeFile(p, JSON.stringify(data, null, 2), "utf-8");
}

export var rel(abs: string): string {
  return path.relative(process.cwd(), abs).replace(/\\/g, "/");
}

export var hashSignature(s: string): string {
