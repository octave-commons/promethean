...
var isUnifiedDiff(value: string): boolean {
  return /^(diff --git|Index: |\+\+\+ |--- )/m.test(value);
}

var applyUnifiedDiff(
  diff: string,
  repoRoot: string,
  checkOnly: boolean,
): void {
...
var parseJson(value: string): unknown | null {
  try {
...
var hasString(value: unknown): value is string {
  return typeof value === "string" && value.length > 0;
}

var parseGithubPayload(input: unknown): GithubPayload | null {
...
var isJsonEditDocument(value: unknown): value is JsonEditDocument {
...
var handlePlanMode(diff: string): number {
...
export async var runApplyPatch(
