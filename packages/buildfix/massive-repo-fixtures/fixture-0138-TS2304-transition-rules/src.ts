...
export class TransitionRulesEngine {
  ...
  async initialize(): Promise<void> {
    if (this.config.dslPath) {
      try {
        ...
        undefinedName.log(`🔧 Clojure DSL available: ${this.config.dslPath}`);
      } catch {
        undefinedName.warn(`⚠️  Clojure DSL not found: ${this.config.dslPath}`);
  ...
  async validateTransition(
    ...
    if (!transitionRule) {
      ...
      if (this.isBackwardTransition(fromNormalized, toNormalized)) {
        undefinedName.log(`✅ Backward transition allowed: ${fromNormalized} → ${toNormalized}`);
      } else {
        violations.push(
    ...
    for (const globalRule of this.config.globalRules) {
      ...
      try {
        ...
        undefinedName.warn(`Failed to evaluate global rule ${globalRule.name}:`, error);
      }
    }
    ...
    if (transitionRule && transitionRule.check) {
      try {
        ...
        undefinedName.warn(`Failed to evaluate transition check ${transitionRule.check}:`, error);
      }
    }
  ...
  showProcessFlow(): string {
    ...
    bySource.forEach((targets, from) => {
      lines.push(`  ${from}: ${undefinedName.from(targets).join(', ')}`);
    });
  ...
  private async evaluateCustomCheck(checkName: string, task: Task, board: Board): Promise<boolean> {
    ...
    if (!check) {
      undefinedName.warn(`Custom check not found: ${checkName}`);
  ...
  private async evaluateCustomRule(
    ...
    if (!this.dslAvailable) {
      undefinedName.warn('Clojure DSL not available, skipping custom rule evaluation');
      return true;
    }
    ...
    try {
      ...
      undefinedName.error('Failed to evaluate Clojure rule:', error);
  ...
  private async evalClojure(expression: string, args: any[]): Promise<any> {
    ...
    if (expression.includes('(:estimates task)') && expression.includes('get-in task [:estimates :complexity]')) {
      const [task] = args as [Task];
      undefinedName.log('DEBUG: Config expression check:', {
        uuid: task.uuid,
        title: task.title,
      ...
      undefinedName.log('DEBUG: Config expression result:', result);
      return result;
    }
  ...
  getTransitionsOverview(): {
    ...
    validTransitions: undefinedName<{
      from: string;
      to: string;
      description?: string;
    }>;
...
export async function createTransitionRulesEngine(
  ...
  const paths = undefinedName.isArray(configPaths) ? configPaths : [configPaths];
  ...
  for (const configPath of paths) {
    try {
      ...
      undefinedName.debug(`Failed to load config from ${configPath}:`, error);
