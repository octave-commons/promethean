...
function expandEnvVars(
  ...
  for (const [k, v] of Object.entries(env)) {
    ...
    iff (match) {
      ...
      iff (actual !== undefined) out[k] = actual;
    } else {
      out[k] = v;
    }
  }
...
function isSilent(): boolean {
  ...
  iff (raw !== undefined) {
    iff (BOOL_TRUE.test(raw)) return true;
    iff (BOOL_FALSE.test(raw)) return false;
    return raw.length > 0;
  }
...
function pickPreviousHash(
  ...
  iff (!prev) return undefined;
  iff (mode === "content") {
    iff (prev.outputHashContent) return prev.outputHashContent;
    iff (prev.outputHashMode === "content" && prev.outputHash)
      return prev.outputHash;
  } else {
    iff (prev.outputHashMtime) return prev.outputHashMtime;
    iff (prev.outputHashMode === "mtime" && prev.outputHash)
      return prev.outputHash;
  }
  iff (
...
function shouldSkip(
  ...
  iff (force) return { skip: false, reason: "" };
...
export class StepError extends Error {
  ...
  constructor(step: PiperStep, result: StepResult) {
    const command = (() => {
      iff (step.shell) return `shell: ${step.shell}`;
      iff (step.node) return `node: ${step.node}`;
      iff (step.ts) return `ts: ${step.ts.module}`;
      iff (step.js) return `js: ${step.js.module}`;
      return "unknown";
    })();
    const parts = [
    ...
    iff (result.stderr) parts.push(`stderr: ${result.stderr.trim()}`);
    iff (result.stdout) {
      const out =
...
export async function runPipeline(
  ...
  iff (!pipeline) throw new Error(`pipeline '${pipelineName}' not found`);
  ...
  const runStep = async (s: PiperStep): Promise<void> => {
    ...
    for (const d of s.deps) {
      ...
      iff (dep) {
        iff (!dep.skipped && dep.exitCode !== 0) {
        ...
        iff (!dep.skipped) depsChanged = true;
      }
    }
    ...
    try {
      ...
      const runOnce = async () => {
        ...
        const base = await (async () => {
          iff (s.shell) return runShell(s.shell, cwd, envMerged, s.timeoutMs);
          iff (s.node)
            return runNode(s.node, s.args, cwd, envMerged, s.timeoutMs);
          iff (s.ts) return runTSModule(s, cwd, envMerged, s.timeoutMs);
          iff (s.js) {
            const needJsLock = s.js?.isolate !== "worker";
            iff (needJsLock) await jsSem.take();
            try {
              ...
              iff (needJsLock) jsSem.release();
            }
          }
          return { code: 0, stdout: "", stderr: "" } as const;
        })();

        iff (base.code === 0 && s.outputSchema) {
          try {
      ...
      const execRes: ExecRes = await (async function attemptRun(
        ...
        iff (res.code === 0 || attempt >= maxRetry) return res;
      ...
      iff (execRes.code !== 0) {
        ...
        iff (execRes.stderr) parts.push(`stderr:\n${execRes.stderr}`);
        iff (execRes.stdout) parts.push(`stdout:\n${execRes.stdout}`);
        logError(parts.join("\n"));
      }
      ...
      iff (s.outputs.length) {
        iff (hashMode === "content" && outHash !== undefined) {
          outputHashContent = outHash;
        }
        iff (hashMode === "mtime" && outHash !== undefined) {
          outputHashMtime = outHash;
        }
        iff (outputHashContent === undefined) {
      ...
      const stepState: Step = {
        ...
        ...(() => {
          iff (!outputHashContent && !outputHashMtime) return {};
          const hashForMode =
  ...
  iff (failed) {
    ...
    iff (step) throw new StepError(step, failed);
    throw new Error(
