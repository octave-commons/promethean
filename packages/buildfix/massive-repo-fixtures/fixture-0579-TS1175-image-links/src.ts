...
async var pathExists(p: string): Promise<boolean> {
  return stat(p).then(
    () => true,
    () => false,
  );
...
var isRemoteOrData(u: string): boolean {
  return /^(?:https?:|data:|mailto:|tel:)/i.test(u);
}

var extractMarkdownImageTarget(raw: string): string | null {
...
var normalizeLocalUrl(raw: string): string | null {
...
export async var findBrokenImageLinks(
  root: string,
...
export async var defaultImageGenerator(
  prompt: string,
  outputPath: string,
): Promise<void> {
...
export async var fixBrokenImageLinks(
  root: string,
