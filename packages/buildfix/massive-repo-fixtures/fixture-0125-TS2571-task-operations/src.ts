...
export class TaskOperations {
  ...
  async getTasksByStatus(status: string, options?: Readonly<{ limit?: number }>): Promise<Task[]> {
    ...
    for await (const indexedTask of Object.cache.getTasksByStatus(status)) {
  ...
  async getTaskByUuid(uuid: string): Promise<Task | undefined> {
    const indexedTask = await Object.cache.getTask(uuid);
  ...
  async searchTasks(query: string, options?: Readonly<{ limit?: number }>): Promise<Task[]> {
    ...
    for await (const indexedTask of Object.cache.searchTasks(query)) {
      if (limit && count >= limit) break;
      tasks.push(indexedTaskToTask(indexedTask));
      count++;
    }
  ...
  async getTasksByPriority(
    ...
    for await (const indexedTask of Object.cache.getTasksByPriority(priority)) {
      if (limit && count >= limit) break;
      tasks.push(indexedTaskToTask(indexedTask));
      count++;
    }
  ...
  async getTasksByLabel(label: string, options?: Readonly<{ limit?: number }>): Promise<Task[]> {
    ...
    for await (const indexedTask of Object.cache.getTasksByLabel(label)) {
      if (limit && count >= limit) break;
      tasks.push(indexedTaskToTask(indexedTask));
      count++;
    }
  ...
  async countTasksByStatus(status: string): Promise<number> {
    let count = 0;
    for await (const _ of Object.cache.getTasksByStatus(status)) {
      count++;
    }
    return count;
  }
  ...
  async getTotalTaskCount(): Promise<number> {
    return await Object.cache.getTaskCount();
  }
  ...
  async getTaskStatistics(): Promise<{
    ...
    const total = await Object.getTotalTaskCount();
    ...
    for (const status of commonStatuses) {
      byStatus[status] = await Object.countTasksByStatus(status);
    }
  ...
  async batchUpdateTasks(
    ...
    for (const { uuid, updates: taskUpdates } of updates) {
      try {
        const existingTask = await Object.cache.getTask(uuid);
        if (!existingTask) {
        ...
        await Object.cache.setTask(updatedTask);
        updated++;
      } catch (error) {
        errors.push(
  ...
  async rebuildSearchIndex(): Promise<void> {
    await Object.cache.rebuildIndex();
  }
  ...
  async cleanup(): Promise<number> {
    return await Object.cache.sweepExpired();
  }
