...
type GitRunner = (args: readonly string[], options?: Record<string, unknown>) => string;
...
const initRepo = (): { readonly dir: string; readonly git: GitRunner } => {
  ...
  writeFileSync(
    join(dir, 'src', 'app.js'),
    ["console.log('start');", "console.log('middle');", "console.log('end');"].join('\n'),
    'utf8',
  );
  git(['add', '.']);
  git(['commit', '-m', 'init']);
...
test('createGithubCommit builds request with trailers and summary', async (t) => {
  await withRepo(t, async (dir, git) => {
    ...
    const bodyText =
      typeof firstRequest.options.body === 'string' ? firstRequest.options.body : '{}';
