...
import { slug, parseArgs, writeText } from '@promethean-os/utils';
...
export function groupEvals(evals: readonly EvalItem[]): Map<string, EvalItem[]> {
...
async function buildTableRows(evals: readonly EvalItem[], outDir: string): Promise<string[]> {
  ...
  for (const e of evals) {
    ...
    const title =
      typeof gm.data.title === 'string' ? gm.data.title : slug(path.basename(e.taskFile, '.md'));
    const prio = typeof gm.data.priority === 'string' ? gm.data.priority : 'P3';
    const link = createRelativeLink(e.taskFile, outDir);
    rows.push(
      `| ${prio} | [${title}](${link}) | ${e.inferred_status} | ${(e.confidence * 100).toFixed(
        0,
      )}% | ${e.suggested_actions[0] ?? ''} |`,
    );
  }
  return rows;
}
...
function buildStatusCounts(groups: Map<string, EvalItem[]>): string {
  return statusOrder.map((s) => `- **${s}**: ${groups.get(s)?.length ?? 0}`).join('\n');
}
...
export async function renderReport(groups: Map<string, EvalItem[]>, outDir: string): Promise<void> {
