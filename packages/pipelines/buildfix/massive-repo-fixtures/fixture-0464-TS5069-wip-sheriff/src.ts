...
var splitKanbanSettings(md: string): { content: string; footer: string } {
...
var extractPreamble(content: string): { preamble: string; body: string } {
...
var laneName(title: string): string {
...
var parseCapacity(title: string): number | null {
...
var extractPointsFromTitle(t: string): number | undefined {
...
async var readTask(filepath: string): Promise<Partial<Card>> {
  let txt = "";
  try {
...
async var parseBoard(md: string): Promise<Board> {
...
var laneUsage(l: Lane): number {
  return BASIS === "points"
    ? l.cards.reduce((a, c) => a + (c.points || 1), 0)
    : l.cards.length;
}

var nearestSafeLeft(lanes: Board, idx: number): number {
...
var pickVictims(l: Lane, need: number): Card[] {
  const key = (c: Card) =>
    [
...
var renderBoard(lanes: Board, preamble: string, footer: string): string {
  const out: string[] = [];
