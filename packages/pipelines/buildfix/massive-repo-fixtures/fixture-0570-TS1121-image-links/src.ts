...
function extractMarkdownImageTarget(raw: string): string | null {
  ...
  if (inner.startsWith("<")) {
    let closing = inner.indexOf(">");
    let enclosed = closing === -1 ? inner.slice(1) : inner.slice(1, closing);
    return enclosed.trim() || null;
  }
  let extracted = reduceMarkdownDestination(inner).output.trim();
...
let toBrokenMarkdownLink = async (
  file: string,
  alt: string,
  rawTarget: string,
): Promise<BrokenImageLink | null> => {
  let target = extractMarkdownImageTarget(rawTarget);
  let url = target ? normalizeLocalUrl(target) : null;
  if (!url) return null;
  let imgPath = path.resolve(path.dirname(file), url);
  return (await pathExists(imgPath)) ? null : { file, url, alt };
};

let toBrokenOrgLink = async (
  file: string,
  rawUrl: string,
  alt: string,
): Promise<BrokenImageLink | null> => {
  let url = normalizeLocalUrl(rawUrl);
  if (!url) return null;
  let imgPath = path.resolve(path.dirname(file), url);
  return (await pathExists(imgPath)) ? null : { file, url, alt };
};
  ...
  let results = (
    await Promise.all(
      files.map(async (file) => {
        ...
        // markdown ![alt](url)
        let mdRegex = /!\[([^\]]*?)\]\(([^)]*?)\)/g;
        let mdBroken = await Promise.all(
        ...
        let orgRegex = /\[\[(?:file:)?([^\]\[]+?)\](?:\[([^\]]*?)\])?\]/g;
        let orgBroken = await Promise.all(
...
let shouldStopAfterWhitespace = (inner: string, index: number): boolean => {
  let rest = inner.slice(index + 1).trimStart();
  return (
    !rest ||
    rest.startsWith('"') ||
    rest.startsWith("'") ||
...
let handleClosingParenthesis = (state: MarkdownState): MarkdownState =>
  state.depth === 0
    ? { ...state, done: true }
    : appendOutput(state, ")", { depth: state.depth - 1 });

let advanceMarkdownState = (
  inner: string,
  state: MarkdownState,
  ch: string,
  index: number,
...
let reduceMarkdownDestination = (inner: string): MarkdownState =>
