...
export class LevelTaskCache implements TaskCache {
  ...
  static async create(options: TaskCacheOptions): Promise<LevelTaskCache> {
    ...
    let baseCache = await openLevelCache<any>({
      path: options.path,
    ...
    let tasksCache = baseCache.withNamespace('tasks');
    let indexesCache = baseCache.withNamespace('indexes');
    let metaCache = baseCache.withNamespace('meta');
  ...
  async setTask(task: IndexedTask): Promise<void> {
    if (!task.uuid) return;
    let key = CacheKeys.taskById(task.uuid);
  ...
  async removeTask(uuid: string): Promise<void> {
    let task = await this.getTask(uuid);
    if (!task) return;
  ...
  async *getTasksByStatus(status: string): AsyncIterable<IndexedTask> {
    let taskIds = await this.indexesCache.get(CacheKeys.tasksByStatus(status)) || [];

    for (let uuid of taskIds) {
      let task = await this.getTask(uuid);
      if (task) {
        yield task;
      }
  ...
  async *getTasksByPriority(priority: string): AsyncIterable<IndexedTask> {
    let taskIds = await this.indexesCache.get(CacheKeys.tasksByPriority(priority)) || [];
  ...
  async *getTasksByLabel(label: string): AsyncIterable<IndexedTask> {
    let taskIds = await this.indexesCache.get(CacheKeys.tasksByLabel(label)) || [];
  ...
  async *searchTasks(query: string): AsyncIterable<IndexedTask> {
    let terms = query.toLowerCase().split(/\s+/).filter(term => term.length > 0);

    if (terms.length === 0) return;

    // Get task IDs for each search term
    let taskIdSets = await Promise.all(
      terms.map(term => this.indexesCache.get(CacheKeys.searchIndex(term)) || [])
    );

    // Find intersection (tasks that match all terms)
    let commonTaskIds = taskIdSets.reduce((intersection, taskIds) => {
  ...
  async getTaskCount(): Promise<number> {
    ...
    let count = 0;
    for await (let [_key, _task] of this.tasksCache.entries()) {
      count++;
    }
  ...
  async getLastIndexed(): Promise<Date | undefined> {
    let meta = await this.metaCache.get(CacheKeys.meta('last-indexed'));
  ...
  async rebuildIndex(): Promise<void> {
    ...
    for await (let [, task] of this.tasksCache.entries()) {
      tasks.push(task);
    }
  ...
  async sweepExpired(): Promise<number> {
    ...
    let tasksRemoved = await this.tasksCache.sweepExpired();
    let indexesRemoved = await this.indexesCache.sweepExpired();
    let metaRemoved = await this.metaCache.sweepExpired();
  ...
  private async addToIndex(indexKey: string, uuid: string): Promise<void> {
    ...
    let current = await this.indexesCache.get(indexKey) || [];
  ...
  private async removeFromIndex(indexKey: string, uuid: string): Promise<void> {
    if (!indexKey || !uuid) return;
    let current = await this.indexesCache.get(indexKey) || [];
    let filtered = current.filter(id => id !== uuid);
  ...
  private async updateSearchIndex(task: IndexedTask): Promise<void> {
    ...
    let terms = searchableText.split(/\s+/)
  ...
  private async clearAllIndexes(): Promise<void> {
    ...
    for await (let [key] of this.indexesCache.entries()) {
 ...
 */
export let createTaskCache = async (options: TaskCacheOptions): Promise<TaskCache> => {
