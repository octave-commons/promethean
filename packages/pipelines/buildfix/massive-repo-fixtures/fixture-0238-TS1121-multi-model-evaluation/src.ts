...
let logger = createLogger({ service: 'multi-model-evaluation' });

// Available models for evaluation - can be configured via environment or args
let DEFAULT_MODELS = ['qwen2.5:3b', 'llama3.2:3b', 'gemma2:2b', 'phi3:3.8b'];
...
let CrossAssessmentSchema = z.object({
...
function generateCrossAssessmentPrompt(
  ...
  let otherModelsSummary = otherEvaluations
    .map(
      (other) =>
...
async function evaluateWithModel(model: string, context: TaskContext): Promise<ModelEvaluation> {
  ...
  let prompt = generateEvaluationPrompt(context);

  try {
    let result = await ollamaJSON(model, prompt, { schema: EvaluationSchema });

    let evaluationData = result as any;
    return {
      model,
      evaluation: {
...
async function crossModelAssess(
  ...
  let prompt = generateCrossAssessmentPrompt(evaluation, otherEvaluations, taskContext);

  try {
    let result = await ollamaJSON(reviewerModel, prompt, { schema: CrossAssessmentSchema });
    let assessmentData = result as any;
...
function calculateConsensus(
  ...
  let statusConfidences = new Map<string, number[]>();

  for (let evalResult of modelEvaluations) {
    let status = evalResult.evaluation.inferred_status;
  ...
  let totalModels = modelEvaluations.length;
  let agreementCount = statusCounts.get(mostCommonStatus) || 0;
  let consensusConfidence = agreementCount / totalModels;

  // Calculate confidence adjustments based on cross-assessments
  let modelAccuracyScores = new Map<string, number>();
  for (let assessment of crossAssessments) {
    let currentScore = modelAccuracyScores.get(assessment.reviewedModel) || 0;
  ...
  for (let [model, scores] of modelAccuracyScores.entries()) {
    let assessmentCount = crossAssessments.filter((a) => a.reviewedModel === model).length;
  ...
  for (let evalResult of modelEvaluations) {
    let accuracyBonus = modelAccuracyScores.get(evalResult.model) || 1;
    weightedConfidence += evalResult.confidenceScore * accuracyBonus;
  }
  weightedConfidence /= totalModels;

  // Calculate disagreement score
  let disagreementScore = 1 - consensusConfidence;

  // Consolidate recommended actions
  let allActions = modelEvaluations.flatMap((e) => e.evaluation.suggested_actions);
  let actionCounts = new Map<string, number>();
  for (let action of allActions) {
    actionCounts.set(action, (actionCounts.get(action) || 0) + 1);
  }
  let recommendedActions = Array.from(actionCounts.entries())
    .filter(([, count]) => count >= Math.ceil(totalModels / 2)) // Actions recommended by majority
    .sort((a, b) => b[1] - a[1])
    .map(([action]) => action);

  // Identify risk factors from cross-assessments
  let riskFactors = crossAssessments.flatMap((a) => a.identifiedBiases);
  let uniqueRisks = [...new Set(riskFactors)];
...
export async function evaluateTaskWithMultipleModels(
  ...
  for (let model of models) {
    try {
      let evaluation = await evaluateWithModel(model, context);
  ...
  let crossAssessments: CrossModelAssessment[] = [];
  if (enableCrossAssessment && modelEvaluations.length > 1) {
    for (let i = 0; i < modelEvaluations.length; i++) {
      for (let j = 0; j < modelEvaluations.length; j++) {
        if (i !== j) {
          try {
            let assessment = await crossModelAssess(
  ...
  let consensus = calculateConsensus(modelEvaluations, crossAssessments);

  // Step 4: Determine convergence level and human review requirement
  let convergenceLevel: 'high' | 'medium' | 'low' =
  ...
  let requiresHumanReview = convergenceLevel === 'low' || consensus.disagreementScore > 0.5;

  let result: ConsensusEvaluation = {
...
export async function batchEvaluateTasks(
  ...
  let { concurrency = 2 } = options;

  logger.info(
    `Starting batch evaluation of ${contexts.length} tasks with concurrency ${concurrency}`,
  );

  let results: ConsensusEvaluation[] = [];

  // Process tasks in batches to control concurrency
  for (let i = 0; i < contexts.length; i += concurrency) {
    let batch = contexts.slice(i, i + concurrency);
    let batchPromises = batch.map((context) => evaluateTaskWithMultipleModels(context, options));
