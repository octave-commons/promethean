 ...
 */
export async function listTaskFiles(dir: string): Promise<ReadonlyArray<string>> {
...
export async function detectChangedFiles(
  ...
  let { promises: fs } = await import('node:fs');
  ...
  let cachedPaths = new Set(metadataCache.keys());
  ...
  for (let file of files) {
    ...
    try {
      ...
      let hash = createContentHash(content);
...
export async function updateFileMetadata(
  ...
  let { promises: fs } = await import('node:fs');
  let updatedCache = new Map(existingCache);
