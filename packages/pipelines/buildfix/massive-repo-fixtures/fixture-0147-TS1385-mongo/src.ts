...
export async function startMongoChangefeed<TDoc extends Document & { _id?: unknown }, TPayload = TDoc>(
    ...
    let coll = db.collection<TDoc>(opts.collection);
    let log = createLogger({ service: `changefeed:${opts.collection}` });
...
function extractDoc<TDoc extends Document>(change: ChangeStreamDocument<TDoc>): TDoc | undefined {
    ...
    let docId = (change as { documentKey?: { _id?: unknown } }).documentKey?._id;
...
async function publishChange<TDoc extends Document, TPayload>(
    ...
    let { topic, change, payload, doc } = args;
    let changeId = change._id;
    let docId = doc._id;
...
async function watchOnce<TDoc extends Document & { _id?: unknown }, TPayload>(
    ...
    let { coll, bus, opts, getStopped, setCurrentStream, getResume, setResume } = args;
    let cs = coll.watch<TDoc>([], {
    ...
    setCurrentStream(cs);
    try {
        for await (let change of cs) {
            if (getStopped()) break;
            let doc = extractDoc<TDoc>(change);
            if (!doc) continue;
            if (opts.filter && !opts.filter(doc)) continue;
            let payload = opts.map ? opts.map(doc) : (doc as unknown as TPayload);
            ...
            let updated = await persistResumeToken(
