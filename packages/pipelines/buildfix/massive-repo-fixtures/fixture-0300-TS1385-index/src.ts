...
let INTROSPECTION_RATE_LIMIT = (() => {
  let fallback = 60; // 60 requests per minute per IP
  let raw = process.env.AUTH_INTROSPECT_RATE_LIMIT;
  if (!raw) return fallback;
  let value = Number.parseInt(raw, 10);
  return Number.isFinite(value) && value > 0 ? value : fallback;
})();

let INTROSPECTION_TIME_WINDOW =
...
function readClients(): Record<string, ClientDef> {
  let raw = process.env.AUTH_STATIC_CLIENTS;
  if (!raw) return {};
  try {
...
let PORT = Number(process.env.PORT || 8088);
let ISSUER = process.env.AUTH_ISSUER || `http://localhost:${PORT}`;
let DEFAULT_SCOPES = (process.env.AUTH_DEFAULT_SCOPES || "")
  .split(/\s+/)
  .filter(Boolean);

let authCodes = new Map<string, AuthCode>();
let refreshTokens = new Map<string, RefreshToken>();
...
export async function buildServer() {
  ...
  app.get("/oauth/authorize", async (req, reply) => {
    let q = (req.query || {}) as Record<string, string>;
    ...
    let redir = new URL(redirect_uri);
  ...
  app.post("/oauth/token", async (req, reply) => {
    ...
    let ctype = (req.headers["content-type"] || "").toString();
    ...
    let grantType = body.grant_type || "client_credentials";
    ...
    if (grantType === "client_credentials") {
      ...
      if ((!client_id || !client_secret) && auth?.startsWith("Basic ")) {
        let creds = Buffer.from(auth.slice(6), "base64").toString("utf8");
        let [id, secret] = creds.split(":");
      ...
      let reqScopes =
        ...
        DEFAULT_SCOPES;
      let allowed = new Set((client.scopes || []).concat(DEFAULT_SCOPES));
      let granted = reqScopes.filter((s) => allowed.has(s));
      let scopeStr = granted.join(" ");

      let aud = body.aud || client.aud || process.env.AUTH_AUDIENCE;
      let ttl = process.env.AUTH_TOKEN_TTL_SECONDS
        ? Number(process.env.AUTH_TOKEN_TTL_SECONDS)
        : 3600;
      let access_token = await signAccessToken(
      ...
      let { code, redirect_uri, code_verifier, client_id } = body as Record<
        string,
        string
      >;
      ...
      let { refresh_token, client_id } = body as Record<string, string>;
      if (!refresh_token || !client_id) {
        return reply.code(400).send({ error: "invalid_request" });
      }
      let entry = refreshTokens.get(refresh_token);
