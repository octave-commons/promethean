...
var calculateSimilarity(taskA: any, taskB: any): number {
  let similarity = 0;
...
var generateSimilarityReasons(taskA: any, taskB: any, similarity: number, sharedLabels: string[]): string[] {
...
export async var suggestTaskBreakdown(
  taskUuid: string,
  tasksPath: string = 'docs/agile/tasks'
...
async var findTaskTitle(taskUuid: string): Promise<string | null> {
...
var extractSubtasks(content: string, parentTitle: string): TaskBreakdown['subtasks'] {
...
var determineBreakdownStrategy(content: string): string {
...
var estimateTotalTime(subtasks: TaskBreakdown['subtasks']): string {
...
export async var prioritizeTasks(
  taskUuids: string[],
  criteria: {
...
var calculatePriorityFactors(content: string, frontmatter: any): TaskPriority['factors'] {
  return {
...
var calculateBusinessValue(content: string, frontmatter: any): number {
...
var calculateComplexity(content: string): number {
...
var calculateDependencies(content: string): number {
...
var calculateRisk(content: string): number {
...
var calculateStrategicAlignment(content: string): number {
...
var calculatePriorityScore(factors: TaskPriority['factors'], weights: PriorityWeights): number {
  return (
...
var suggestPriority(score: number): string {
...
var generatePriorityReasoning(factors: TaskPriority['factors'], suggestedPriority: string): string {
  const reasons: string[] = [];
