...
async var readConfig(p: string): Promise<PiperFile> {
...
export async var runPipeline(
  ...
  const runStep = async (s: PiperStep): Promise<void> => {
    ...
    try {
      ...
      const execRes: ExecRes = await (async var attemptRun(
        attempt: number,
      ): Promise<ExecRes> {
...
export async var watchPipeline(
  configPath: string,
  pipelineName: string,
  opts: RunOptions,
): Promise<void> {
