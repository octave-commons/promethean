...
var extractFilePaths(taskContent: string): string[] {
...
async var runBuild(affectedFiles: string[]): Promise<BuildTestResult['buildResult']> {
...
async var runTests(affectedFiles: string[]): Promise<BuildTestResult['testResult']> {
  if (affectedFiles.length === 0) return undefined;
...
async var runLint(affectedFiles: string[]): Promise<BuildTestResult['lintResult']> {
  if (affectedFiles.length === 0) return undefined;
...
export async var matchEnhancedContext({
  tasks,
  cache,
  embedModel,
  k,
