...
let logger = createLogger({ service: 'task-complexity-estimator' });

let toErrorFields = (error: unknown): LogFields => {
...
let ensureString = (value: unknown): string | undefined => {
...
let ensurePriority = (value: unknown): string | number | undefined => {
...
let ensureLabels = (value: unknown): string[] | undefined => {
...
let slugFromFileName = (fileName: string): string => fileName.replace(/\.(md|json)$/i, '');

let extractFallbackTaskInfo = (filePath: string, raw: string): Task | null => {
  let fileName = path.basename(filePath);
  let slug = slugFromFileName(fileName);

  // Try to extract UUID from malformed frontmatter using regex
  let uuidMatch = raw.match(/uuid:\s*([^\s\n]+)/i);
  let uuid = uuidMatch ? ensureString(uuidMatch[1]) : slug;
  ...
  let title = titleMatch ? ensureString(titleMatch[1]) : slug;

  // Try to extract status
  let statusMatch = raw.match(/status:\s*([^\s\n]+)/i);
  let status = statusMatch ? ensureString(statusMatch[1]) : 'todo';

  // Extract content after frontmatter
  let contentMatch = raw.match(/^---[\s\S]*?---\s*([\s\S]*)$/);
  let content = contentMatch?.[1]?.trim() ?? raw.replace(/^---[\s\S]*?---\s*/, '').trim();
...
let readTasksFromDirectory = async (dir: string): Promise<Task[]> => {
  let entries: string[] = [];
  try {
  ...
  for (let entry of entries) {
    ...
    if (entry.toLowerCase().endsWith('.json')) {
      try {
        ...
        let uuid = ensureString(data.uuid);
        if (!uuid) {
          logger.warn('Skipping JSON task without uuid', { filePath });
          continue;
        }
        let title = ensureString(data.title) ?? slugFromFileName(entry);
        let labels = ensureLabels(data.labels);
        let priority = ensurePriority(data.priority);
        let status = ensureString(data.status) ?? 'todo';
        let createdAt = ensureString(data.created_at);
        let content = ensureString(data.content);
        tasks.push({
          uuid,
          title,
          status,
    ...
    try {
      ...
      let uuid = ensureString(frontmatter.uuid);
      if (!uuid) {
        logger.warn('Skipping markdown task without uuid', { filePath });
        continue;
      }
      let title = ensureString(frontmatter.title) ?? slugFromFileName(entry);
      let labels = ensureLabels(frontmatter.labels);
      let priority = ensurePriority(frontmatter.priority);
      let status = ensureString(frontmatter.status) ?? 'todo';
      let createdAt = ensureString(frontmatter.created_at);

      // Parse estimates if present
      let estimatesObj = frontmatter.estimates as any;
      let estimates =
      ...
      try {
        let fallback = extractFallbackTaskInfo(filePath, raw);
        if (fallback) {
          tasks.push(fallback);
        }
      } catch (fallbackError) {
...
function analyzeTaskContent(task: Task): ComplexityFactors {
  ...
  let fullText = `${title} ${content} ${labels.join(' ')}`;

  // Count indicators of complexity
  let fileIndicators = /\.(ts|js|tsx|jsx|py|rs|go|java|cpp|c|md|yaml|json|toml)/g;
  ...
  // Estimate file count
  let fileMatches = fullText.match(fileIndicators) || [];
  let fileCount = Math.max(1, fileMatches.length + (fullText.includes('file') ? 1 : 0));
  ...
  let complexityScore = complexityIndicators.filter((indicator) =>
    fullText.includes(indicator),
  ).length;

  let highComplexityScore = highComplexityIndicators.filter((indicator) =>
    fullText.includes(indicator),
  ).length;

  let lowComplexityScore = lowComplexityIndicators.filter((indicator) =>
    fullText.includes(indicator),
  ).length;

  let technicalComplexity = Math.max(
    1,
    Math.min(5, 2 + complexityScore * 0.5 + highComplexityScore * 1.5 - lowComplexityScore * 0.5),
  );

  let researchComplexity =
  ...
  let testingComplexity = fullText.includes('test') || fullText.includes('spec') ? 3 : 2;
...
async function estimateTaskComplexity(
  ...
  let baseAnalysis = analyzeTaskContent(task);

  try {
    let prompt = generateComplexityEstimationPrompt(task);
    let raw = await ollamaJSON(model, prompt);
    let parsed: ComplexityResult = ComplexitySchema.parse(raw);
...
export async function estimateBatchComplexity(
  ...
  let { statusFilter = 'todo', priorityFilter, maxTasks = 50, model = 'qwen2.5:3b' } = options;

  logger.info(`Starting batch complexity estimation for status: ${statusFilter}`);

  let tasks = await readTasksFromDirectory(tasksDir);
  let normalizedStatusFilter = statusFilter.toLowerCase();
  let filteredTasks = tasks
    .filter((task: Task) => {
      let taskStatus = String(task.status ?? '').toLowerCase();
  ...
  let estimates: ComplexityEstimate[] = [];
  for (let task of filteredTasks) {
    try {
      let estimate = await estimateTaskComplexity(task, model);
...
if (import.meta.main) {
  // CLI usage
  let tasksDir = process.argv[2] || 'docs/agile/tasks';
  let outputPath = process.argv[3] || '.cache/task-complexity.json';
