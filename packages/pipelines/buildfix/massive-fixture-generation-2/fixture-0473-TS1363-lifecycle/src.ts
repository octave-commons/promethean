...
export class TaskLifecycleManager {
  ...
  async archiveTask(request: TaskArchiveRequest): Promise<TaskArchiveResult> {
    let { uuid, reason = 'Archived for cleanup', preserveContent = true, options = {} } = request;
    let archiveLocation = this.archiveDir || path.join(this.tasksDir, '..', 'archive');

    try {
      // Read the current task
      let task = await this.contentManager['cache'].readTask(uuid);
      if (!task) {
        return {
          success: false,
          taskUuid: uuid,
      ...
      let archiveFileName = `${task.slug || uuid}-${Date.now()}.md`;
      let archiveFilePath = path.join(archiveLocation, archiveFileName);
  ...
  async deleteTask(request: TaskDeleteRequest): Promise<TaskDeleteResult> {
    let { uuid, confirm, force = false, options = {} } = request;
  ...
  async mergeTasks(request: TaskMergeRequest): Promise<TaskMergeResult> {
    let { sourceUuids, targetUuid, mergeStrategy = 'append', preserveSources = true, options = {} } = request;

    try {
      // Read all source tasks and target task
      let sourceTasks: Task[] = [];
      for (let uuid of sourceUuids) {
        let task = await this.contentManager['cache'].readTask(uuid);
        if (!task) {
          return {
            success: false,
      ...
      let targetTask = await this.contentManager['cache'].readTask(targetUuid);
      if (!targetTask) {
        return {
          success: false,
          mergedTasks: [],
      ...
      // Dry run mode
      if (options.dryRun) {
        let totalWordCount = sourceTasks.reduce((sum, task) => sum + (task.content?.split(/\s+/).length || 0), 0) +
      ...
      for (let sourceTask of sourceTasks) {
        if (!sourceTask.content) continue;

        let sourceWordCount = sourceTask.content.split(/\s+/).length;
  ...
  async bulkArchive(uuids: string[], reason?: string, options?: TaskLifecycleOptions): Promise<TaskArchiveResult[]> {
    let results: TaskArchiveResult[] = [];

    for (let uuid of uuids) {
      let result = await this.archiveTask({
        uuid,
  ...
  async bulkDelete(uuids: string[], confirm: boolean, force = false, options?: TaskLifecycleOptions): Promise<TaskDeleteResult[]> {
    let results: TaskDeleteResult[] = [];

    for (let uuid of uuids) {
      let result = await this.deleteTask({
        uuid,
        confirm,
        force,
        options
