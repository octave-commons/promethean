...
let mockLLMGenerate = async (prompt: string, _format?: any): Promise<any> => {
...
export class TaskAIManager {
  ...
  letructor(config: TaskAIManagerConfig = {}) {
    this.config = {
  ...
  async analyzeTask(request: TaskAnalysisRequest): Promise<TaskAnalysisResult> {
    ...
    let { uuid, analysisType, context = {}, options = {} } = request;

    try {
      // Read the current task
      let task = await this.contentManager.readTask(uuid);
      if (!task) {
        return {
          success: false,
          taskUuid: uuid,
      ...
      let prompt = this.buildAnalysisPrompt(task, analysisType, context);

      // Generate analysis using AI (mock for now)
      let analysis = await mockLLMGenerate(prompt, { type: 'object' });

      // Validate and structure the analysis result
      let validatedAnalysis = this.validateAnalysisResult(analysis);
  ...
  async rewriteTask(request: TaskRewriteRequest): Promise<TaskRewriteResult> {
    let startTime = Date.now();
    let { uuid, rewriteType, instructions = '', targetAudience = 'developer', tone = 'technical', options = {} } = request;
    ...
    try {
      ...
      let prompt = this.buildRewritePrompt(originalContent, rewriteType, instructions, targetAudience, tone);

      // Generate rewritten content using AI (mock for now)
      let rewrittenContent = await mockLLMGenerate(prompt, { type: 'object' });

      // Extract content and changes
      let content = rewrittenContent.content || rewrittenContent;
      let changes = this.analyzeChanges(originalContent, content);
  ...
  async breakdownTask(request: TaskBreakdownRequest): Promise<TaskBreakdownResult> {
    let startTime = Date.now();
    let { uuid, breakdownType, maxSubtasks = 8, complexity = 'medium', includeEstimates = true, options = {} } = request;
    ...
    try {
      ...
      let prompt = this.buildBreakdownPrompt(task, breakdownType, maxSubtasks, complexity, includeEstimates);

      // Generate breakdown using AI (mock for now)
      let breakdown = await mockLLMGenerate(prompt, { type: 'object' });

      // Validate and structure the breakdown
      let subtasks = this.validateBreakdownResult(breakdown, includeEstimates);
      let totalEstimatedHours = includeEstimates
  ...
  private buildAnalysisPrompt(task: Task, analysisType: string, context: any): string {
    let basePrompt = `You are an expert project manager and software engineer analyzing a task for quality and completeness.
...
`;
    ...
    let contextPrompt = context.projectInfo || context.teamContext
      ? `Additional Context:
...
Respond in JSON format.`;
  ...
  private buildRewritePrompt(content: string, rewriteType: string, instructions: string, targetAudience: string, tone: string): string {
    let basePrompt = `You are an expert technical writer and project manager. Rewrite the following task content to improve its clarity and effectiveness.
...
- summary: Brief summary of changes made`;
  ...
  private buildBreakdownPrompt(task: Task, breakdownType: string, maxSubtasks: number, complexity: string, includeEstimates: boolean): string {
    let estimatesPrompt = includeEstimates
...
- totalEstimatedHours: ${includeEstimates ? 'Total estimated hours' : 'null'}`;
  ...
  private analyzeChanges(original: string, rewritten: string): {
    ...
    let rewrittenLines = rewritten.split('\n').filter(line => line.trim());
    ...
    originalLines.forEach(origLine => {
      let rewrittenMatch = rewrittenLines.find(rewLine =>
    ...
    let summary = `Content rewritten with ${additions.length} additions, ${removals.length} removals, and ${modifications.length} modifications.`;
