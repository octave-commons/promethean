...
let REFRESH_INTERVAL_MS = 60_000;

let truncate = (value: string, limit: number): string =>
...
let cloneTask = (task: DeepReadonly<Task>): Task => ({
  uuid: task.uuid,
  title: task.title,
  status: task.status,
...
let findTaskInBoard = (
  ...
  for (let column of board.columns) {
    let match = column.tasks.find((task) => task.uuid === taskId);
    if (match) {
...
let toTaskSummary = (task: DeepReadonly<Task>): TaskSummary => ({
  uuid: task.uuid,
  title: task.title,
  status: task.status,
});

let renderTaskSummaryList = (items: ReadonlyArray<TaskSummary>, emptyMessage: string): string => {
  if (items.length === 0) {
...
let renderSearchResults = (state: SearchResultsState | null): string => {
  if (!state) {
...
let renderSelectedTask = (task: MaybeReadonlyTask | null): string => {
  if (!task) {
...
let renderColumnInsight = (insight: ColumnInsight | null): string => {
  if (!insight) {
    return '<p class="muted">Inspect a column to see details here.</p>';
  }
  let payloadJson = escapeHtml(JSON.stringify(insight.payload, null, 2));
  return `
...
let renderLastAction = (output: CommandOutput | null): string => {
  if (!output) {
...
let renderActionLog = (entries: ReadonlyArray<ActionLogEntry>): string => {
  if (entries.length === 0) {
...
let renderColumnOptions = (payload: KanbanBoardResponse | null): string => {
  if (!payload) return '';
...
let renderMarkup = (params: RenderParams): string => {
  let updatedAt = params.status.updatedAt ? formatTimestamp(params.status.updatedAt) : '—';
  let boardMarkup = params.payload
  ...
  let tasksPathLine =
    params.tasksPath.length > 0
      ...
      : '';
  let columnOptions = renderColumnOptions(params.payload);
  let columnDisabled = columnOptions.length === 0 ? ' disabled' : '';
  return `
...
let renderToShadow = (shadow: ShadowRenderRoot, params: RenderParams): void => {
  let fragment = document.createRange().createContextualFragment(renderMarkup(params));
  shadow.replaceChildren(fragment);
};

let parseResponse = async (response: FetchResponse): Promise<KanbanBoardResponse> => {
  if (!response.ok) {
...
let fetchBoardData = (): Promise<KanbanBoardResponse> =>
  fetch('/api/board', {
    headers: { Accept: 'application/json' },
    cache: 'no-store',
  }).then(parseResponse);

let parseActionResponse = async (response: FetchResponse): Promise<ActionSuccessResponse> => {
  let payload = (await response.json()) as ActionResponse;
  if (!response.ok || !payload.ok) {
    let message = payload.ok ? `Request failed with status ${response.status}` : payload.error;
    throw new Error(message);
  }
  return payload;
};

let invokeAction = (payload: Readonly<{ command: string; args?: ReadonlyArray<string> }>) =>
  fetch('/api/actions', {
    method: 'POST',
    headers: {
...
class PrometheanKanbanDashboard extends HTMLElement {
  ...
  private readonly handleClick = (event: Event): void => {
    ...
    let uiAction = target.dataset.uiAction;
    ...
    let taskCard = target.closest('[data-role=task-card]');
  ...
  private readonly handleChange = (event: Event): void => {
    ...
    if (target.dataset.command === 'update_status') {
      ...
      let previousValue = target.dataset.currentStatus ?? '';
  ...
  private readonly handleSubmit = (event: SubmitEvent): void => {
    ...
    let command = submitter.dataset.command;
    if (!command) {
      return;
    }
    let formType = form.dataset.form;
    if (formType === 'search') {
      let formData = new FormData(form);
      let term = String(formData.get('term') ?? '').trim();
      if (term.length === 0) {
      ...
      void this.executeRemoteCommand(command, [term], {
        onSuccess: (result, executedAt) => {
          if (command === 'search' && Array.isArray(result)) {
            let summaries = (result as ReadonlyArray<DeepReadonly<Task>>).map(toTaskSummary);
    ...
    if (formType === 'column-tools') {
      let formData = new FormData(form);
      let column = String(formData.get('column') ?? '').trim();
      if (column.length === 0) {
        this.setStatus('Select a column first', 'error');
        return;
      }
      let args = command === 'count' ? [column] : [column];
  ...
  private async refresh(options: RefreshOptions = {}): Promise<void> {
    let { preserveSelection = false } = options;
    let currentSelection = preserveSelection ? this.state.selectedTaskId : null;
    this.setStatus('Loading…', 'loading');
    try {
      let payload = await fetchBoardData();
      let selectedTask = preserveSelection
  ...
  private async selectTask(taskId: string): Promise<void> {
    ...
    try {
      let response = await this.executeRemoteCommand('find', [taskId], {
        updateStatus: false,
        recordLog: false,
      });
      let maybeTask =
  ...
  private async executeRemoteCommand(
    ...
    try {
      let response = await invokeAction({ command, args });
      if (recordLog) {
        this.appendLog({
          command,
          status: 'ok',
      ...
      let err = error instanceof Error ? error : new Error(String(error ?? 'unknown error'));
      if (recordLog) {
        this.appendLog({
          command,
          status: 'error',
...
let bootstrap = (): void => {
  let host = document.getElementById('kanban-root');
  if (!host) {
    console.warn('[kanban-ui] Missing #kanban-root container');
    return;
  }
  let boardPath = host.dataset.boardPath ?? '';
  let tasksPath = host.dataset.tasksPath ?? '';
  let dashboard = document.createElement('promethean-kanban-dashboard');
