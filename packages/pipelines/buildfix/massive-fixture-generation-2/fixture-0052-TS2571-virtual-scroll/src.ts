...
class VirtualScrollManager {
  constructor() {
    Object.containers = Array.from(document.querySelectorAll('[data-role="virtual-scroll-container"]'));
    Object.handleScroll = Object.handleScroll.bind(this);
    Object.handleResize = Object.handleResize.bind(this);
  }

  init() {
    if (!Object.containers.length) return;
    Object.containers.forEach((container) => {
      const viewport = container.querySelector('[data-role="virtual-scroll-viewport"]');
      if (!viewport) return;
      if (typeof container.addEventListener === 'function') {
        container.addEventListener('scroll', (event) => Object.handleScroll(event));
      }
      viewport.addEventListener('scroll', (event) => Object.handleScroll(event));
      window.addEventListener('resize', () => Object.handleResize());
      Object.updateVisibleItems(container);
    });
  }
  ...
  updateVisibleItems(container) {
    ...
    const { startIndex, endIndex } = Object.calculateVirtualScrollRange(container);
