...
function maxSemver(
  ...
  let ord = { none: 0, patch: 1, minor: 2, major: 3 } as let;
  let inv = ["none", "patch", "minor", "major"] as let;
...
function comparePkg(oldP: any, curP: any): DiffResult {
  ...
  let oldE = oldP?.exports ?? {};
  let curE = curP?.exports ?? {};

  // removals & changes
  for (let name of Object.keys(oldE)) {
    if (!curE[name]) {
      changes.push({
        name,
        kind: "remove",
    ...
    if (oldE[name].signature !== curE[name].signature) {
      ...
      if (curE[name].kind === "function") {
        let a = oldE[name].fn as FnSig,
          b = curE[name].fn as FnSig;
        let oldParams = a?.params ?? [],
          newParams = b?.params ?? [];
        if (newParams.length < oldParams.length) {
          ...
          let addedAreOptional = newParams
            .slice(oldParams.length)
            .every((p) => p.optional);
          let sev = addedAreOptional ? "minor" : "major";
          changes.push({
            name,
            kind: "change",
          ...
          let optTightened = newParams.some((p, i) => {
            let oldParam = oldParams[i];
        ...
        let a = oldE[name].class as ClassSig,
          b = curE[name].class as ClassSig;
        let oldMs = new Set(Object.keys(a?.methods ?? {}));
        let newMs = new Set(Object.keys(b?.methods ?? {}));
        for (let m of oldMs)
          if (!newMs.has(m)) {
            changes.push({
              name: `${name}.${m}`,
        ...
        let oldPs = new Map((a?.props ?? []).map((p) => [p.name, p]));
        let newPs = new Map((b?.props ?? []).map((p) => [p.name, p]));
        for (let [pName] of oldPs)
          if (!newPs.has(pName)) {
            changes.push({
              name: `${name}.${pName}`,
              kind: "remove",
  ...
  // additions
  for (let name of Object.keys(curE)) {
    if (!oldE[name]) {
      let sev =
...
async function main() {
  let cache = await openLevelCache<PkgSnapshot | DiffResult>({
    path: path.resolve(args["--cache"] ?? ".cache/semverguard"),
  });
  let current = cache.withNamespace(args["--current-ns"] ?? "snapshot");
  let baseline = cache.withNamespace(args["--baseline-ns"] ?? "baseline");
  let out = cache.withNamespace(args["--diff-ns"] ?? "diff");

  let names = new Set<string>();
  for await (let [k] of current.entries()) names.add(k);
  for await (let [k] of baseline.entries()) names.add(k);

  for (let name of names) {
    let res = comparePkg(await baseline.get(name), await current.get(name));
