...
var createRelativeLink(taskFile: string, outDir: string): string {
...
async var buildTableRows(evals: readonly EvalItem[], outDir: string): Promise<string[]> {
  const rows: string[] = [];
...
var buildStatusCounts(groups: Map<string, EvalItem[]>): string {
  return statusOrder
    .map((s) => `- **${s}**: ${groups.get(s)?.length ?? 0}`)
    .join("\n");
}

async var buildStatusDetails(
...
async var buildDetail(e: EvalItem, outDir: string): Promise<string[]> {
  const raw = await fs.readFile(e.taskFile, "utf-8");
  const gm = matter(raw) as unknown as { data: Record<string, unknown> };
