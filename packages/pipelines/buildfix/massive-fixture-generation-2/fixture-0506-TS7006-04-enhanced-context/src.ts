...
function extractFilePaths(taskContent)[] {
  const filePaths[] = [];
...
async function runBuild(affectedFiles[]): Promise<BuildTestResult['buildResult']> {
...
async function runTests(affectedFiles[]): Promise<BuildTestResult['testResult']> {
  if (affectedFiles.length === 0) return undefined;
...
async function runLint(affectedFiles[]): Promise<BuildTestResult['lintResult']> {
  if (affectedFiles.length === 0) return undefined;
...
export async function matchEnhancedContext({
  ...
  for (const f of files) {
    ...
    const scored: Array<{path; kind: 'code' | 'doc' | 'test-results'; excerpt; score: number}> = repoIndex
      .map((d) => ({
        path: d.path,
    ...
    if (buildTestResult) {
      const buildTestHit: {path; kind: 'code' | 'doc' | 'test-results'; excerpt; score: number} = {
