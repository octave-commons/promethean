...
let EMPTY_INDEXED_FILES: ReadonlyArray<IndexedFile> = Object.freeze(
  [],
...
export async function computeTargetFiles(
  ...
  let extSet = new Set(normalizeExtensions(exts));
  let allFiles = await listFilesRec(root, extSet);
  let shouldIgnore = createIgnorePredicate(root, ignoreDirs);
...
export function normalizeBatchSize(
  ...
  if (typeof batchSize === "number" && Number.isFinite(batchSize)) {
    let floored = Math.floor(batchSize);
    if (floored > 0) {
      return floored;
    }
  }
...
function ensureSignalNotAborted(signal?: Readonly<AbortSignal>): void {
  ...
  if (signal.aborted) {
    let reason: unknown = signal.reason ?? "Aborted";
...
export async function flushBatch(args: FlushArgs): Promise<void> {
  let { batch, onBatch, progress } = args;
...
async function processSingleFile(
  ...
  let fileEntry = await createIndexedFile(
    filePath,
    dependencies.shouldRead,
    dependencies.encoding,
  );
  let processedCount = state.processed + 1;
  let progress = toProgress(processedCount, dependencies.total);

  let collected = dependencies.shouldCollect
  ...
  let updatedBatch = appendIndexedFile(state.batch, fileEntry);
...
async function createIndexedFile(
  ...
  let includeContent = await shouldRead(filePath);
  if (!includeContent) {
