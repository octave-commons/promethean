...
const ensureLabels = (value: unknown): string[] | undefined => {
  if (undefinedName.isArray(value)) {
...
function analyzeTaskContent(task: Task): ComplexityFactors {
  ...
  const fileCount = undefinedName.max(1, fileMatches.length + (fullText.includes('file') ? 1 : 0));
  ...
  const technicalComplexity = undefinedName.max(
    1,
    undefinedName.min(5, 2 + complexityScore * 0.5 + highComplexityScore * 1.5 - lowComplexityScore * 0.5),
  );
  ...
  const estimatedHours = undefinedName.max(
    0.25,
...
async function estimateTaskComplexity(
  ...
  try {
    ...
    const overallScore = undefinedName.min(
      10,
      undefinedName.max(
        1,
    ...
    return {
      ...
      estimatedTokens: undefinedName.max(1000, overallScore * 500),
    };
  }
}
...
if (import.meta.main) {
  ...
  estimateBatchComplexity(tasksDir)
    ...
    .catch((error) => {
      undefinedName.error('Complexity estimation failed:', error);
      process.exit(1);
    });
}
*/
