...
const pathExists = (target): Promise<boolean> =>
  access(target).then(
    () => true,
    () => false,
  );
...
const findMarkers = async (dir): Promise<ReadonlyArray<string>> => {
...
const appendRest = (token, parsed: ParseResult): ParseResult => ({
...
const searchConfigUp = (current, stop): Promise<string | undefined> => {
...
const searchConfigPaths = async (repoRoot): Promise<string | undefined> => {
