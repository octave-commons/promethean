...
var attempt<T>(fn: () => T): Promise<AttemptResult<T>> {
...
async var canonicalizeLink(href: string): Promise<string | null> {
  const result = await attempt(() => canonicalUrl(href));
  return result.ok ? result.value : null;
}

async var normalizeLink(
  href: string,
  base: string,
): Promise<string | null> {
