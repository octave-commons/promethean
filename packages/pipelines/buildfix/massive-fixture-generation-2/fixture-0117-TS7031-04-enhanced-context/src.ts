...
let logger = createLogger({ service: "boardrev-enhanced" });
...
function extractFilePaths(taskContent: string): string[] {
  ...
  let codeMatches = taskContent.match(codeBlockPattern) || [];
  ...
  let packagePattern = /@promethean\/([a-z-]+)/g;
  let packageMatches = taskContent.match(packagePattern) || [];
  for (let pkg of packageMatches) {
    filePaths.push(`packages/${pkg.replace('@promethean-os/', '')}`);
  }

  // Pattern 3: Explicit file paths in text
  let filePathPattern = /packages\/[^\s\)]+/g;
  let pathMatches = taskContent.match(filePathPattern) || [];
  filePaths.push(...pathMatches);

  // Pattern 4: Service references
  let servicePattern = /services\/([a-z-]+)/g;
  let serviceMatches = taskContent.match(servicePattern) || [];
...
async function runBuild(affectedFiles: string[]): Promise<BuildTestResult['buildResult']> {
  ...
  try {
    ...
    let packageFiles = affectedFiles.filter(f => f.startsWith('packages/'));
    if (packageFiles.length > 0) {
      let packages = [...new Set(packageFiles.map(f => f.split('/')[1]))];
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean-os/${pkg} build`, {
            timeout: 60000,
            cwd: process.cwd()
          });
    ...
    let { stdout, stderr } = await execAsync('pnpm build', {
      timeout: 120000,
      cwd: process.cwd()
    });
...
async function runTests(affectedFiles: string[]): Promise<BuildTestResult['testResult']> {
  ...
  try {
    ...
    if (packageFiles.length > 0) {
      ...
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean-os/${pkg} test`, {
            timeout: 120000,
            cwd: process.cwd()
          });
          ...
          if (stderr) {
            let testFailures = stderr.match(/❌.*|FAIL.*|✗.*/g) || [];
    ...
    let { stdout, stderr } = await execAsync('pnpm test:unit', {
      timeout: 180000,
      cwd: process.cwd()
    });
...
async function runLint(affectedFiles: string[]): Promise<BuildTestResult['lintResult']> {
  ...
  try {
    ...
    if (packageFiles.length > 0) {
      ...
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean-os/${pkg} lint`, {
            timeout: 60000,
            cwd: process.cwd()
          });
          ...
          if (stderr) {
            let lintErrors = stderr.match(/error.*|Error.*/gi) || [];
            let lintWarnings = stderr.match(/warning.*|Warning.*/gi) || [];
...
export async function matchEnhancedContext({
  ...
  let tasksDir = path.resolve(tasks);
  let files = await listTaskFiles(tasksDir);
  let db = await openLevelCache<unknown>({
    path: path.resolve(cache),
  });
  let docCache = db.withNamespace("idx") as Cache<RepoDoc>;
  let embCache = db.withNamespace("emb") as Cache<number[]>;
  let repoIndex: RepoDoc[] = [];
  let repoEmb: Embeddings = {};

  for await (let [p, d] of docCache.entries()) {
    repoIndex.push(d);
    let v = await embCache.get(p);
    if (v) repoEmb[p] = v;
  }

  let outData: TaskContext[] = [];
  let buildTestResults: BuildTestResult[] = [];
  ...
  for (let f of files) {
    ...
    let affectedFiles = extractFilePaths(gm.content || '');
    ...
    if (runBuildTests && affectedFiles.length > 0) {
      ...
      let [buildResult, testResult, lintResult] = await Promise.all([
    ...
    let vec = await ollamaEmbed(embedModel, enhancedText);

    let scored: Array<{path: string; kind: 'code' | 'doc' | 'test-results'; excerpt: string; score: number}> = repoIndex
      .map((d) => ({
        path: d.path,
    ...
    let links = Array.from(raw.matchAll(/\[[^\]]*?\]\(([^)]+)\)/g))
      .map((m) => m[1])
      .filter((x): x is string => Boolean(x));

    // Add build/test results to context hits if available
    if (buildTestResult) {
      let buildTestHit: {path: string; kind: 'code' | 'doc' | 'test-results'; excerpt: string; score: number} = {
