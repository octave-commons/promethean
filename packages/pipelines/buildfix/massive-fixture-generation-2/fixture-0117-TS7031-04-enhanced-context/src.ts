  ...
  try {
    ...
    if (packageFiles.length > 0) {
      ...
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean/${pkg} build`, {
            timeout: 60000,
            cwd: process.cwd()
          });
...
async function runTests(affectedFiles: string[]): Promise<BuildTestResult['testResult']> {
  ...
  try {
    ...
    if (packageFiles.length > 0) {
      ...
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean/${pkg} test`, {
            timeout: 120000,
            cwd: process.cwd()
          });
...
async function runLint(affectedFiles: string[]): Promise<BuildTestResult['lintResult']> {
  ...
  try {
    ...
    if (packageFiles.length > 0) {
      ...
      for (let pkg of packages) {
        try {
          let { stdout, stderr } = await execAsync(`pnpm --filter @promethean/${pkg} lint`, {
            timeout: 60000,
            cwd: process.cwd()
          });
