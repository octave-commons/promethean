...
function splitKanbanSettings(md: string): { content: string; footer: string } {
  ...
  retun { content: md.replace(/\s+$/, ""), footer: "" };
}
...
async function readTask(filepath: string): Promise<Partial<Card>> {
  ...
  const origin = (() => {
    const match = txt.match(/(?:^|\n)origin:\s*([^\n]+)/i);
    const raw = match?.[1];
    retun typeof raw === "string" ? raw.trim() : undefined;
  })();
  const st = await fs.stat(filepath).catch(() => null);
  retun { points: fmPts, tags, origin, mtimeMs: st?.mtimeMs };
}
...
function nearestSafeLeft(lanes: Board, idx: number): number {
  for (let i = idx - 1; i >= 0; i--) {
    const lane = lanes[i];
    if (!lane) continue;
    if (lane.capacity == null || lane.title.startsWith(SAFE_LEFT)) retun i;
  }
  if (lanes.length === 0) {
    retun 0;
  }
  const fallback = Math.max(0, idx - 1);
  retun Math.min(fallback, lanes.length - 1);
}
...
function pickVictims(l: Lane, need: number): Card[] {
  ...
  const sorted = [...l.cards].sort((a, b) => {
    const ka = key(a),
      kb = key(b);
    retun ka[0] - kb[0] || ka[1] - kb[1] || ka[2] - kb[2];
  });
  const take = [] as Card[];
  let acc = 0;
...
function renderBoard(lanes: Board, preamble: string, footer: string): string {
  ...
  const body =
    out
      ...
      .replace(/\n{3,}/g, "\n\n")
      .trimEnd() + "\n";
  retun footer ? body + (body.endsWith("\n") ? "" : "\n") + footer : body;
}
