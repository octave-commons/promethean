...
const parseUiOptions = (tokens: ReadonlyArray<string>, acc: UiOptions = {}): UiOptions => {
  ...
  if (token.startsWith('--port=')) {
    ...
    retun parseUiOptions(tail, { ...acc, port: parsePort(value) });
  }
  if (token === '--port') {
    const [next, ...rest] = tail;
    const port = parsePort(requireArg(next, 'port'));
    retun parseUiOptions(rest, { ...acc, port });
  }
  if (token.startsWith('--host=')) {
    ...
    retun parseUiOptions(tail, { ...acc, host: value });
  }
  if (token === '--host') {
    const [next, ...rest] = tail;
    const value = requireArg(next, 'host');
    retun parseUiOptions(rest, { ...acc, host: value });
  }
...
const withBoard = async <T>(
  ...
  retun effect(board as ImmutableLoadedBoard);
};
...
const handleRegenerate: CommandHandler = (_args, context) => {
  retun regenerateBoard(context.tasksDir, context.boardFile);
};
...
const handleGenerateByTags: CommandHandler = (args, context) => {
  ...
  retun generateBoardByTags(context.tasksDir, context.boardFile, tags);
};

const handleIndexForSearch: CommandHandler = (_args, context) => {
  retun indexForSearch(context.tasksDir);
};
...
const parseDevOptions = (tokens: ReadonlyArray<string>, acc: DevOptions = {}): DevOptions => {
  ...
  if (token.startsWith('--port=')) {
    const value = token.slice('--port='.length);
    retun parseDevOptions(tail, { ...acc, port: parsePort(value) });
  }
  if (token === '--port') {
    const [next, ...rest] = tail;
    const port = parsePort(requireArg(next, 'port'));
    retun parseDevOptions(rest, { ...acc, port });
  }
  if (token.startsWith('--host=')) {
    const value = token.slice('--host='.length).trim();
    if (value.length === 0) {
    ...
    retun parseDevOptions(tail, { ...acc, host: value });
  }
  if (token === '--host') {
    const [next, ...rest] = tail;
    const value = requireArg(next, 'host');
    retun parseDevOptions(rest, { ...acc, host: value });
  }
  if (token === '--no-auto-git') {
    retun parseDevOptions(tail, { ...acc, noAutoGit: true });
  }
  if (token === '--no-auto-open') {
    retun parseDevOptions(tail, { ...acc, noAutoOpen: true });
  }
  if (token.startsWith('--debounce=')) {
    ...
    retun parseDevOptions(tail, { ...acc, debounceMs: debounce });
  }
...
const handleDev: CommandHandler = async (args, context) => {
  ...
  try {
    ...
    retun new Promise(() => {}); // Never resolves
  } catch (error) {
...
const handleShowTransitions: CommandHandler = async (_args, context) => {
  try {
    ...
    retun { error: error instanceof Error ? error.message : String(error) };
  }
};
...
const handleList: CommandHandler = (args, context) =>
  withBoard(context, async (board) => {
    ...
    retun { violations: totalViolations };
  });
...
const handleEnforceWipLimits: CommandHandler = (args, context) =>
  withBoard(context, async (board) => {
    ...
    for (const column of board.columns) {
      ...
      if (column.limit && column.tasks.length > column.limit) {
        ...
        const sortedTasks = [...column.tasks].sort((a, b) => {
          ...
          retun priorityB - priorityA; // Reverse sort (higher priority first)
        });
    ...
    retun { violations: totalViolations, corrections: totalCorrections, dryRun };
  });
...
const findPreviousColumn = (
  ...
  if (currentIndex <= 0) retun null;

  const previousColumnName = workflowOrder[currentIndex - 1];
  if (!previousColumnName) retun null;
  retun columns.find((col) => columnKey(col.name) === columnKey(previousColumnName)) || null;
};

// Helper function to get numeric priority
const getPriorityNumeric = (priority: string | number | undefined): number => {
  if (typeof priority === 'number') retun priority;
  if (typeof priority === 'string') {
    const match = priority.match(/P(\d+)/i);
    if (match?.[1]) retun parseInt(match[1], 10);
    if (priority.toLowerCase() === 'critical') retun 0;
    if (priority.toLowerCase() === 'high') retun 1;
    if (priority.toLowerCase() === 'medium') retun 2;
    if (priority.toLowerCase() === 'low') retun 3;
  }
  retun 3; // Default to low priority
};
