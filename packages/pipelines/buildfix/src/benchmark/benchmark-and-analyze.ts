...
class BenchmarkOrchestrator {
  ...
  async runBenchmarkAndAnalysis(config: BenchmarkConfig): Promise<void> {
    console.log('🚀 BuildFix Benchmark & Analysis Orchestrator');
    console.log('==========================================');
    ...
    try {
      ...
      if (config.analyzeOnly && config.reportPath) {
        // Analysis only mode
        console.log('📊 Analysis-only mode detected');
      ...
      console.log('\n✅ Benchmark and analysis completed successfully!');
    } catch (error) {
  ...
  private async runAnalysis(reportPath: string, analysisModel?: string): Promise<void> {
    ...
    try {
      ...
      if (analysisMatch) {
        console.log(`📝 Analysis report generated: ${analysisMatch[1]}`);
      }
    } catch (error) {
...
## 🤖 Key AI Insights
${analysisData.split('## 📊 Key Findings')[1]?.split('##')[0] || 'Insights not available'}

## 💡 Top Recommendations
${analysisData.split('## 💡 Recommendations')[1]?.split('##')[0] || 'Recommendations not available'}
...
Examples:
  ...
  tsx benchmark-and-analyze.ts --analysis-model qwen3:14b
    ...
    if (args.values['analyze-only'] && !args.values.report) {
      console.error('❌ --analyze-only requires --report to be specified');
      process.exit(1);
    }
