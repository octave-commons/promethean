...
// Security: Validate GitHub CLI arguments to prevent injection
function validateGhArgs(args: readonly string[]): void {
  const dangerousPatterns = /[;&|`$(){}[\]\\]/;
  for (const arg of args) {
    if (dangerousPatterns.test(arg)) {
      throw new Error(`Dangerous characters in GitHub CLI argument: ${arg}`);
    }
  }
}

async function runGhCommand(args: readonly string[]): Promise<string | undefined> {
  // Security: Validate arguments before execution
  validateGhArgs(args);

  const child = spawn('gh', args, { stdio: ['ignore', 'pipe', 'inherit'] });

  const stdoutPromise = child.stdout ? text(child.stdout).catch(() => '') : Promise.resolve('');
...
export async function commitIfChanges(message: string): Promise<string | undefined> {
...
export async function pushBranch(branch: string, remote: string): Promise<boolean> {
