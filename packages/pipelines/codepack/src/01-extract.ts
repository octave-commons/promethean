...
import { relPath, sha1 } from './utils.js';
...
export async function collectCodeBlocks(options: ExtractOptions): Promise<readonly CodeBlock[]> {
    ...
    onFile: async (file: IndexedFile) => {
      ...
      const extracted = extractCodeBlocksWithContext(content).map((block) => {
        ...
        const base: CodeBlock = {
          id: sha1([abs, block.startLine, block.endLine, sha1(block.value)].join('|')),
          srcPath: abs,
          relPath: rel,
...
export async function extractAndCache(options: CacheOptions): Promise<number> {
  ...
  await cache.batch(blocks.map((b) => ({ type: 'put' as const, key: b.id, value: b })));
  await cache.close();
