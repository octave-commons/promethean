...
export async function scan(options: { root?: string; cache?: string } = {}): Promise<void> {
  ...
  await cache.close();
  console.log(`readmeflow: scanned ${pkgMap.size} packages → ${options.cache ?? '.cache/readmes'}`);
}
