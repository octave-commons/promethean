...
import { CoreMessage, MessageType, AgentAddress, Priority } from '../core/types';
import { z } from 'zod';
...
export class AgentBusAdapter {
  private crisisHandlers = new Map<string, (msg: CrisisMessage) => Promise<void>>();
  ...
  fromAgentBus(busMessage: AgentBusMessage): CrisisMessage {
    ...
    return {
      ...
      affectedAgents: this.identifyAffectedAgents(crisisType, busMessage.payload),
  ...
  private identifyAffectedAgents(crisisType: CrisisMessageType, payload: any): AgentAddress[] {
...
export class CrisisCoordinator {
   ...
   */
  async distributeWorkload(crisisId: string, tasks: any[]): Promise<WorkDistribution> {
  ...
  private async handleSystemEmergency(message: CrisisMessage, session: CrisisSession): Promise<void> {
  ...
  private async routeToHandlers(message: CrisisMessage, session: CrisisSession): Promise<void> {
  ...
  private async trackRequiredResponses(message: CrisisMessage, session: CrisisSession): Promise<void> {
  ...
  private calculateOptimalDistribution(tasks: any[], agents: AgentInfo[]): WorkDistribution {
  ...
  private findSuitableAgent(task: any, agents: AgentInfo[], startIndex: number): AgentInfo | null {
