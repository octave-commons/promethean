...
import type { EventRecord } from "@promethean-os/event";
...
const ensureExchange = async (channel: Channel | ConfirmChannel, binding: {
  name: string;
  type?: string;
  durable?: boolean;
}): Promise<void> => {
...
const ensureQueue = async (channel: Channel, name: string, options: Parameters<Channel["assertQueue"]>[1]): Promise<void> => {
...
const parseEnvelope = <T>(msg: ConsumeMessage): RabbitEnvelope<T> => {
  ...
  if (msg.properties.contentType === JSON_CONTENT_TYPE || msg.properties.contentType === undefined) {
    try {
...
const publishWithConfirm = async (
  ...
  await new Promise<void>((resolve, reject) => {
    channel.publish(exchangeName, routingKey, content, properties, (err: Error | null | undefined) => {
      if (err) {
        reject(err);
      } else {
        resolve();
...
export const createRabbitContext = (options: RabbitContextOptions = {}): RabbitContext => {
  ...
  const publish: RabbitContext["publish"] = async (topic, payload, opts) => {
    ...
    const exchangeName = opts?.exchange ?? config.exchange?.name ?? `${config.namespace ?? DEFAULT_NAMESPACE}.events`;
  ...
  const subscribe: RabbitContext["subscribe"] = async (binding, handler) => {
    ...
    const exchangeName = binding.exchange ?? config.exchange?.name ?? `${config.namespace ?? DEFAULT_NAMESPACE}.events`;
    await ensureExchange(channel, {
      name: exchangeName,
    ...
    const queueOptions = binding.queue
      ? {
          ...
          deadLetterRoutingKey:
            binding.deadLetterRoutingKey ?? `${queueName}.dlq`,
          ...(config.defaultQueue ?? {}),
    ...
    const consumer = await channel.consume(
      ...
      async (msg: ConsumeMessage | null) => {
        ...
        try {
          ...
          instrumentation.onDelivery?.({ queue: queueName, routingKey: msg.fields.routingKey, topic: envelope.topic });
