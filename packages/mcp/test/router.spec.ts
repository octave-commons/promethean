...
test('attachRouter emits progress, result, error and cleans up on close', async (t) => {
    ...
    bridgeSocket.send(JSON.stringify({ kind: 'tool.chunk', id: '1', data: { step: 1 } }));
    bridgeSocket.send(JSON.stringify({ kind: 'tool.result', id: '2', result: { done: true } }));
    bridgeSocket.send(JSON.stringify({ kind: 'tool.error', id: '3', error: 'boom' }));
    ...
    await new Promise<void>((res, rej) => bridgeServer.close((err) => (err ? rej(err) : res())));
});
