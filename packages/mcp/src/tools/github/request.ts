  ...
  const invoke = async (args: any) => {
    ...
    if (!args.paginate) {
      ...
      if (res.status === 304 && ctx.cache) {
        ...
        return { status: 200, headers: {}, data: JSON.parse(new TextDecoder().decode(body)) };
      }
    ...
    while (page <= max) {
      ...
      const res = await ctx.fetch(url, { method: "GET", headers } as RequestInit);
      const text = await res.text();
      let data: any;
      try { data = text ? JSON.parse(text) : null; } catch { data = text; }
      if (!Array.isArray(data) || res.status >= 400) return { page, status: res.status, data };
      pages.push(...data);
