...
const createGithubGraphqlClient = (ctx: ToolContext) => {
  ...
  return async <T>({
    ...
    if (!res.ok) {
      throw new Error(
        `[github.review] GraphQL request failed with ${
          res.status
...
const getPullRequestId = async (
  ...
  if (!pr) {
    throw new Error(
      `[github.review] Pull request not found: ${owner}/${name}#${number}`,
    );
  }
  return pr.id;
};
...
export const githubReviewOpenPullRequest: ToolFactory = (ctx) => {
  ...
  return {
    spec: {
      name: "github.review.openPullRequest",
      description:
...
export const githubReviewGetComments: ToolFactory = (ctx) => {
  ...
  return {
    spec: {
      name: "github.review.getComments",
      description:
...
export const githubReviewGetReviewComments: ToolFactory = (ctx) => {
  ...
  return {
    spec: {
      name: "github.review.getReviewComments",
      description:
...
export const githubReviewSubmitComment: ToolFactory = (ctx) => {
  ...
  return {
    ...
    invoke: async (raw: unknown) => {
      ...
      if (!node) {
        throw new Error(
          "[github.review] Comment submission succeeded without comment node",
        );
      }
      return node;
    },
...
const ReviewCommentInputSchema = z
  ...
  .refine(
    (value) =>
      typeof value.position === "number" || typeof value.line === "number",
    {
...
export const githubReviewSubmitReview: ToolFactory = (ctx) => {
  ...
  return {
    spec: {
      name: "github.review.submitReview",
      description:
    ...
    invoke: async (raw: unknown) => {
      ...
      if (!review) {
        throw new Error(
          "[github.review] Review submission did not return review data",
        );
      }
      return review;
    },
...
export const githubReviewGetActionStatus: ToolFactory = (ctx) => {
  ...
  return {
    spec: {
      name: "github.review.getActionStatus",
      description:
...
export const githubReviewCreateBranch: ToolFactory = () => {
  ...
  return {
    spec: {
      name: "github.review.createBranch",
      description:
