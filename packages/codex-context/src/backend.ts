...
export class OllamaBackend implements BackendClient {
  ...
  > {
    ...
    try {
      ...
      if (typeof res?.prompt_eval_count === "number") {
        usage.prompt_tokens = res.prompt_eval_count;
      }
      if (typeof res?.eval_count === "number") {
        usage.completion_tokens = res.eval_count;
      }
      if (
        typeof res?.eval_count === "number" &&
        typeof res?.prompt_eval_count === "number"
      ) {
        usage.total_tokens = res.eval_count + res.prompt_eval_count;
      }
...
export class OllamaOpenAIBackend implements BackendClient {
  ...
  constructor(
    ...
    this.baseUrl =
      baseUrl || process.env.OLLAMA_OPENAI_BASE || "http://127.0.0.1:11434/v1";
  ...
  > {
    const base =
      (cfg && "baseUrl" in (cfg as any) && (cfg as any).baseUrl) ||
      this.baseUrl;
    const apiKey =
      (cfg && "apiKey" in (cfg as any) && (cfg as any).apiKey) || this.apiKey;
    ...
    try {
      ...
      const t1 = process.hrtime.bigint();
      const text =
        js?.choices?.[0]?.message?.content || js?.choices?.[0]?.text || "";
      const ret: {
        text: string;
        usage?: {
          prompt_tokens?: number;
