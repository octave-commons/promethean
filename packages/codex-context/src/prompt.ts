...
export function buildAugmentedPrompt(
  ...
  const maxCtx =
    opts?.maxContextTokens ?? Number(process.env.MAX_CONTEXT_TOKENS || 1024);
  let ctxTokenCount = 0;
  for (const hit of retrieved.search.slice(0, 32)) {
    const cite: { path: string; startLine?: number; endLine?: number } = {
      path: hit.path,
    };
    if (typeof hit.startLine === "number") cite.startLine = hit.startLine;
    if (typeof hit.endLine === "number") cite.endLine = hit.endLine;
    citations.push(cite);
