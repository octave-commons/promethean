...
export class SmartGptrRetriever implements Retriever {
	...
	constructor(
		...
		// @ts-ignore
		this.fetcher =
			fetchImpl || (globalThis as any).fetch || (nodeFetch as any);
	}
	...
	async retrieve(
		...
		try {
			...
			out.search = results.map((r: any) => ({
				...
				startLine:
					Number(r.startLine || r.metadata?.startLine || 1) ||
					undefined,
				endLine:
					Number(r.endLine || r.metadata?.endLine || 1) || undefined,
			}));
