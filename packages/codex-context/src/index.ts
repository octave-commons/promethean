...
export function createApp(deps: AppDeps = {}): Express {
	...
	const SMARTGPT_TOKEN =
		process.env.SMARTGPT_TOKEN || process.env.SMARTGPT_BEARER;
	const retriever =
		deps.retriever || new SmartGptrRetriever(SMARTGPT_URL, SMARTGPT_TOKEN);
	...
	function parseGenParams(body: any): Partial<BackendConfig> {
		const cfg: Partial<BackendConfig> = {
			model: MODEL,
	...
	app.get("/health", (req, res) => {
		const dur =
			req && (req as any)._start
				? Number(
						(process.hrtime.bigint() - (req as any)._start) /
							1000000n,
					)
				: 0;
	...
	app.post("/api/pull", async (req, res): Promise<void> => {
		const name = (req.body?.name || req.body?.model || "").trim?.() || "";
		const stream = Boolean(req.body?.stream);
		if (!name)
			return void res
				.status(400)
				.json({ error: { message: "name (or model) is required" } });
		try {
			if (stream) {
			...
			return void res
				.status(502)
				.json({ error: { message: e?.message || String(e) } });
		}
	});
	...
	app.post("/v1/chat/completions", async (req, res) => {
		...
		const sessionId =
			(body as any)?.session_id && String((body as any).session_id);
		const prior: any[] =
			sessionId && sessions.get(sessionId)
				? [...(sessions.get(sessionId) as any[])]
				: [];
		const mergedMessages = prior.length
			? [...prior, ...body.messages]
			: body.messages;
		...
		const text =
			typeof chatRes === "string" ? chatRes : (chatRes?.text ?? "");
		...
		const usage: Usage = {
			prompt_tokens:
				(typeof chatRes === "object" &&
					chatRes?.usage?.prompt_tokens) ||
				(estimateTokens(promptJoined) as any),
			completion_tokens:
				(typeof chatRes === "object" &&
					chatRes?.usage?.completion_tokens) ||
		...
		try {
			...
			const saveArgs: Parameters<typeof persistArtifact>[0] = {
				request: body,
			...
			if (deps.docsDir) (saveArgs as any).baseDir = deps.docsDir;
			await persistArtifact(saveArgs);
	...
	app.post("/v1/completions", async (req, res) => {
		...
		span.debug("request.body", {
			has_prompt: Boolean(body?.prompt),
			model: body?.model,
		});
