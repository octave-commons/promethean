 ...
 */
export const OpencodeInterfacePlugin: Plugin = async (pluginContext: PluginInput) => {
  // Initialize stores on plugin load
  await initializeStores();

  // Use OpenCode client from plugin context
  const opencodeClient = pluginContext.client as OpencodeClient;
      ...
      'get-session': tool({
        ...
        async execute(args) {
          try {
            ...
            const limit =
              args.limit !== undefined ? validate.number(args.limit, 'limit') : undefined;
            const offset =
