...
class APIClient {
  ...
  async getLLMMessages(actorId: string): Promise<ChatMessage[]> {
    const response = await this.client.get<APIResponse<ChatMessage[]>>(`/actors/${actorId}/messages`)
    return response.data.data || []
  }

  async sendLLMMessage(actorId: string, message: string): Promise<ChatMessage> {
    const response = await this.client.post<APIResponse<ChatMessage>>(`/actors/${actorId}/messages`, {
      content: message,
    })
    return response.data.data!
  }
  ...
  async executeMCPTool(toolName: string, args: Record<string, unknown>): Promise<MCPToolExecution> {
    const response = await this.client.post<APIResponse<MCPToolExecution>>(`/tools/${toolName}/execute`, args)
    return response.data.data!
  }
