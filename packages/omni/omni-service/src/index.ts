  ...
  try {
    ...
    const adapters = Object.entries(config.adapters)
      ...
      .map(([name, adapter]) => ({
        ...
        endpoint:
          name === 'rest'
            ? `${(adapter as any).prefix}/${(adapter as any).version}`
            : name === 'graphql'
              ? (adapter as any).endpoint
              : name === 'websocket'
                ? (adapter as any).path
                : name === 'mcp'
                  ? (adapter as any).prefix
                  : '',
        ...adapter,
      }));
    ...
    if (config.adapters.rest?.enabled) {
      console.log(
        `   🌐 REST API: http://${host}:${port}${config.adapters.rest.prefix}/${config.adapters.rest.version}`,
      );
    }
    if (config.adapters.graphql?.enabled) {
      ...
      if (config.adapters.graphql.playground) {
        console.log(
          `   🎮 GraphQL Playground: http://${host}:${port}${config.adapters.graphql.endpoint}/playground`,
        );
      }
    }
