...
test('should register and execute before hooks', async (t) => {
    ...
    const result = await hookManager.executeBeforeHooks('test-tool', { test: 'value' }, { pluginContext: {} });
    t.is(called, true);
...
test('should register and execute after hooks', async (t) => {
    ...
    const result = await hookManager.executeAfterHooks('test-tool', { test: 'value' }, { success: true, result: { success: true }, metadata: {}, executionTime: 100 }, { pluginContext: {} });
    t.is(called, true);
...
test('should handle hook execution errors', async (t) => {
    ...
    const result = await hookManager.executeBeforeHooks('test-tool', { test: 'value' }, { pluginContext: {} }, { continueOnError: true });
    t.is(result.metrics.length, 1);
...
test('should respect tool pattern matching', async (t) => {
    let called = false;
    registerBeforeHook('pattern-hook', async () => {
        called = true;
    }, { tools: ['test-*'] });
    // Should match
...
//# sourceMappingURL=hook-system.test.js.map