...
unifiedAgentCommands
  ...
  .option('-m, --message <message>', 'Initial message to send to agent')
  .option('-t, --title <title>', 'Session title')
  .option('-f, --files <files...>', 'Files to include in session')
  .option('-d, --delegates <delegates...>', 'Delegates to assign to session')
  .option('-p, --priority <priority>', 'Task priority (low, medium, high, urgent)', 'medium')
  .option('--auto-start', 'Automatically start agent session', false)
...
unifiedAgentCommands
  ...
  .action(async (sessionId, options) => {
    try {
      ...
      console.log(`✅ Agent session stopped successfully!`);
    } catch (error) {
      console.error(`❌ Failed to stop session: ${(error as Error).message}`);
      process.exit(1);
    }
  });
