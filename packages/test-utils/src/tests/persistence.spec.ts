...
test('countDocuments respects filters and operators', async (t) => {
    ...
    t.is(await c.countDocuments({ 'user.age': { $gte: 30 } as any }), 2);
    t.is(await c.countDocuments({ 'user.age': { $lt: 30 } as any }), 1);
    t.is(await c.countDocuments({ body: { $exists: true } as any }), 2);
    t.is(await c.countDocuments({ body: { $exists: false } as any }), 1);
    t.is(await c.countDocuments({ body: { $not: /hello/ } as any }), 2);
    t.is(await c.countDocuments({ 'user.age': { $nin: [45] } as any }), 2);
});
...
test('updateMany applies to all matching documents', async (t) => {
    ...
    await c.updateMany({ 'user.age': 30 } as any, { $set: { body: 'x' } } as any);
    const all = await c.find({}).toArray();
    const bodies = Object.fromEntries(all.map((d: any) => [d.id, d.body]));
